extends RefCounted
class_name ROTHInstallation

enum Region {
	UNKNOWN,
	UK,
	US,
	GERMANY,
	ITALY,
	FRANCE,
	SPAIN,
}

const OLD_EXE: String = "3.925"
const NEW_EXE: String = "3.983"

#region MD5Sums
const MD5: Dictionary = {
	Region.UK: {
		"ABADON1A.GDV": "875aaa7bfca53c1446112cf7fd01017c",
		"ABADON1B.GDV": "d24c1f5b27801235bd8728d709f23770",
		"ABADON1C.GDV": "4e35006def9dd053738cc13acb15cbad",
		"ABADON1D.GDV": "8a35c55c5da228c35512bd2882c978b7",
		"ABADON1E.GDV": "45722eddec4f3ffadd054ae6dd5b5524",
		"ABADON1F.GDV": "9a3aca815015424e8022eaa50d570bde",
		"ABADON1G.GDV": "31b2dd2b8f1e0fe464030a236f0735e4",
		"ABADON1H.GDV": "cc65983ce4aed5792ea74ad547cf9ceb",
		"ABADON2A.GDV": "bd1e376c103475f3e683661490108050",
		"ABADON2B.GDV": "d2e1c549251414133bdcd18df854e88e",
		"ABADON2C.GDV": "7a60e9442ba746c3fd772feb104e3ff9",
		"ABADON2D.GDV": "de254f0e379d99a057ab910d3dcb9bc9",
		"ABADON2E.GDV": "74f92a8243a75804452e7afef15d7730",
		"ABADON2F.GDV": "5e9ffb2563e37dedc35fa2600c88e71d",
		"ABADON2G.GDV": "5e603755d9db2d39a7c3cc5cc3e53540",
		"ABADON2H.GDV": "50bfe19b3362260dd3c0190d132a662c",
		"ABADON2I.GDV": "03eaa41aea9234e8c2de7888b9718cdd",
		"ABAGATE2.RAW": "0b8ceef19cd1243c866da3dcd8884f79",
		"ADEMO.DAS": "724ce6eb75a1b9e1a3eecceace333de0",
		"AELF.RAW": "1951e11021028b183e96e0ceabd29e94",
		"ALEF01.GDV": "729724083ca9bae214d28d01edefa284",
		"ALEF02.GDV": "eae5f4e10eb155294db5a90d09c49d07",
		"ALEF03A.GDV": "9fcb12f156e3cccdd54612c057676b4e",
		"ALEF03B.GDV": "91fa9e052f3bc0e3e80bb582ab810089",
		"ALEF03C.GDV": "c05cd355a5cc736d36121029ce85de35",
		"ALEF03D.GDV": "3bda98527ce8113bc46a903a1affaead",
		"ALEF04.GDV": "a125c262df84409987274fe4b882bf30",
		"ANUBIS.RAW": "cb4acdf0fc7bdd0f38ac31961e33747a",
		"AQUA1.RAW": "3d3580ca0c2ad5fce5472e5bd8c3ecd3",
		"AQUA2.RAW": "813d6ac1dad118a676494b7a82a3853f",
		"BACKDROP.RAW": "2c9debcf210c1bfa089c5dbfae7d2f40",
		"BEAST.GDV": "390864b0022375df3d6ec81a16731049",
		"BELEXTRA.GDV": "8a046f2013373be648e53138e72286ad",
		"BELIAL1A.GDV": "db3d0dbc8770be4f3213081395282bf4",
		"BELIAL1B.GDV": "bddf517a87f1cd5fdf776124cc90acc8",
		"BELIAL1C.GDV": "db7ce3020cafc379556e36d11b8a1775",
		"BELIAL1E.GDV": "d82c7f65e5383fbbca666f979bc4d183",
		"BELIAL1F.GDV": "249554085619cbc8caf17e4fddf64ede",
		"BELSB1.GDV": "3816707c1621a31e01b8799852d344c9",
		"BELSB3.GDV": "908429e6eea3090c0e8c1fa4b7bd4a29",
		"BELSB4.GDV": "f2614e7ec40d93bd7a456da09a08f10b",
		"BELSB5.GDV": "4bedf46a3fbf079f8bb71423edfafeb8",
		"BELSB6.GDV": "787d823d73b2f1d072a98bb24ec7f89f",
		"BENDGMA.GDV": "a3f224130d8c919f877ff60c6d0a270f",
		"BENDGMB.GDV": "6f866fe69757f75b190ad9ee04bca2bf",
		"BETRAYAL.GDV": "5f64ddeee7ef7057335cb9073831bd44",
		"BRANDED.GDV": "a9004e3528ad00db36f51dc9f1570660",
		"CAVERNS.RAW": "259313cba21820f7290fd717cb909375",
		"CAVERNS2.RAW": "47fe249059d99efeec1b1bcba9e4e22e",
		"CAVERNS3.RAW": "f81ccfa715126a582218baa77f0ff375",
		"CHPTR1.GDV": "672fc19a89bbd5744eea21865f57a34e",
		"CHPTR10.GDV": "9254fe881894fa75d6c8ffd4bc244896",
		"CHPTR11.GDV": "d293040aa7a0439ab6788d39631c7aaa",
		"CHPTR12.GDV": "7196f8f2c2b9f7baea7b01cd6064cbfc",
		"CHPTR13.GDV": "f922d21ddd11d1e351b631baf165f7ac",
		"CHPTR14.GDV": "cb03ee1d5ab644e4cf45aaba9f3aa10e",
		"CHPTR15.GDV": "232e05a22c14db00f22033d86d00bc10",
		"CHPTR16.GDV": "1014b61d1a6d60db4f2f1ac11c42a3a3",
		"CHPTR17.GDV": "d925b78468ae9264855cf68b6db0c059",
		"CHPTR18.GDV": "74028b3914f45a39e116395c569dede2",
		"CHPTR19.GDV": "55c3341995cb250f9530faafdb28e11d",
		"CHPTR2.GDV": "52eb2f44469827cddf284a31acf104c1",
		"CHPTR20.GDV": "1c1579b0948a9d847ccfea3b1c376031",
		"CHPTR3.GDV": "0c23ac243547d857f4bb419e34873091",
		"CHPTR4.GDV": "242874dc909a0d77e77220dbec218b14",
		"CHPTR5.GDV": "3c29e96f7aa0866b65c0084838eeb2b0",
		"CHPTR6.GDV": "cbf82a83bfdc9c087c70fc1c0d6a555b",
		"CHPTR7.GDV": "6861827ea71b552a03b524238c34bdb2",
		"CHPTR8.GDV": "16622a8c4f47a145767c20c513560db9",
		"CHPTR9.GDV": "e37cfe2dbd52934852abc774d16ff53c",
		"CHURCH1.RAW": "1932dddc70959c14767129f38042a15c",
		"CONFIG.INI": "c80c58cb5f91ad76e6731961f2908e9a",
		"DBASE100.DAT": "47eb55fa473006896979efc82603d35f",
		"DBASE200.DAT": "091d3e26b7b29c404d23634e2ed95d00",
		"DBASE300.DAT": "efbc5ca8c31f649a8537664da0c8a64e",
		"DBASE400.DAT": "7ad1200277fa68b64f1e07c01ed99b96",
		"DBASE500.DAT": "610bc520033f5e6c1bd9a0980481d991",
		"DEMO.DAS": "19b82c32cebb7004b457d8a30e2d2afa",
		"DEMO1.DAS": "4d6963b6709e1ddb2b5f51967b0f88a6",
		"DEMO2.DAS": "5ce067e2ba96be35fcde3c3ad729b169",
		"DEMO3.DAS": "e083da270769a0269091a527b6931fc6",
		"DEMO4.DAS": "37fc5e2daef6dc350c69a66b3299a7e0",
		"DODGERA.GDV": "e3aa122d551b18cb2c5b9d1765ff8e8a",
		"DOMINION.RAW": "746edcd7fd431131331462c59eebc4b1",
		"DOPPLE.GDV": "9e19bbdb187cf61b9148175ebf05788c",
		"DOPPLE.RAW": "b51ed2ab7b4070f69c58bee5341b8994",
		"DOS4GW.EXE": "006211ca60fa3d4b38a9a8da2ade1056",
		"DRUM.BNK": "7ecc2b70d2eb0d93475aca120b21e46b",
		"ECREDITS.GDV": "66df7243cc5994df451ae189b515ac8a",
		"EFFIGYA1.GDV": "1567dc85751a5f18bc20638643862964",
		"EFFIGYA2.GDV": "0edd09e532b5cce6c34af4152bef43f0",
		"EFFIGYA3.GDV": "846d3c69025027ffd0f486df94c376bf",
		"EFFIGYB.GDV": "475b7568d9fd044d1fd7c9406bab47ff",
		"EFFIGYC.GDV": "a663e9ad15aa68fa3926b8d2ab5d3df7",
		"ELOHIM1.RAW": "fdc5cfadf9e4cb8abd40b53a59c99acb",
		"ENCHANT.GDV": "40994a4dc49624e3f2b1b4cd78becf42",
		"EPILOGUE.GDV": "48d6b7436da7a9979516f8609b0fa26f",
		"FALSHIRE.GDV": "04bcafafa36f3c90496b4dab0ed823c3",
		"FILELIST.TXT": "87ea749e253b472e869394270190ece0",
		"FLOTRAP1.GDV": "9ea90ea37a012e7e8de4e0a50692275c",
		"FLOTRAP2.GDV": "e76071aeae4ce6484a6b683e892095fb",
		"FLOTRAP3.GDV": "7c8a56fe81199eed0aa6ed52593916f2",
		"FLOTRAP4.GDV": "7e9a893debd3a7384e819f4b0423f969",
		"FLOTRAP5.GDV": "dc4f8d9fde0c3465074e8056e34ec50a",
		"FLOTRAP6.GDV": "aa9c4a86a478c7ca4b7e0be8d33746c2",
		"FULLKEEP.GDV": "00a9a18389dd1eb9ded3bcf82df86899",
		"FX22.SFX": "3999135d197890ce4aad9895112ebc91",
		"FXSCRIPT.SFX": "6391d983dc94a976f6d20bc5c421043b",
		"GARDENS.GDV": "4c3c82dd8002932c098bd505427bb005",
		"GATE01A.GDV": "84a4f0c4cf9a9e7859362ade746625ae",
		"GATE01B.GDV": "1612aed2e6be65225223ffa60b95608a",
		"GAUL01.GDV": "bbd391860f071617c9c70300576d182b",
		"GAUL02.GDV": "89450060c4866c779d1ac9b647847a44",
		"GLASSWIN.GDV": "1bb9b8c4ef95e67162c38c6f74f90cb7",
		"GNARL1.RAW": "c2fa03b29464080244b5256bef2bf5b5",
		"GNARLA.GDV": "28cadf8aaa893304dad49a8556f90024",
		"GNARLB.GDV": "4525f188f5825977ca4a19b0b9e641c6",
		"GNARLC.GDV": "a67dd5242a85958b6f9344d5e8d74ead",
		"GNARLD.GDV": "b92d5d141bb634bddd1fcdf81ba06d6c",
		"GNARLE.GDV": "eaa0bdddbaf52934d90a370a1be4fb26",
		"GNARLF.GDV": "f3131702247865c34b82506c61e6d377",
		"GNARLG.GDV": "5647a8f75b68007145ad35e8ef1d2019",
		"GNARLH.GDV": "bdb97568b914f5b82fcb0a158cca2b3c",
		"GNARLI.GDV": "315ccd97bc69364edd17c344b3776dd2",
		"GRAVE.RAW": "f7eccaba245cd3a202b6189993e65f94",
		"GRAVIS.INI": "e6f30ac9fe0d9f8d0a0f9bdb68ba0134",
		"GREMLOGO.GDV": "15ccc918aaedb72e5a4cd7f1efdf8ca5",
		"HAWK01.GDV": "645dcda9b38bce2f9917c2bf63916b63",
		"HAWK02A.GDV": "6cc31e20b51ac233064544fd3db34b31",
		"HAWK02B.GDV": "1e66a0bf9bd33f79d8ed2c744b2ffbb8",
		"HAWK02C.GDV": "26df75c923098b55eda03e1556543388",
		"HAWK02D.GDV": "4091ac3ebe3dee9498376d3349f581fb",
		"HAWK03A.GDV": "4f433f7304adca05ac564f19c4293b3a",
		"HAWK03B.GDV": "2452b0e4043fd712ac827954f0ec7e0b",
		"HAWK03C.GDV": "7b2333f700d757d16f58cd0736862f8b",
		"HAWK03D.GDV": "ba87faa563c90e5dc9b77730d38ba225",
		"HAWK03E.GDV": "52bc6c585c0211aa8fa94fdbf3be79b3",
		"HAWK03F.GDV": "654bd4bd8fc5657276871204843d0b22",
		"HAWK03G.GDV": "28e4c8f5a76308fffd96b311e9214539",
		"HAWK03H.GDV": "9e0614ea2951b149f8c6419e587deeb7",
		"HAWK03I.GDV": "112acd6ae8830f11226db0c00af7c197",
		"HAWK03J.GDV": "cbee5641d2426396de93769861e19ca3",
		"HAWK03K.GDV": "578d5b518654a635f6f711f8fc87ed32",
		"HAWK03L.GDV": "da5c030029fda7d7e87d0c902144b5ed",
		"HAWK03M.GDV": "9fc354ce13ab27db6b8cf489bddfab41",
		"HAWK03N.GDV": "7514b8a54219630abbe0afdf2b2a313c",
		"HAWK03O.GDV": "1858fce83c84c906395edb621a60c434",
		"HAWK03P.GDV": "541a57fde1579bab2b1db81e0c2b634f",
		"HAWK04.GDV": "1bec787204a3097d99354cdb779af824",
		"HMIDET.386": "59b3f813848cda336763648d628db917",
		"HMIDET.DRV": "f4e4ed8302f18aac0a8f8bb153a61566",
		"HMIDRV.386": "d730de4f9e6e4d8b95738f7cdde7c140",
		"HMIDRV.DRV": "cef61ca0b324c59d8e38c59ba1ddb191",
		"HMIMDRV.386": "ddfac2fec57dfd841ccfb64adee3b35e",
		"HMIMDRV.DRV": "59ad9c334c62a258b576728ef69d8af5",
		"ICONS.ALL": "db3f71fc7d13171768ff203d20777286",
		"INSTALL.EXE": "19af77f62aa8f966206595aad3df479a",
		"INSTALL.INI": "4b8f4a61d6f9dd01bc1ddd483959895d",
		"INSTALL.PIF": "2446579e6000298658f17d47a6bdab09",
		"INSTGRAV.INI": "8bf7b8cf7ba33e1353d2de82e142b41c",
		"INSTRA.GDV": "e91cad809509993f47f4fed50a9b4b11",
		"INSTRB.GDV": "b1690d999853bf4bfad0f02dcf7864f1",
		"INSTRC.GDV": "dea9065398ac8479e14cc35ca54dae39",
		"INSTRD.GDV": "c1b34a5f117e84ab36cb242b3007a495",
		"INSTRE.GDV": "9a3c607cb7c15b53aa63c7eda0f8aca1",
		"INSTRF.GDV": "111100aa7cdf3aac593154da8c8b9a64",
		"INSTRG.GDV": "460953cf67c706bbe918b62409e550d6",
		"INSTRH.GDV": "1fdbbb019c774521bd14da0b75ec1307",
		"INSTRI.GDV": "ad03008cf6748f79b6bb45137e2946df",
		"INTRO.GDV": "a83311895526e0ec1a4e829f424936ca",
		"LOADPATS.EXE": "b24134b3fab09b2727b49a0fe1d64d8e",
		"LRINTH.RAW": "d26817cc747828d10b7566387492c491",
		"LRINTH1.RAW": "a886ad8a3c6f98aeeb26f9d8b40d1275",
		"MAS3.RAW": "22db0686b64aee9f84261ac423922c56",
		"MAS4.RAW": "bd189628c95b2a854213105adeb6c3d4",
		"MAS6.RAW": "1e126f561296eeaa88df5490455558c1",
		"MAS7.RAW": "d0d5ffce0acab775fde690d3d6c9485a",
		"MAUSO1EA.RAW": "f2e0564a8177730dcb135588da40a04d",
		"MAUSO1EB.RAW": "7d03e78b3c4a849b98d2a6146bdd2d6d",
		"MAZE.RAW": "032fb3a2e1eb0a8f4e0f4463aafedd7b",
		"MELODIC.BNK": "2adb649534bd5cd78900d416149c9642",
		"MOVIE.GDV": "4bc7be9c6d275ad853b2c134f633bfa1",
		"MT32MAP.MTX": "23aa641a6b7a5855eecfb6fb48be7dc1",
		"OPTEMP1.RAW": "d3d1c4220bcbeb47306dff30f91c84b2",
		"RANDALL.GDV": "02d9067ac806695c9505534d4cb88fac",
		"RAPHAEL.GDV": "8840e6eb2fd07870efc46ad408e84198",
		"RAPHAELH.GDV": "c117cf84221096ce2b5db3519ba1add3",
		"RAPHAELI.GDV": "f2073bc3b53810670d1cbdc6ecf9c5a1",
		"RAPHAELJ.GDV": "8a9ab31edb96c1dc339142402d54b3a8",
		"RAPHAELK.GDV": "d0facd591f5021f2c7e9f685982f8468",
		"RAPHAELL.GDV": "9512ed59729f2a0d5a7c4ef57df6c0e8",
		"RAPHAELM.GDV": "361297e2cc7819611d2cef8f49fb9121",
		"RAPHAELN.GDV": "3006311f3f46151e7d904884b3ca1b50",
		"RAPHAELO.GDV": "caae9f764110d77b77a389e55e2123f2",
		"RAQUIA1.RAW": "8a990b37e82d828325cda02e1490b979",
		"RAQUIA2.RAW": "5ddb7d371939be302e0a34a868999093",
		"RAQUIA3.RAW": "ec482df7eb3f9acbb7a5a24b852b7c00",
		"RAQUIA4.RAW": "d48f9411bbb6d2602e4bc427b4931719",
		"RAQUIA5.RAW": "73f904455a3af45de6c2e7979dd03c9c",
		"RATPFA.GDV": "3599f60e941faeb9c65c8ac5bb734322",
		"RATPFB.GDV": "5462b0c55fae0de44ea22c9983cfc7d8",
		"RAYSEL1A.GDV": "dd7e93e0514d2de40ff9a5f4db477bd5",
		"RAYSEL1B.GDV": "b8061c6fc3c2fcf21ab5bf47f83a5cd7",
		"RAYSEL1C.GDV": "00228ab5978641363e75958cf99329ef",
		"RAYSEL1D.GDV": "9a13da8f6f15335fe55ae01365ea52ed",
		"RAYSEL1E.GDV": "7fe81f2406b0182718138b2354114d49",
		"RAYSEL1F.GDV": "0869018790a2c760abf6c9ac188cecb6",
		"RAYSEL1G.GDV": "66257a4898e6874f26fa42749d21dedf",
		"README.TXT": "ae39856ded3215f7c7316533899c6761",
		"REALMS.BAT": "0c75f2e5343819642202c4edd5ddc564",
		"REBECCA.GDV": "2d1be567fad8344117a78774a0ea9991",
		"REFLECTA.GDV": "2ce5af633a46e594764ecbe81ef58548",
		"REFLECTB.GDV": "42d524b5457453761e14f1b8149213eb",
		"REFLECTC.GDV": "6880d727dc07088be07ade81484306a7",
		"RITUAL.GDV": "d109ad65f3d8949e69efb3f78631ba5d",
		"ROTH.EXE": "f0f93c7931b9a678469095d3d7f54c04",
		"ROTH.INI": "fc87215fb74f086fba326f835ef4a6ea",
		"ROTH.RES": "362c915b0635175e0edbe943550b5df9",
		"SALVAT.RAW": "9a2fd2e219b7897d2b08194b9b170c33",
		"SHRIVEA.GDV": "6a439308029985ff61e7a0a1c34fbb2c",
		"SHRIVEB.GDV": "020e664f0dba7bcab496d3a8070a4416",
		"SHRIVEC.GDV": "e53aa09129692f8cc9e0ca94f30d52fa",
		"SHRIVED.GDV": "888ab5cc02ed74434c8b33f451330339",
		"SOULST2.RAW": "5da1523903a0647f07b3522e47d1cc53",
		"SOULST3.RAW": "49ef092c5fe47327f54136da27c681e1",
		"STUDY1.RAW": "1e4ae31eef4a7d9b1fa268e9c0499157",
		"STUDY2.RAW": "0fbf64cef04e5bfd8e2fd0610ded4b0b",
		"STUDY3.RAW": "df6745247395fb067b01c59f8c8b887c",
		"STUDY4.RAW": "21e8a152ae59afc791c8edc91afc9edb",
		"TALEWELA.GDV": "a31a56078c1a20ee015d654e8f37e33b",
		"TALEWELB.GDV": "6ed62f75370dfe07e49e4a50d6a0b883",
		"TALEWELC.GDV": "d3b0ea25f9dd383dc9f4253fbc07eb09",
		"TEMPLE1.RAW": "3db310591029edd93a745168ecabb77e",
		"TGATE1F.RAW": "01549e438c65383c050ebc93e1ad1026",
		"TGATE1G.RAW": "c98b11832900dc61ae9fde46aef1488d",
		"TGATE1H.RAW": "01d8f012afd8f25d8a10a6352d316272",
		"TGATE1I.RAW": "7e590a67b84c2f0dee9dcd918f7a51e5",
		"TOWER1.RAW": "d6efe0ea2e4f5c2d47ee8ecb360f0776",
		"TUNNWARP.GDV": "28ac2c20475d62c3c317d60feb01b426",
		"VERSE.GDV": "3d799d2feedec79dda19170b980aeef4",
		"VICAR.RAW": "cf623746c15d5af413931c123f9621f2",
		"VICAR1.RAW": "9b676ff1eea0293958291c0812fac4ef",
		"WALKWAY1.GDV": "3d753d31c409e79a5488be7f7f6a4444",
		"WALKWAY2.GDV": "eb32b94861dca6ecc4e19127d09cdf98",
		"WALKWAY3.GDV": "2bc5b4ef929abb5007799d4d1df09f7a",
		"WALKWY1A.GDV": "00bfbd5caf042f1b824257e4081f03d4",
		"WIN95RUN.PIF": "d863b3e75aec130d6957aa714c70298e",
	},
	Region.US: {
		"ABADON1A.GDV": "875aaa7bfca53c1446112cf7fd01017c",
		"ABADON1B.GDV": "d24c1f5b27801235bd8728d709f23770",
		"ABADON1C.GDV": "4e35006def9dd053738cc13acb15cbad",
		"ABADON1D.GDV": "8a35c55c5da228c35512bd2882c978b7",
		"ABADON1E.GDV": "45722eddec4f3ffadd054ae6dd5b5524",
		"ABADON1F.GDV": "9a3aca815015424e8022eaa50d570bde",
		"ABADON1G.GDV": "31b2dd2b8f1e0fe464030a236f0735e4",
		"ABADON1H.GDV": "cc65983ce4aed5792ea74ad547cf9ceb",
		"ABADON2A.GDV": "bd1e376c103475f3e683661490108050",
		"ABADON2B.GDV": "d2e1c549251414133bdcd18df854e88e",
		"ABADON2C.GDV": "7a60e9442ba746c3fd772feb104e3ff9",
		"ABADON2D.GDV": "de254f0e379d99a057ab910d3dcb9bc9",
		"ABADON2E.GDV": "74f92a8243a75804452e7afef15d7730",
		"ABADON2F.GDV": "5e9ffb2563e37dedc35fa2600c88e71d",
		"ABADON2G.GDV": "5e603755d9db2d39a7c3cc5cc3e53540",
		"ABADON2H.GDV": "50bfe19b3362260dd3c0190d132a662c",
		"ABADON2I.GDV": "03eaa41aea9234e8c2de7888b9718cdd",
		"ABAGATE2.RAW": "36a05ba9452fd5a14249a06cf78a304a",
		"ADEMO.DAS": "724ce6eb75a1b9e1a3eecceace333de0",
		"AELF.RAW": "615223ddd754fc89641c2c50a6fa5f84",
		"ALEF01.GDV": "729724083ca9bae214d28d01edefa284",
		"ALEF02.GDV": "eae5f4e10eb155294db5a90d09c49d07",
		"ALEF03A.GDV": "9fcb12f156e3cccdd54612c057676b4e",
		"ALEF03B.GDV": "91fa9e052f3bc0e3e80bb582ab810089",
		"ALEF03C.GDV": "c05cd355a5cc736d36121029ce85de35",
		"ALEF03D.GDV": "3bda98527ce8113bc46a903a1affaead",
		"ALEF04.GDV": "a125c262df84409987274fe4b882bf30",
		"ANUBIS.RAW": "799026a5e1574e5a312601b02ce93879",
		"AQUA1.RAW": "359b9a370554826a7482422487e5155c",
		"AQUA2.RAW": "813d6ac1dad118a676494b7a82a3853f",
		"BACKDROP.RAW": "2c9debcf210c1bfa089c5dbfae7d2f40",
		"BEAST.GDV": "390864b0022375df3d6ec81a16731049",
		"BELEXTRA.GDV": "8a046f2013373be648e53138e72286ad",
		"BELIAL1A.GDV": "db3d0dbc8770be4f3213081395282bf4",
		"BELIAL1B.GDV": "bddf517a87f1cd5fdf776124cc90acc8",
		"BELIAL1C.GDV": "db7ce3020cafc379556e36d11b8a1775",
		"BELIAL1E.GDV": "d82c7f65e5383fbbca666f979bc4d183",
		"BELIAL1F.GDV": "249554085619cbc8caf17e4fddf64ede",
		"BELSB1.GDV": "3816707c1621a31e01b8799852d344c9",
		"BELSB3.GDV": "908429e6eea3090c0e8c1fa4b7bd4a29",
		"BELSB4.GDV": "f2614e7ec40d93bd7a456da09a08f10b",
		"BELSB5.GDV": "4bedf46a3fbf079f8bb71423edfafeb8",
		"BELSB6.GDV": "787d823d73b2f1d072a98bb24ec7f89f",
		"BENDGMA.GDV": "a3f224130d8c919f877ff60c6d0a270f",
		"BENDGMB.GDV": "6f866fe69757f75b190ad9ee04bca2bf",
		"BETRAYAL.GDV": "5f64ddeee7ef7057335cb9073831bd44",
		"BRANDED.GDV": "a9004e3528ad00db36f51dc9f1570660",
		"CAVERNS.RAW": "f8c281067b0cbb4377cd88ed1468765f",
		"CAVERNS2.RAW": "3305d61219ef7697d77e04fe3c250c72",
		"CAVERNS3.RAW": "8e3ea4e4789f1521cfb7db1dd2266158",
		"CHPTR1.GDV": "672fc19a89bbd5744eea21865f57a34e",
		"CHPTR10.GDV": "9254fe881894fa75d6c8ffd4bc244896",
		"CHPTR11.GDV": "d293040aa7a0439ab6788d39631c7aaa",
		"CHPTR12.GDV": "7196f8f2c2b9f7baea7b01cd6064cbfc",
		"CHPTR13.GDV": "f922d21ddd11d1e351b631baf165f7ac",
		"CHPTR14.GDV": "cb03ee1d5ab644e4cf45aaba9f3aa10e",
		"CHPTR15.GDV": "232e05a22c14db00f22033d86d00bc10",
		"CHPTR16.GDV": "1014b61d1a6d60db4f2f1ac11c42a3a3",
		"CHPTR17.GDV": "d925b78468ae9264855cf68b6db0c059",
		"CHPTR18.GDV": "74028b3914f45a39e116395c569dede2",
		"CHPTR19.GDV": "55c3341995cb250f9530faafdb28e11d",
		"CHPTR2.GDV": "52eb2f44469827cddf284a31acf104c1",
		"CHPTR20.GDV": "1c1579b0948a9d847ccfea3b1c376031",
		"CHPTR3.GDV": "0c23ac243547d857f4bb419e34873091",
		"CHPTR4.GDV": "242874dc909a0d77e77220dbec218b14",
		"CHPTR5.GDV": "3c29e96f7aa0866b65c0084838eeb2b0",
		"CHPTR6.GDV": "cbf82a83bfdc9c087c70fc1c0d6a555b",
		"CHPTR7.GDV": "6861827ea71b552a03b524238c34bdb2",
		"CHPTR8.GDV": "16622a8c4f47a145767c20c513560db9",
		"CHPTR9.GDV": "e37cfe2dbd52934852abc774d16ff53c",
		"CHURCH1.RAW": "6b9c0a0bc540feebd2b66a30a30c0d3d",
		"CONFIG.INI": "71912cd9dec35375a0eb81f31c87120e",
		"DBASE100.DAT": "cc543fcae705081e259dbce3c3617dc7",
		"DBASE200.DAT": "091d3e26b7b29c404d23634e2ed95d00",
		"DBASE300.DAT": "1cfa7a08b7cd7c881b3ecb02f466421d",
		"DBASE400.DAT": "00001c12d9e773e82252d74f0b1ce7fa",
		"DBASE500.DAT": "e8639ae1dbc6bd71d6f5d8277b149a8e",
		"DEMO.DAS": "19b82c32cebb7004b457d8a30e2d2afa",
		"DEMO1.DAS": "4d6963b6709e1ddb2b5f51967b0f88a6",
		"DEMO2.DAS": "1829a6e04d3eb32bd12a7804816efbdb",
		"DEMO3.DAS": "e083da270769a0269091a527b6931fc6",
		"DEMO4.DAS": "37fc5e2daef6dc350c69a66b3299a7e0",
		"DODGERA.GDV": "e3aa122d551b18cb2c5b9d1765ff8e8a",
		"DOMINION.RAW": "44896acbc3f57ff4d546754bc60d1514",
		"DOPPLE.GDV": "9e19bbdb187cf61b9148175ebf05788c",
		"DOPPLE.RAW": "6ef4d4f64212f65904b1d065270f9393",
		"DOS4GW.EXE": "006211ca60fa3d4b38a9a8da2ade1056",
		"DRUM.BNK": "7ecc2b70d2eb0d93475aca120b21e46b",
		"ECREDITS.GDV": "0093c79c05f6cceeafce49ac5e0b1cba",
		"EFFIGYA1.GDV": "1567dc85751a5f18bc20638643862964",
		"EFFIGYA2.GDV": "0edd09e532b5cce6c34af4152bef43f0",
		"EFFIGYA3.GDV": "846d3c69025027ffd0f486df94c376bf",
		"EFFIGYB.GDV": "475b7568d9fd044d1fd7c9406bab47ff",
		"EFFIGYC.GDV": "a663e9ad15aa68fa3926b8d2ab5d3df7",
		"ELOHIM1.RAW": "608a9d479938d1bfa0807f94439b1954",
		"ENCHANT.GDV": "40994a4dc49624e3f2b1b4cd78becf42",
		"EPILOGUE.GDV": "48d6b7436da7a9979516f8609b0fa26f",
		"FALSHIRE.GDV": "04bcafafa36f3c90496b4dab0ed823c3",
		"FILELIST.TXT": "5fd492d8a0da6b5a332ab93171f443c6",
		"FLOTRAP1.GDV": "9ea90ea37a012e7e8de4e0a50692275c",
		"FLOTRAP2.GDV": "e76071aeae4ce6484a6b683e892095fb",
		"FLOTRAP3.GDV": "7c8a56fe81199eed0aa6ed52593916f2",
		"FLOTRAP4.GDV": "7e9a893debd3a7384e819f4b0423f969",
		"FLOTRAP5.GDV": "dc4f8d9fde0c3465074e8056e34ec50a",
		"FLOTRAP6.GDV": "aa9c4a86a478c7ca4b7e0be8d33746c2",
		"FULLKEEP.GDV": "00a9a18389dd1eb9ded3bcf82df86899",
		"FX22.SFX": "3999135d197890ce4aad9895112ebc91",
		"FXSCRIPT.SFX": "6391d983dc94a976f6d20bc5c421043b",
		"GARDENS.GDV": "4c3c82dd8002932c098bd505427bb005",
		"GATE01A.GDV": "84a4f0c4cf9a9e7859362ade746625ae",
		"GATE01B.GDV": "1612aed2e6be65225223ffa60b95608a",
		"GAUL01.GDV": "bbd391860f071617c9c70300576d182b",
		"GAUL02.GDV": "89450060c4866c779d1ac9b647847a44",
		"GLASSWIN.GDV": "1bb9b8c4ef95e67162c38c6f74f90cb7",
		"GNARL1.RAW": "a7a8e89d195151ee4acdf8a85e9fa4c7",
		"GNARLA.GDV": "28cadf8aaa893304dad49a8556f90024",
		"GNARLB.GDV": "4525f188f5825977ca4a19b0b9e641c6",
		"GNARLC.GDV": "a67dd5242a85958b6f9344d5e8d74ead",
		"GNARLD.GDV": "b92d5d141bb634bddd1fcdf81ba06d6c",
		"GNARLE.GDV": "eaa0bdddbaf52934d90a370a1be4fb26",
		"GNARLF.GDV": "f3131702247865c34b82506c61e6d377",
		"GNARLG.GDV": "5647a8f75b68007145ad35e8ef1d2019",
		"GNARLH.GDV": "bdb97568b914f5b82fcb0a158cca2b3c",
		"GNARLI.GDV": "315ccd97bc69364edd17c344b3776dd2",
		"GRAVE.RAW": "2a943d9ede5841111a79a621c471cbca",
		"GRAVIS.INI": "e6f30ac9fe0d9f8d0a0f9bdb68ba0134",
		"GREMLOGO.GDV": "15ccc918aaedb72e5a4cd7f1efdf8ca5",
		"HAWK01.GDV": "645dcda9b38bce2f9917c2bf63916b63",
		"HAWK02A.GDV": "6cc31e20b51ac233064544fd3db34b31",
		"HAWK02B.GDV": "1e66a0bf9bd33f79d8ed2c744b2ffbb8",
		"HAWK02C.GDV": "26df75c923098b55eda03e1556543388",
		"HAWK02D.GDV": "4091ac3ebe3dee9498376d3349f581fb",
		"HAWK03A.GDV": "4f433f7304adca05ac564f19c4293b3a",
		"HAWK03B.GDV": "2452b0e4043fd712ac827954f0ec7e0b",
		"HAWK03C.GDV": "7b2333f700d757d16f58cd0736862f8b",
		"HAWK03D.GDV": "ba87faa563c90e5dc9b77730d38ba225",
		"HAWK03E.GDV": "52bc6c585c0211aa8fa94fdbf3be79b3",
		"HAWK03F.GDV": "654bd4bd8fc5657276871204843d0b22",
		"HAWK03G.GDV": "28e4c8f5a76308fffd96b311e9214539",
		"HAWK03H.GDV": "9e0614ea2951b149f8c6419e587deeb7",
		"HAWK03I.GDV": "112acd6ae8830f11226db0c00af7c197",
		"HAWK03J.GDV": "cbee5641d2426396de93769861e19ca3",
		"HAWK03K.GDV": "578d5b518654a635f6f711f8fc87ed32",
		"HAWK03L.GDV": "da5c030029fda7d7e87d0c902144b5ed",
		"HAWK03M.GDV": "9fc354ce13ab27db6b8cf489bddfab41",
		"HAWK03N.GDV": "7514b8a54219630abbe0afdf2b2a313c",
		"HAWK03O.GDV": "1858fce83c84c906395edb621a60c434",
		"HAWK03P.GDV": "541a57fde1579bab2b1db81e0c2b634f",
		"HAWK04.GDV": "1bec787204a3097d99354cdb779af824",
		"HMIDET.386": "59b3f813848cda336763648d628db917",
		"HMIDET.DRV": "f4e4ed8302f18aac0a8f8bb153a61566",
		"HMIDRV.386": "d730de4f9e6e4d8b95738f7cdde7c140",
		"HMIDRV.DRV": "cef61ca0b324c59d8e38c59ba1ddb191",
		"HMIMDRV.386": "ddfac2fec57dfd841ccfb64adee3b35e",
		"HMIMDRV.DRV": "59ad9c334c62a258b576728ef69d8af5",
		"ICONS.ALL": "91c4c33339cc0661be9568d79ead8911",
		"INSTALL.EXE": "10c0ddd74a3aa6a1f850beea96c80c25",
		"INSTALL.INI": "59a1f83e3f53234bd0824e017a786356",
		"INSTALL.PIF": "ff7009cc1aa16f626871e6bc06e56e43",
		"INSTGRAV.INI": "8bf7b8cf7ba33e1353d2de82e142b41c",
		"INSTRA.GDV": "e91cad809509993f47f4fed50a9b4b11",
		"INSTRB.GDV": "b1690d999853bf4bfad0f02dcf7864f1",
		"INSTRC.GDV": "dea9065398ac8479e14cc35ca54dae39",
		"INSTRD.GDV": "c1b34a5f117e84ab36cb242b3007a495",
		"INSTRE.GDV": "9a3c607cb7c15b53aa63c7eda0f8aca1",
		"INSTRF.GDV": "111100aa7cdf3aac593154da8c8b9a64",
		"INSTRG.GDV": "460953cf67c706bbe918b62409e550d6",
		"INSTRH.GDV": "1fdbbb019c774521bd14da0b75ec1307",
		"INSTRI.GDV": "ad03008cf6748f79b6bb45137e2946df",
		"INTRO.GDV": "cdf16beefe4cc719b56b1d74318e18b8",
		"LOADPATS.EXE": "b24134b3fab09b2727b49a0fe1d64d8e",
		"LRINTH.RAW": "c360fa64b41a962ef74715a752e2a601",
		"LRINTH1.RAW": "c33ea660d0195daadaefb9c4ac6d4973",
		"MAS3.RAW": "9433fe3c8650f02c17f660f48ed96105",
		"MAS4.RAW": "495899fadc5e0aed8bf4629f1dd10742",
		"MAS6.RAW": "1e126f561296eeaa88df5490455558c1",
		"MAS7.RAW": "ea15ea4a67b9a7953b5066821953a002",
		"MAUSO1EA.RAW": "a0b76820c14600753823f6bd723c5ff4",
		"MAUSO1EB.RAW": "fa68e3e52147984fad267fad99febd0a",
		"MAZE.RAW": "756dc91868d45eb57c6c67345f414b1d",
		"MELODIC.BNK": "2adb649534bd5cd78900d416149c9642",
		"MOVIE.GDV": "4bc7be9c6d275ad853b2c134f633bfa1",
		"MT32MAP.MTX": "23aa641a6b7a5855eecfb6fb48be7dc1",
		"OPTEMP1.RAW": "474639af854a2703e63cd812244b6164",
		"RANDALL.GDV": "02d9067ac806695c9505534d4cb88fac",
		"RAPHAEL.GDV": "8840e6eb2fd07870efc46ad408e84198",
		"RAPHAELH.GDV": "c117cf84221096ce2b5db3519ba1add3",
		"RAPHAELI.GDV": "f2073bc3b53810670d1cbdc6ecf9c5a1",
		"RAPHAELJ.GDV": "8a9ab31edb96c1dc339142402d54b3a8",
		"RAPHAELK.GDV": "d0facd591f5021f2c7e9f685982f8468",
		"RAPHAELL.GDV": "9512ed59729f2a0d5a7c4ef57df6c0e8",
		"RAPHAELM.GDV": "361297e2cc7819611d2cef8f49fb9121",
		"RAPHAELN.GDV": "3006311f3f46151e7d904884b3ca1b50",
		"RAPHAELO.GDV": "caae9f764110d77b77a389e55e2123f2",
		"RAQUIA1.RAW": "8a990b37e82d828325cda02e1490b979",
		"RAQUIA2.RAW": "5ddb7d371939be302e0a34a868999093",
		"RAQUIA3.RAW": "818341506472bb50a6ae8d7f1e586092",
		"RAQUIA4.RAW": "84e8913763e0be3f5122b3708e006c95",
		"RAQUIA5.RAW": "35392803892e846d34e1bbdfabbb79ab",
		"RATPFA.GDV": "3599f60e941faeb9c65c8ac5bb734322",
		"RATPFB.GDV": "5462b0c55fae0de44ea22c9983cfc7d8",
		"RAYSEL1A.GDV": "dd7e93e0514d2de40ff9a5f4db477bd5",
		"RAYSEL1B.GDV": "b8061c6fc3c2fcf21ab5bf47f83a5cd7",
		"RAYSEL1C.GDV": "00228ab5978641363e75958cf99329ef",
		"RAYSEL1D.GDV": "9a13da8f6f15335fe55ae01365ea52ed",
		"RAYSEL1E.GDV": "7fe81f2406b0182718138b2354114d49",
		"RAYSEL1F.GDV": "0869018790a2c760abf6c9ac188cecb6",
		"RAYSEL1G.GDV": "66257a4898e6874f26fa42749d21dedf",
		"README.TXT": "e4d4824c3d7a1492438813b9cbf13ba6",
		"REALMS.BAT": "dc657e5d8e011c99cfb6de2f2fac164d",
		"REBECCA.GDV": "2d1be567fad8344117a78774a0ea9991",
		"REFLECTA.GDV": "2ce5af633a46e594764ecbe81ef58548",
		"REFLECTB.GDV": "42d524b5457453761e14f1b8149213eb",
		"REFLECTC.GDV": "6880d727dc07088be07ade81484306a7",
		"RITUAL.GDV": "d109ad65f3d8949e69efb3f78631ba5d",
		"ROTH.EXE": "f588469eb868373a339bebb5fba5a9bb",
		"ROTH.INI": "e024d03734fd0996139e3dca1b0da677",
		"ROTH.RES": "c0247b5d64b43bbf1e7edb12198fbb72",
		"SALVAT.RAW": "53b13eeb5ab7fec93515dd054e62aa08",
		"SHRIVEA.GDV": "6a439308029985ff61e7a0a1c34fbb2c",
		"SHRIVEB.GDV": "020e664f0dba7bcab496d3a8070a4416",
		"SHRIVEC.GDV": "e53aa09129692f8cc9e0ca94f30d52fa",
		"SHRIVED.GDV": "888ab5cc02ed74434c8b33f451330339",
		"SOULST2.RAW": "f0cad71c8afc062d53608ef11cb6d909",
		"SOULST3.RAW": "a8e7a235bc9abcbd77b48b53edda6d65",
		"STUDY1.RAW": "bfa4969d04e8d3b7733422e48416b0de",
		"STUDY2.RAW": "08ff34b8a5fd3960e87348249baee7c5",
		"STUDY3.RAW": "470b41d230e77e43af5f61b83586fe1a",
		"STUDY4.RAW": "5d35a447593aca6456bbf7661db072bf",
		"TALEWELA.GDV": "a31a56078c1a20ee015d654e8f37e33b",
		"TALEWELB.GDV": "6ed62f75370dfe07e49e4a50d6a0b883",
		"TALEWELC.GDV": "d3b0ea25f9dd383dc9f4253fbc07eb09",
		"TEMPLE1.RAW": "b3ce9b844300a9770bb1b81f9fc28b9b",
		"TGATE1F.RAW": "156e4258fe3ee25689f74e215a8f1e13",
		"TGATE1G.RAW": "1027ec59614318cae2d0e8b51bcaf54b",
		"TGATE1H.RAW": "23e6938f1d51a510c60fd72f961cbac9",
		"TGATE1I.RAW": "7e590a67b84c2f0dee9dcd918f7a51e5",
		"TOWER1.RAW": "b3a9b47f44776e99c37c626cb974a1b2",
		"TUNNWARP.GDV": "28ac2c20475d62c3c317d60feb01b426",
		"VERSE.GDV": "3d799d2feedec79dda19170b980aeef4",
		"VICAR.RAW": "6d0916d51019a8dda162240aa72d4abf",
		"VICAR1.RAW": "7fd18c3fe1800bc607fe5a1cf30bb024",
		"WALKWAY1.GDV": "3d753d31c409e79a5488be7f7f6a4444",
		"WALKWAY2.GDV": "eb32b94861dca6ecc4e19127d09cdf98",
		"WALKWAY3.GDV": "2bc5b4ef929abb5007799d4d1df09f7a",
		"WALKWY1A.GDV": "00bfbd5caf042f1b824257e4081f03d4",
		"WIN95RUN.PIF": "3313c20924fc9fa83319c34ea840f264",
	},
	Region.FRANCE: {
		"ABADON1A.GDV": "198681930f75d2acba6ebe8400c20575",
		"ABADON1B.GDV": "e0e752a871e3d3910e4319336a890dbd",
		"ABADON1C.GDV": "65386c465c6ca715d60ee36674d26f1a",
		"ABADON1D.GDV": "62d7d15fffd23a31ebccd605c958da9f",
		"ABADON1E.GDV": "6cc94a21a74e942492fb61ea7912c89f",
		"ABADON1F.GDV": "b691a32a77d92c44d1891e36b4202cfc",
		"ABADON1G.GDV": "d45a891866700dc1940984e614326598",
		"ABADON1H.GDV": "a1d2a07e81926c4cb3c14b6901fa1255",
		"ABADON2A.GDV": "b965b9701262e3d9a4b758fd151941e5",
		"ABADON2B.GDV": "73ed0118ef0becb6d26ef326b8ef9ee8",
		"ABADON2C.GDV": "2ac79da19975c6641f059eb9896d56c6",
		"ABADON2D.GDV": "31d172d8a0d3bccec65a2df9af241f2a",
		"ABADON2E.GDV": "b3876d6f848e7ccae9e156d7dea9fd7e",
		"ABADON2F.GDV": "45f6f4d8293e5746cc62e73b80ec683e",
		"ABADON2G.GDV": "47d07dacbdfbf906fd0755c4ab314cb1",
		"ABADON2H.GDV": "733e148feb28d6961fb7797525a5fa8e",
		"ABADON2I.GDV": "9491cf365800d4e4bceecb82542ce467",
		"ABAGATE2.RAW": "36a05ba9452fd5a14249a06cf78a304a",
		"ADEMO.DAS": "724ce6eb75a1b9e1a3eecceace333de0",
		"AELF.RAW": "615223ddd754fc89641c2c50a6fa5f84",
		"ALEF01.GDV": "99633e4fcdc7950e60b9dc7d12338f8d",
		"ALEF02.GDV": "99fe5ed3f6a0c345c3fcbd8b4225eed5",
		"ALEF03A.GDV": "4285d55d54911dac7ffa7511c57d35c7",
		"ALEF03B.GDV": "fa5fc74d78b1e8c823c31e7865f3a63b",
		"ALEF03C.GDV": "ba747b413087749d8683df6ec48016cf",
		"ALEF03D.GDV": "b5d3964ba8567e33ebd744203a7f7d6f",
		"ALEF04.GDV": "ebd56a36c2a09ff871be6fa4690d2a80",
		"ANUBIS.RAW": "f3d0726ca2f54c7592f0db18f44b8fdb",
		"AQUA1.RAW": "359b9a370554826a7482422487e5155c",
		"AQUA2.RAW": "813d6ac1dad118a676494b7a82a3853f",
		"BACKDROP.RAW": "2c9debcf210c1bfa089c5dbfae7d2f40",
		"BEAST.GDV": "1347f1c642265a82ce75c2a8e2e2f14d",
		"BELEXTRA.GDV": "80c6093fe99d7197a95f2682187dc4cb",
		"BELIAL1A.GDV": "dc34c7ed37dc602b48f272de71a6b691",
		"BELIAL1B.GDV": "35fade5c94a49d21ccc81e48fe5ead8a",
		"BELIAL1C.GDV": "e0051db6dfb8023f5943a073ba32d367",
		"BELIAL1E.GDV": "8a57e7fdcd6622ef266e4c0a5e25e1b3",
		"BELIAL1F.GDV": "4afe9d080d3e606d8886476d95910745",
		"BELSB1.GDV": "474c211fc04709f4c7b59aa404b2cd71",
		"BELSB3.GDV": "2b26c85542935fcbfed72dea9171182b",
		"BELSB4.GDV": "f4e02abaef10490688803b20e2da0f40",
		"BELSB5.GDV": "94c05f6d36fde73fb2a746258c77f59e",
		"BELSB6.GDV": "3bf88d1630e21c954be6b47b4b4916dc",
		"BENDGMA.GDV": "a3f224130d8c919f877ff60c6d0a270f",
		"BENDGMB.GDV": "a185e6a2e67b9762edd5ff3552a1ad46",
		"BETRAYAL.GDV": "c4e055b6d946e72951d28fba596922e4",
		"BRANDED.GDV": "a9004e3528ad00db36f51dc9f1570660",
		"CAVERNS.RAW": "f8c281067b0cbb4377cd88ed1468765f",
		"CAVERNS2.RAW": "3305d61219ef7697d77e04fe3c250c72",
		"CAVERNS3.RAW": "8e3ea4e4789f1521cfb7db1dd2266158",
		"CHPTR1.GDV": "01834b0de1c39c002adbaeee2e4a953c",
		"CHPTR10.GDV": "b125e5649c59b670a5a7d66a98d5a6ae",
		"CHPTR11.GDV": "065a7a679d8c6b19a2aa7d00b6d2819c",
		"CHPTR12.GDV": "6cae39840afa7ed63f82bdde559bce07",
		"CHPTR13.GDV": "65a9034866a17f3e8e88eff0104be687",
		"CHPTR14.GDV": "e13f65019c586269b6907f2658aa34df",
		"CHPTR15.GDV": "094d4cc07e852f76150121e094ecfe41",
		"CHPTR16.GDV": "79e9a9084f9cd2bc42aba11be15bf782",
		"CHPTR17.GDV": "c256755664580abffc33b4dd427ce8d3",
		"CHPTR18.GDV": "7a73376ef9b01be588a1f8e5948afcda",
		"CHPTR19.GDV": "a9de9b409a0a179f6d44a2946680170c",
		"CHPTR2.GDV": "b9d21df9363410e2f8d4731b7cd7b964",
		"CHPTR20.GDV": "50a9ec50bba8372b8b7aea5057857731",
		"CHPTR3.GDV": "997a079438ee8621d1d4d54999fbb3f3",
		"CHPTR4.GDV": "7603d145db861a843850ce250c7adf9f",
		"CHPTR5.GDV": "8c91d6e1fd7e2a6dd0a0c38227dd2d62",
		"CHPTR6.GDV": "4c1839e2050abf3088c00d80ac6d3d7d",
		"CHPTR7.GDV": "1d0aedf7312823f1d57d5b7146bb2c43",
		"CHPTR8.GDV": "905da2aeaac3a653ba792a666fc75270",
		"CHPTR9.GDV": "50f8708768e40a4f627c1399b8a53efc",
		"CHURCH1.RAW": "6b9c0a0bc540feebd2b66a30a30c0d3d",
		"CONFIG.INI": "c72f32b7131062bacd46acdb389b9e3b",
		"DBASE100.DAT": "0021a814d9d90069426f5fd0dd107a90",
		"DBASE200.DAT": "636d371e190621f4a34d2a72d5f2e8cd",
		"DBASE300.DAT": "11e3c8330374496d87b691afb480e656",
		"DBASE400.DAT": "5e9d9d7bb012638a9b64c5e96e7dd938",
		"DBASE500.DAT": "a4300592576ea4c6cbd8d43bf19903eb",
		"DEMO.DAS": "19b82c32cebb7004b457d8a30e2d2afa",
		"DEMO1.DAS": "9d11f4ea57983b205881a7c33963d528",
		"DEMO2.DAS": "5ce067e2ba96be35fcde3c3ad729b169",
		"DEMO3.DAS": "e083da270769a0269091a527b6931fc6",
		"DEMO4.DAS": "2ee98b7626c64ca95c38e46dd5184376",
		"DODGERA.GDV": "c10d87a987a0e03c12b24faaa432d61b",
		"DOMINION.RAW": "44896acbc3f57ff4d546754bc60d1514",
		"DOPPLE.GDV": "bb0e2a83cf653f231dde69f12d633385",
		"DOPPLE.RAW": "6ef4d4f64212f65904b1d065270f9393",
		"DOS4GW.EXE": "006211ca60fa3d4b38a9a8da2ade1056",
		"DRUM.BNK": "7ecc2b70d2eb0d93475aca120b21e46b",
		"ECREDITS.GDV": "268a578e8f6b70a9cde5c24c26c0314a",
		"EFFIGYA1.GDV": "779d11d615f07701a2d15dbb4eaf5908",
		"EFFIGYA2.GDV": "792638bf3c9c71554ba25cf41ce9d3a1",
		"EFFIGYA3.GDV": "e80f0fd85f7de82b45d58fc1b0d99667",
		"EFFIGYB.GDV": "1f652a2da74dd46946bf7221528b70d9",
		"EFFIGYC.GDV": "dab8756891d1dc0dd3585de64bf45a84",
		"ELOHIM1.RAW": "608a9d479938d1bfa0807f94439b1954",
		"ENCHANT.GDV": "0ce68823a7b1fb5443494084cba7d015",
		"EPILOGUE.GDV": "457de81908e1234d8f063897cc0bfe1d",
		"FALSHIRE.GDV": "8772e20eb6e1a54f837615f3383f882a",
		"FILELIST.TXT": "87ea749e253b472e869394270190ece0",
		"FLOTRAP1.GDV": "3d44ce7072659f909f492837d19baa3b",
		"FLOTRAP2.GDV": "412ff4de9d72c79722f7914996681bce",
		"FLOTRAP3.GDV": "4fcd0682456c250923792b33ab59de69",
		"FLOTRAP4.GDV": "dbe873f1dd67d05d0780cce88a2034cc",
		"FLOTRAP5.GDV": "3b57f0af01c2d47742e3e4d1d10490dd",
		"FLOTRAP6.GDV": "d901cac8edfd3e864b4547eebf30fcd4",
		"FULLKEEP.GDV": "932aa441c543dd8fe975e1798ab2cdb0",
		"FX22.SFX": "3999135d197890ce4aad9895112ebc91",
		"FXSCRIPT.SFX": "6391d983dc94a976f6d20bc5c421043b",
		"GARDENS.GDV": "d667dad251da6ea84655dce3b5524066",
		"GATE01A.GDV": "5d0ee870c65aa9772dd52bc5717257a3",
		"GATE01B.GDV": "572cac5c481891b0cbb110567c295b36",
		"GAUL01.GDV": "562b61bb7ee527522a358b8d91e05519",
		"GAUL02.GDV": "b4dfd088707907c285daf8cc82ce11e1",
		"GLASSWIN.GDV": "063463e4a883f1785ae45e1f2d87a82e",
		"GNARL1.RAW": "a7a8e89d195151ee4acdf8a85e9fa4c7",
		"GNARLA.GDV": "b1480a7f7b6e4c744392b9801f574bb8",
		"GNARLB.GDV": "c4ae83a177cbed81da4a6d49b6bb72d2",
		"GNARLC.GDV": "af9298170e245c85de6786294315f4da",
		"GNARLD.GDV": "36295875179210f745182699a4af16a5",
		"GNARLE.GDV": "20765582e359833a2ab6aef97351f005",
		"GNARLF.GDV": "96f95172d54475449878bc250bf9d526",
		"GNARLG.GDV": "9e22ec94390136c44ccfc40261c06f51",
		"GNARLH.GDV": "b8780f3711ab8995b66bdd327e748009",
		"GNARLI.GDV": "315ccd97bc69364edd17c344b3776dd2",
		"GRAVE.RAW": "2a943d9ede5841111a79a621c471cbca",
		"GRAVIS.INI": "e6f30ac9fe0d9f8d0a0f9bdb68ba0134",
		"GREMLOGO.GDV": "15ccc918aaedb72e5a4cd7f1efdf8ca5",
		"HAWK01.GDV": "535e21f4e51239daa92bb6c87a0b6265",
		"HAWK02A.GDV": "f64dda5422c0bef37efbd79fd6045f92",
		"HAWK02B.GDV": "9ac61268a338d6d41a9f8cffe7a5c2bc",
		"HAWK02C.GDV": "7433848829b0f567fa287f68af560690",
		"HAWK02D.GDV": "69fd9f6525575b9bee637e80be8be8c8",
		"HAWK03A.GDV": "fcaaff879a467c00612c6a08db1e12e4",
		"HAWK03B.GDV": "7715f64496d0416d51a3c3e3b2ede87b",
		"HAWK03C.GDV": "fd9c7ccfb53ec5dac0c056730d5e4bc7",
		"HAWK03D.GDV": "dec653d649d1b945afa69ba3d67c34fe",
		"HAWK03E.GDV": "1c7a65da59159203037024a8ee1269fe",
		"HAWK03F.GDV": "130c33559fba6dbc6a66fea44c2667b6",
		"HAWK03G.GDV": "6487a3ec42a657a3677c4728cc721832",
		"HAWK03H.GDV": "090b9cda5f4e68e197cda1436c00228f",
		"HAWK03I.GDV": "cc8b926c5c78c7ad714787781a2be067",
		"HAWK03J.GDV": "97bb3fc4891d03187a69353f7721ea80",
		"HAWK03K.GDV": "221767d97642316dcbbd7a1b90fd0926",
		"HAWK03L.GDV": "61ca8150425cbd86cbce524d87f2dc32",
		"HAWK03M.GDV": "59f172ffe0f14de1fa4ec3d0371efac6",
		"HAWK03N.GDV": "f94007a3c074388fc2f1a948e256c11f",
		"HAWK03O.GDV": "c6538aa4a4afdd582c390ad4ce8fde16",
		"HAWK03P.GDV": "a2c98dd71073a9fadc0c43a8d4f26d2f",
		"HAWK04.GDV": "3f79157598c1b7016447b1fbdec520c4",
		"HMIDET.386": "59b3f813848cda336763648d628db917",
		"HMIDET.DRV": "f4e4ed8302f18aac0a8f8bb153a61566",
		"HMIDRV.386": "d730de4f9e6e4d8b95738f7cdde7c140",
		"HMIDRV.DRV": "cef61ca0b324c59d8e38c59ba1ddb191",
		"HMIMDRV.386": "ddfac2fec57dfd841ccfb64adee3b35e",
		"HMIMDRV.DRV": "59ad9c334c62a258b576728ef69d8af5",
		"ICONS.ALL": "db3f71fc7d13171768ff203d20777286",
		"INSTALL.EXE": "19af77f62aa8f966206595aad3df479a",
		"INSTALL.INI": "9f060661e20bf3bb44516f013fa538de",
		"INSTALL.PIF": "6ff7294dd3cbb469e3e947b2c49a29ea",
		"INSTGRAV.INI": "8bf7b8cf7ba33e1353d2de82e142b41c",
		"INSTRA.GDV": "e9fab3e3e3347c1b72c1a16c0cf1328c",
		"INSTRB.GDV": "3a655352bed29c1bbeefd5cae329c362",
		"INSTRC.GDV": "301c349f72403d755caea62282d3c426",
		"INSTRD.GDV": "fd788699c86174dc2b77975feb569111",
		"INSTRE.GDV": "5c5ec0be28ea41605dad384c06ac929a",
		"INSTRF.GDV": "2ee628c2d7e93053f71f870a42b66293",
		"INSTRG.GDV": "8f27a77170acaaaeb39384604c4a4713",
		"INSTRH.GDV": "30584ce5a9ab605d737a7af9e39458bc",
		"INSTRI.GDV": "28bb15337064cf8f78b15a2787a7124b",
		"INTRO.GDV": "3136e19e43e2b10b737716b4d05aeba1",
		"LOADPATS.EXE": "b24134b3fab09b2727b49a0fe1d64d8e",
		"LRINTH.RAW": "c360fa64b41a962ef74715a752e2a601",
		"LRINTH1.RAW": "c33ea660d0195daadaefb9c4ac6d4973",
		"MAS3.RAW": "9433fe3c8650f02c17f660f48ed96105",
		"MAS4.RAW": "495899fadc5e0aed8bf4629f1dd10742",
		"MAS6.RAW": "1e126f561296eeaa88df5490455558c1",
		"MAS7.RAW": "ea15ea4a67b9a7953b5066821953a002",
		"MAUSO1EA.RAW": "a0b76820c14600753823f6bd723c5ff4",
		"MAUSO1EB.RAW": "fa68e3e52147984fad267fad99febd0a",
		"MAZE.RAW": "756dc91868d45eb57c6c67345f414b1d",
		"MELODIC.BNK": "2adb649534bd5cd78900d416149c9642",
		"MOVIE.GDV": "4bc7be9c6d275ad853b2c134f633bfa1",
		"MT32MAP.MTX": "23aa641a6b7a5855eecfb6fb48be7dc1",
		"OPTEMP1.RAW": "474639af854a2703e63cd812244b6164",
		"RANDALL.GDV": "36eb27c8c7d92b337a31ad3a47d966c1",
		"RAPHAEL.GDV": "f60e4a1205422551104c33ce8dc1584e",
		"RAPHAELH.GDV": "895de4a88615032df8a4d6fd4f64d58e",
		"RAPHAELI.GDV": "3243fbf24dd7e6b43021ded29205f61d",
		"RAPHAELJ.GDV": "56692f4ee3eb6a91a91023eb09df1e1f",
		"RAPHAELK.GDV": "c3a15e462191fd5e5da47b4887fb01c8",
		"RAPHAELL.GDV": "1c229b3eb53bec0d93794392447f5448",
		"RAPHAELM.GDV": "1f9b5bcbe531cfc4e95a62c3ad57c959",
		"RAPHAELN.GDV": "25cdce177069f34d9873d34d76aa935e",
		"RAPHAELO.GDV": "484495dfb00329c290e9919f268294a4",
		"RAQUIA1.RAW": "8a990b37e82d828325cda02e1490b979",
		"RAQUIA2.RAW": "5ddb7d371939be302e0a34a868999093",
		"RAQUIA3.RAW": "818341506472bb50a6ae8d7f1e586092",
		"RAQUIA4.RAW": "84e8913763e0be3f5122b3708e006c95",
		"RAQUIA5.RAW": "35392803892e846d34e1bbdfabbb79ab",
		"RATPFA.GDV": "6de23a1b5020c1ea1d547cdfc2655a04",
		"RATPFB.GDV": "18f54ddb89f75ba50ef678ebc9bc5b70",
		"RAYSEL1A.GDV": "e113f6300d8e894c9f8f92edb63cd859",
		"RAYSEL1B.GDV": "aefce406ffcf949646152b5ac456937b",
		"RAYSEL1C.GDV": "5ee2db02a81df7b7b44a286b3cf55bec",
		"RAYSEL1D.GDV": "2bb5167f2be5af368aacc242eefe5fd6",
		"RAYSEL1E.GDV": "783a7a4451a4f50c826c7af005db8a23",
		"RAYSEL1F.GDV": "3530002ebb8484dc185ac4d8b426cac9",
		"RAYSEL1G.GDV": "3a43b5cc46bdc83c44a475e1f91a383d",
		"README.TXT": "1ea0f5d589f17ec745263210e025e392",
		"REALMS.BAT": "0c75f2e5343819642202c4edd5ddc564",
		"REBECCA.GDV": "76484b5fe19ab1da413a55f74ca445be",
		"REFLECTA.GDV": "9ef60acbd827f12b436e1efdfbc67cb2",
		"REFLECTB.GDV": "42d524b5457453761e14f1b8149213eb",
		"REFLECTC.GDV": "ea93b661e7ee9977db1dc9aae27abc2e",
		"RITUAL.GDV": "e990346cc998cdf34b934e30febd0df6",
		"ROTH.EXE": "c11ab446c6d92e4e89d557864aa62997",
		"ROTH.INI": "fc87215fb74f086fba326f835ef4a6ea",
		"ROTH.RES": "e25891a9db4548467774814e409b1250",
		"SALVAT.RAW": "53b13eeb5ab7fec93515dd054e62aa08",
		"SHRIVEA.GDV": "3a0324ea55a2ddbc8a1507b78d1ac954",
		"SHRIVEB.GDV": "37dc7201f025ef3f1a698a2b5b050b18",
		"SHRIVEC.GDV": "695fa3fc7064d52219c38b152274624f",
		"SHRIVED.GDV": "5fc476554af09e8c29f0708333954456",
		"SOULST2.RAW": "f0cad71c8afc062d53608ef11cb6d909",
		"SOULST3.RAW": "a8e7a235bc9abcbd77b48b53edda6d65",
		"STUDY1.RAW": "cd12f39458573d24028828a525fa6668",
		"STUDY2.RAW": "08ff34b8a5fd3960e87348249baee7c5",
		"STUDY3.RAW": "470b41d230e77e43af5f61b83586fe1a",
		"STUDY4.RAW": "5d35a447593aca6456bbf7661db072bf",
		"TALEWELA.GDV": "73378fd8a01d791212c00d24da4ab8b8",
		"TALEWELB.GDV": "2591cccdaf41fd947545b556af2e8fe0",
		"TALEWELC.GDV": "6197a1c646e9d3acaa377be32fe65d43",
		"TEMPLE1.RAW": "b3ce9b844300a9770bb1b81f9fc28b9b",
		"TGATE1F.RAW": "01549e438c65383c050ebc93e1ad1026",
		"TGATE1G.RAW": "e8cbc7cf96aa73869444176cde2ec31e",
		"TGATE1H.RAW": "23e6938f1d51a510c60fd72f961cbac9",
		"TGATE1I.RAW": "7e590a67b84c2f0dee9dcd918f7a51e5",
		"TOWER1.RAW": "b17c350b5f5b92fc13dc14dcaef6410a",
		"TUNNWARP.GDV": "28ac2c20475d62c3c317d60feb01b426",
		"VERSE.GDV": "28ac2c20475d62c3c317d60feb01b426",
		"VICAR.RAW": "36a8b0cb42023a7016252b4a41df0000",
		"VICAR1.RAW": "7fd18c3fe1800bc607fe5a1cf30bb024",
		"WALKWAY1.GDV": "ceb14c0296c8cc225de269087129f188",
		"WALKWAY2.GDV": "16a4219fe6c82d8fa74ef86febe5542a",
		"WALKWAY3.GDV": "2bc5b4ef929abb5007799d4d1df09f7a",
		"WALKWY1A.GDV": "deacb97ae36c716d86837ca6063a2c0a",
		"WIN95RUN.PIF": "d863b3e75aec130d6957aa714c70298e",
	},
	Region.GERMANY: {
		"ABADON1A.GDV": "7a106e1da4277f51e2e0f61f3094913c",
		"ABADON1B.GDV": "66babbf470d6f0908987554664478198",
		"ABADON1C.GDV": "5d92a2cbbece19522ce7461aa69b8443",
		"ABADON1D.GDV": "fca5aa3d07ad72dd0026ca5104aec39b",
		"ABADON1E.GDV": "1aaa8e200befcd272981b5ba58e47096",
		"ABADON1F.GDV": "753ffbddce98e22445df04962f0b2a9f",
		"ABADON1G.GDV": "932ce459d16f29bdedd6f944c0dc989c",
		"ABADON1H.GDV": "2c4b42bf128da2a2c4ccf5b5932b5db4",
		"ABADON2A.GDV": "c83c76633f7acb2ded56b4f97c0f6349",
		"ABADON2B.GDV": "8d935b0949c7e2d61e8cd1ff93275859",
		"ABADON2C.GDV": "0ccc552c9c4be9da88ca71c013f897d7",
		"ABADON2D.GDV": "cf9561938a071ed4eea6a7cd893a73c4",
		"ABADON2E.GDV": "98d3ab0388bd6fa456ee9be8eafb8cb6",
		"ABADON2F.GDV": "f9019447cc8b0934251b66f5a49be9c8",
		"ABADON2G.GDV": "94ab0181c5eb21df6561eca79d70b3c9",
		"ABADON2H.GDV": "e4edfe239e09aabb93ba08e970c3843a",
		"ABADON2I.GDV": "103ced96833336a773d4aa299f6dad7e",
		"ABAGATE2.RAW": "0b8ceef19cd1243c866da3dcd8884f79",
		"ADEMO.DAS": "724ce6eb75a1b9e1a3eecceace333de0",
		"AELF.RAW": "1951e11021028b183e96e0ceabd29e94",
		"ALEF01.GDV": "586b02fcfef1b3993c1484052b091573",
		"ALEF02.GDV": "a806f5bbf7e2e132911069f1861d45a3",
		"ALEF03A.GDV": "d956e93e187fb0c5e420c903027d02ff",
		"ALEF03B.GDV": "c3bc1b22f18f77a32ce51f5a740d08bc",
		"ALEF03C.GDV": "f31413c43a71cc518f949a0d6d69e535",
		"ALEF03D.GDV": "484654f30c22db1337fc5598a76b3de3",
		"ALEF04.GDV": "8a6452c0465f8311a68d045310016f23",
		"ANUBIS.RAW": "cb4acdf0fc7bdd0f38ac31961e33747a",
		"AQUA1.RAW": "3d3580ca0c2ad5fce5472e5bd8c3ecd3",
		"AQUA2.RAW": "813d6ac1dad118a676494b7a82a3853f",
		"BACKDROP.RAW": "2c9debcf210c1bfa089c5dbfae7d2f40",
		"BEAST.GDV": "1451a43e693e59007ac7f2bd03ca3e06",
		"BELEXTRA.GDV": "8a046f2013373be648e53138e72286ad",
		"BELIAL1A.GDV": "5ffc10f42f2abd7584c1a700176f51e5",
		"BELIAL1B.GDV": "7970b00e325e5a5a100fa689e21bb936",
		"BELIAL1C.GDV": "efbd4878346d498f8d59c015b5987442",
		"BELIAL1E.GDV": "18c992422b6142d5954eb85806f62cc4",
		"BELIAL1F.GDV": "d6cd9cee2d112c96b8875a4a6645ab3a",
		"BELSB1.GDV": "d32d8f73d030b72a027f0681af82114d",
		"BELSB2.GDV": "5f568d947785c1ca2b37d92c5267ce98",
		"BELSB3.GDV": "d03fa0d93d89b7de01459d641c736f3f",
		"BELSB4.GDV": "feaf6518fa988c2e47607d8d88fe456f",
		"BELSB5.GDV": "71e9d364af4ef8c0c7dfddb1fcf2715b",
		"BELSB6.GDV": "566d53a139b7ad904d71f6f8535c0610",
		"BENDGMA.GDV": "a3f224130d8c919f877ff60c6d0a270f",
		"BENDGMB.GDV": "11e7dc8e3b9e59381aa04f38eed85303",
		"BETRAYAL.GDV": "6a877dea648985530651a902766f1ce4",
		"BRANDED.GDV": "a9004e3528ad00db36f51dc9f1570660",
		"CAVERNS.RAW": "259313cba21820f7290fd717cb909375",
		"CAVERNS2.RAW": "47fe249059d99efeec1b1bcba9e4e22e",
		"CAVERNS3.RAW": "f81ccfa715126a582218baa77f0ff375",
		"CHPTR1.GDV": "27666f25c17c2961be3f5772ce29d593",
		"CHPTR10.GDV": "61fb69eb9258ac2e4a473bd72eedc766",
		"CHPTR11.GDV": "fbe0570568e4c8f8424240d5537b0200",
		"CHPTR12.GDV": "3e15297847e55e88eb04458beca01443",
		"CHPTR13.GDV": "4639f84b7c267a444cab9886e4e0d01b",
		"CHPTR14.GDV": "1ab7c3b302ec7a8f014c2a7624146807",
		"CHPTR15.GDV": "3542e02cdb452153d18fb0ced889c5ba",
		"CHPTR16.GDV": "e23f125478a936ab13eb11115a0aacdd",
		"CHPTR17.GDV": "72ca22a5a219173daf0a871d97b7ccf9",
		"CHPTR18.GDV": "4c451b69f4d799c5822f6bf5e62fe49e",
		"CHPTR19.GDV": "514cdd77562acc10dd6eed396c0f1e18",
		"CHPTR2.GDV": "d2243b7ab75ba46eb19882f3b67ac7f5",
		"CHPTR20.GDV": "6da95077cca43c2dd9ee4f5d874c3b41",
		"CHPTR3.GDV": "8b7201c8c01623febdcc361d88cdb64d",
		"CHPTR4.GDV": "37fdfe428ebcb15167fce31792e45298",
		"CHPTR5.GDV": "d1af6f8cb720efcee2644a9d7763f058",
		"CHPTR6.GDV": "38dcac326fafe5d12a5889f7b5dbccf7",
		"CHPTR7.GDV": "277d562dc4bc59afcb6d19f2f05b1e7b",
		"CHPTR8.GDV": "261c5955dc827c58435a3ff73ba93314",
		"CHPTR9.GDV": "61c0c589c358bf62c455d84370b76a54",
		"CHURCH1.RAW": "1932dddc70959c14767129f38042a15c",
		"CONFIG.INI": "344c282a159a9f8e18961d4dcf23d976",
		"DBASE100.DAT": "d4599700ab4df3fdb9c649da41accda8",
		"DBASE200.DAT": "636d371e190621f4a34d2a72d5f2e8cd",
		"DBASE300.DAT": "5f525c248bacf67b0e92bc9450103942",
		"DBASE400.DAT": "023563037fb18704ea248c94b3265e24",
		"DBASE500.DAT": "5097d309b2f2e2061f68d0e1f95aa37f",
		"DEMO.DAS": "19b82c32cebb7004b457d8a30e2d2afa",
		"DEMO1.DAS": "5a798d423b3714c24de05ead57ac5f12",
		"DEMO2.DAS": "5ce067e2ba96be35fcde3c3ad729b169",
		"DEMO3.DAS": "e083da270769a0269091a527b6931fc6",
		"DEMO4.DAS": "a97e844f22d1a18cc93974e11fa50fdb",
		"DODGERA.GDV": "d1f381ccadf03cb2401c39360ef8fb50",
		"DOMINION.RAW": "746edcd7fd431131331462c59eebc4b1",
		"DOPPLE.GDV": "5715492f2ef11c1475dbea3967bcaae2",
		"DOPPLE.RAW": "b51ed2ab7b4070f69c58bee5341b8994",
		"DOS4GW.EXE": "006211ca60fa3d4b38a9a8da2ade1056",
		"DRUM.BNK": "7ecc2b70d2eb0d93475aca120b21e46b",
		"ECREDITS.GDV": "164d5a08651d6e3221ec769f3ae2d5bd",
		"EFFIGYA1.GDV": "c5234f7a432786965c0227c2e7474f38",
		"EFFIGYA2.GDV": "32ba174f55fc78ff47a63730dfa35827",
		"EFFIGYA3.GDV": "5b0467ffa904408356b274fe4df6a095",
		"EFFIGYB.GDV": "b39008a71967bcf36f200285beca60e0",
		"EFFIGYC.GDV": "dcdddb4814d6cccc3a956440643bbc8b",
		"ELOHIM1.RAW": "fdc5cfadf9e4cb8abd40b53a59c99acb",
		"ENCHANT.GDV": "acd788633f457e6b96ad335d3e40a21a",
		"EPILOGUE.GDV": "b0ff78c4f3c0074eb2bb5bc843c1c79c",
		"FALSHIRE.GDV": "3abbc788100456d6356ed8d18301d942",
		"FILELIST.TXT": "34d5b9d86e6d53264a551f3b0b93047b",
		"FLOTRAP1.GDV": "2abd621421c387ea1360b0f4c4660f49",
		"FLOTRAP2.GDV": "13e2e198a5fe0c0fdb39e6e87cc89b26",
		"FLOTRAP3.GDV": "c2c912f17744ae53ba488aa21c8e1d9e",
		"FLOTRAP4.GDV": "2b5d93b6b9fd04b5bbe4de52dd145c5f",
		"FLOTRAP5.GDV": "bf36705df2e8970f93975a18f488e6ed",
		"FLOTRAP6.GDV": "3eadab51e2c160c96c9b74c914d6458a",
		"FULLKEEP.GDV": "eb92ad1e3591b44a409b9ff3829ccf7e",
		"FX22.SFX": "3999135d197890ce4aad9895112ebc91",
		"FXSCRIPT.SFX": "6391d983dc94a976f6d20bc5c421043b",
		"GARDENS.GDV": "5eea544e2d41c741515bf5b99a5cf425",
		"GATE01A.GDV": "88b9849fd857a3d238fb61e2a58b5fff",
		"GATE01B.GDV": "a6d0ed6b148800c2ef10208e63fdb4b0",
		"GAUL01.GDV": "99c8b58fa8dd21fa035bfb73ab5a01fd",
		"GAUL02.GDV": "3848668edb5f0d9639d6edc4b572dd9a",
		"GLASSWIN.GDV": "468073b3a60f75c67924f46a8e4504d4",
		"GNARL1.RAW": "c2fa03b29464080244b5256bef2bf5b5",
		"GNARLA.GDV": "3b959dc8d66760cb7d398093e78a2b7f",
		"GNARLB.GDV": "de002ff38a776b4fd016fa99162d2e32",
		"GNARLC.GDV": "417f23a9ba8e18198f7dfb68c6b722d1",
		"GNARLD.GDV": "8f70647bdcab29ef3c7362aa06444db1",
		"GNARLE.GDV": "9f0af3a508bf1f7906da3f7d225eb71c",
		"GNARLF.GDV": "ccd5cc6680808014e87507cb7eba7752",
		"GNARLG.GDV": "15e84f54a4d4edea7e491de1342fe105",
		"GNARLH.GDV": "906deb13512ba6fc00f5f544d542ff43",
		"GNARLI.GDV": "315ccd97bc69364edd17c344b3776dd2",
		"GRAVE.RAW": "f7eccaba245cd3a202b6189993e65f94",
		"GRAVIS.INI": "e6f30ac9fe0d9f8d0a0f9bdb68ba0134",
		"GREMLOGO.GDV": "15ccc918aaedb72e5a4cd7f1efdf8ca5",
		"HAWK01.GDV": "27fb560be90f25e90658c6af7983c88d",
		"HAWK02A.GDV": "6bb0c0d33d04cd73e7819294a0497d24",
		"HAWK02B.GDV": "e260fba8480cfba14eefe7133f918267",
		"HAWK02C.GDV": "9390ec37003fbf557646ce81a7644d8a",
		"HAWK02D.GDV": "91baef3b0c7b50fb8f3143ccdb1af525",
		"HAWK03A.GDV": "ab2dca3a368a198e990cf73642ef06bc",
		"HAWK03B.GDV": "d6944c832e92ba3fa9531146d1f0f86a",
		"HAWK03C.GDV": "0ce8df37c6c70adf00a4135b1f911e1d",
		"HAWK03D.GDV": "e2f7168ab4da8582f23ce8c6f87e0d8c",
		"HAWK03E.GDV": "1a60840dd489ae2662c659e1744d049a",
		"HAWK03F.GDV": "63e88c5eb187fb89c411da41e7b5732f",
		"HAWK03G.GDV": "a3b37e7fe8f7adba234708f825181e74",
		"HAWK03H.GDV": "de67a371537d9fa9767b138166dfc9f6",
		"HAWK03I.GDV": "31a945c8e6a9c7372fec35c0141029c7",
		"HAWK03J.GDV": "af514e0574ebd4dfa02c2d0c5793bd02",
		"HAWK03K.GDV": "4afe3e3d438311ab18d1a3ac5f317d63",
		"HAWK03L.GDV": "e892c728a5bd30837e8513a5a91f5030",
		"HAWK03M.GDV": "549353ac73a61be7921f0d5d27219bf0",
		"HAWK03N.GDV": "d1f6e924bf0917c19e9dafc03a456338",
		"HAWK03O.GDV": "4f5fd99c8d63107356028fae27486acd",
		"HAWK03P.GDV": "270f7fd3aafce22112c88d6fc15f5d93",
		"HAWK04.GDV": "5a28742941d68e5521e13b5cfc7a1173",
		"HMIDET.386": "59b3f813848cda336763648d628db917",
		"HMIDET.DRV": "f4e4ed8302f18aac0a8f8bb153a61566",
		"HMIDRV.386": "d730de4f9e6e4d8b95738f7cdde7c140",
		"HMIDRV.DRV": "cef61ca0b324c59d8e38c59ba1ddb191",
		"HMIMDRV.386": "ddfac2fec57dfd841ccfb64adee3b35e",
		"HMIMDRV.DRV": "59ad9c334c62a258b576728ef69d8af5",
		"ICONS.ALL": "db3f71fc7d13171768ff203d20777286",
		"INSTALL.EXE": "19af77f62aa8f966206595aad3df479a",
		"INSTALL.INI": "107973d894e4a51ddf8e2a6c1471a804",
		"INSTALL.PIF": "59e3fd81050871b4dc9605de2c90ec72",
		"INSTGRAV.INI": "8bf7b8cf7ba33e1353d2de82e142b41c",
		"INSTRA.GDV": "81383b5495ee27ca4af5426a00c3fb43",
		"INSTRB.GDV": "c65e85470bc601c19ec386a614fa59ef",
		"INSTRC.GDV": "138ae3f45fe3f2fd30ea541b8547fa5f",
		"INSTRD.GDV": "fbd9a385ca52a7898c762a8801511d29",
		"INSTRE.GDV": "03806fe4f11b7a2c47ec8371ee022b3c",
		"INSTRF.GDV": "fb7686ab9216f5bff7038a820fc963c9",
		"INSTRG.GDV": "eabae52ed9b256e11c54f42854351f8b",
		"INSTRH.GDV": "63e362ef0ed714e3c9a7037a4939eea3",
		"INSTRI.GDV": "13a13e3b1a99a6c682abb2d888bde287",
		"INTRO.GDV": "c9a9da2f333f1dcaabaded372986397e",
		"LOADPATS.EXE": "b24134b3fab09b2727b49a0fe1d64d8e",
		"LRINTH.RAW": "d26817cc747828d10b7566387492c491",
		"LRINTH1.RAW": "a886ad8a3c6f98aeeb26f9d8b40d1275",
		"MAS3.RAW": "22db0686b64aee9f84261ac423922c56",
		"MAS4.RAW": "bd189628c95b2a854213105adeb6c3d4",
		"MAS6.RAW": "1e126f561296eeaa88df5490455558c1",
		"MAS7.RAW": "d0d5ffce0acab775fde690d3d6c9485a",
		"MAUSO1EA.RAW": "f2e0564a8177730dcb135588da40a04d",
		"MAUSO1EB.RAW": "7d03e78b3c4a849b98d2a6146bdd2d6d",
		"MAZE.RAW": "032fb3a2e1eb0a8f4e0f4463aafedd7b",
		"MELODIC.BNK": "2adb649534bd5cd78900d416149c9642",
		"MOVIE.GDV": "4bc7be9c6d275ad853b2c134f633bfa1",
		"MT32MAP.MTX": "23aa641a6b7a5855eecfb6fb48be7dc1",
		"OPTEMP1.RAW": "d3d1c4220bcbeb47306dff30f91c84b2",
		"RANDALL.GDV": "2f61c1b288e74cf0bc457e53f2e9de63",
		"RAPHAEL.GDV": "8e79db450844334d7e74684ae06a61c6",
		"RAPHAELH.GDV": "e7d9b9f9fffb0ac1d143ce0d9da2e8b7",
		"RAPHAELI.GDV": "6a6d7e2b94c106145a4db365bc7fc5fa",
		"RAPHAELJ.GDV": "a007a26f61c13cd693f01cfce3b46a87",
		"RAPHAELK.GDV": "29ff4b77c76bce0342859b37418a4a65",
		"RAPHAELL.GDV": "a318116e93ea15c1feaa0463c2208305",
		"RAPHAELM.GDV": "e17fbc0db6d8c560efad3b32f0e30cc5",
		"RAPHAELN.GDV": "3feabdeeba1341b72b9b9e225ca0fece",
		"RAPHAELO.GDV": "7c552ac19290e9ceba802bf4042316b7",
		"RAQUIA1.RAW": "8a990b37e82d828325cda02e1490b979",
		"RAQUIA2.RAW": "5ddb7d371939be302e0a34a868999093",
		"RAQUIA3.RAW": "ec482df7eb3f9acbb7a5a24b852b7c00",
		"RAQUIA4.RAW": "d48f9411bbb6d2602e4bc427b4931719",
		"RAQUIA5.RAW": "73f904455a3af45de6c2e7979dd03c9c",
		"RATPFA.GDV": "2cdb3e7714af3377b89f0c5966b9f32f",
		"RATPFB.GDV": "10f4fb50b50c04bcf397d16d94739d02",
		"RAYSEL1A.GDV": "65d676e45b15189baf83fae8c3a2209f",
		"RAYSEL1B.GDV": "343d4fd20f2d066fafdba53e3ed331cc",
		"RAYSEL1C.GDV": "72ce52e456a8741435556cb0496f943b",
		"RAYSEL1D.GDV": "641b5d1b8a9feafb820fb9f3b4ad2b47",
		"RAYSEL1E.GDV": "35f2ac242e369967860f3041fbf1984c",
		"RAYSEL1F.GDV": "5d46cd05630bf107f8a355a1ca2517f2",
		"RAYSEL1G.GDV": "e6ee653203b8c0e0ac4d2e4084879773",
		"README.TXT": "fd3979535329202d9ab10e18fa78c71f",
		"REALMS.BAT": "0c75f2e5343819642202c4edd5ddc564",
		"REBECCA.GDV": "f1936c4f96e447e4f881bb9a23e2aa17",
		"REFLECTA.GDV": "bb04893f1aee13870346c1353912294c",
		"REFLECTB.GDV": "42d524b5457453761e14f1b8149213eb",
		"REFLECTC.GDV": "516ea4e7d95cf47d89d814c80323a775",
		"RITUAL.GDV": "856135842bb5ab24864971dfb2d6ce82",
		"ROTH.EXE": "c11ab446c6d92e4e89d557864aa62997",
		"ROTH.INI": "fc87215fb74f086fba326f835ef4a6ea",
		"ROTH.RES": "58d0c8cce3b6c2b768edeb3cd8c0ccd6",
		"SALVAT.RAW": "9a2fd2e219b7897d2b08194b9b170c33",
		"SHRIVEA.GDV": "04474e108e67f9b7fe26dcf4e9e1ba3c",
		"SHRIVEB.GDV": "3eb460f97c40906e08b31737db35169b",
		"SHRIVEC.GDV": "373195f7d8c568f04d2e5c18c5410ca9",
		"SHRIVED.GDV": "29c984ca22f9fb08eb1faa5450a7ec69",
		"SOULST2.RAW": "5da1523903a0647f07b3522e47d1cc53",
		"SOULST3.RAW": "49ef092c5fe47327f54136da27c681e1",
		"STUDY1.RAW": "1e4ae31eef4a7d9b1fa268e9c0499157",
		"STUDY2.RAW": "0fbf64cef04e5bfd8e2fd0610ded4b0b",
		"STUDY3.RAW": "df6745247395fb067b01c59f8c8b887c",
		"STUDY4.RAW": "21e8a152ae59afc791c8edc91afc9edb",
		"TALEWELA.GDV": "77613c13afdbaf3e51e92ba897b4a5d3",
		"TALEWELB.GDV": "862b85f327834b6d2b490d257229e8ac",
		"TALEWELC.GDV": "9b833afe78f5308786940deabc7d7fff",
		"TEMPLE1.RAW": "3db310591029edd93a745168ecabb77e",
		"TGATE1F.RAW": "01549e438c65383c050ebc93e1ad1026",
		"TGATE1G.RAW": "c98b11832900dc61ae9fde46aef1488d",
		"TGATE1H.RAW": "01d8f012afd8f25d8a10a6352d316272",
		"TGATE1I.RAW": "7e590a67b84c2f0dee9dcd918f7a51e5",
		"TOWER1.RAW": "d6efe0ea2e4f5c2d47ee8ecb360f0776",
		"TUNNWARP.GDV": "28ac2c20475d62c3c317d60feb01b426",
		"VERSE.GDV": "28ac2c20475d62c3c317d60feb01b426",
		"VICAR.RAW": "cf623746c15d5af413931c123f9621f2",
		"VICAR1.RAW": "9b676ff1eea0293958291c0812fac4ef",
		"WALKWAY1.GDV": "405cffdcbf693e301af57d48206adb4c",
		"WALKWAY2.GDV": "3eeae40ccf34e8afd8120f8ac269f3bf",
		"WALKWAY3.GDV": "2bc5b4ef929abb5007799d4d1df09f7a",
		"WALKWY1A.GDV": "00bfbd5caf042f1b824257e4081f03d4",
		"WIN95RUN.PIF": "d863b3e75aec130d6957aa714c70298e",
	},
	Region.ITALY: {
		"ABADON1A.GDV": "47282b342dbeb7337a12fedc12af9b4e",
		"ABADON1B.GDV": "1f56dfdd5a4f3b2fcd84e724b112661e",
		"ABADON1C.GDV": "00daa70074e1b9a060c147f21e1abe0f",
		"ABADON1D.GDV": "67b5a63d0314eedff5778e2cb49eee40",
		"ABADON1E.GDV": "ed5252d0cb1ee022e24cb6ced2ebf7bc",
		"ABADON1F.GDV": "245eee7f04687d2534e741b1f7c0ebf4",
		"ABADON1G.GDV": "972ce0fdd63dbb7c73f7fc3b8961e7e8",
		"ABADON1H.GDV": "7598f7e3968489e0f3303ae77a529b9b",
		"ABADON2A.GDV": "7a857261205a5b81eae2c2a3ab02c7b0",
		"ABADON2B.GDV": "c3800b6241101b2f9f4e2a314689ca89",
		"ABADON2C.GDV": "d13838a9600a92e274ec565126741f54",
		"ABADON2D.GDV": "62910f2fb33dfd6f5a46e0455d7251ba",
		"ABADON2E.GDV": "840ce60463df733a7de431b500427d5b",
		"ABADON2F.GDV": "bd45bfef79e2b26df69f30f4fa5fe161",
		"ABADON2G.GDV": "d81ad51cb4461b7f53f3ac5ec06d2877",
		"ABADON2H.GDV": "69a1feaec6d68116a2a39e15c068ad8f",
		"ABADON2I.GDV": "9972c611f1237f5bdd0872c3d8e9b263",
		"ABAGATE2.RAW": "36a05ba9452fd5a14249a06cf78a304a",
		"ADEMO.DAS": "724ce6eb75a1b9e1a3eecceace333de0",
		"AELF.RAW": "615223ddd754fc89641c2c50a6fa5f84",
		"ALEF01.GDV": "233453c34c8c7cb2d8dd16ce714f620d",
		"ALEF02.GDV": "f9a232d2f7f83574453705083a4f4aa9",
		"ALEF03A.GDV": "da6b9b09ee587bf79348223d925fb238",
		"ALEF03B.GDV": "74209523d1f7e0ca463d04959903d9d3",
		"ALEF03C.GDV": "d7010a6cfa948525f2232c6bc5bd7910",
		"ALEF03D.GDV": "3842619d785c5b063e0a1b57bb4e671a",
		"ALEF04.GDV": "8ce407bbb90d1195a0c91017580e2de0",
		"ANUBIS.RAW": "f3d0726ca2f54c7592f0db18f44b8fdb",
		"AQUA1.RAW": "359b9a370554826a7482422487e5155c",
		"AQUA2.RAW": "813d6ac1dad118a676494b7a82a3853f",
		"BACKDROP.RAW": "2c9debcf210c1bfa089c5dbfae7d2f40",
		"BEAST.GDV": "602e469d3802099006c93f1e846af306",
		"BELEXTRA.GDV": "2ecf6de0e0fbf2eeeb0f50620b428c51",
		"BELIAL1A.GDV": "3d52eb12a8d8e9d41964ee08b0160d55",
		"BELIAL1B.GDV": "f1dd52a30b0a1e3227003e57dd70b7c6",
		"BELIAL1C.GDV": "8d42429d66cde0ea08e03a2e8b386b9e",
		"BELIAL1E.GDV": "1ee72551a0209ab106421798472dbb58",
		"BELIAL1F.GDV": "5cfcb3fbae055093674f91e9abaec26a",
		"BELSB1.GDV": "1814256f939be3ad2d4ac743ec0f4ea1",
		"BELSB2.GDV": "fd15d53a3afe66791fba35579551e9d5",
		"BELSB3.GDV": "862555c4338e4a1a46f846d5233affba",
		"BELSB4.GDV": "9cdea3b23deb16f26941a99e8e58bef9",
		"BELSB5.GDV": "37d83a8572bf24e86c268ad3312c3d4e",
		"BELSB6.GDV": "55793c7161698ffcaaded291e62de92c",
		"BENDGMA.GDV": "a3f224130d8c919f877ff60c6d0a270f",
		"BENDGMB.GDV": "4a1f66368d398094ec3dd0f6fa8c237d",
		"BETRAYAL.GDV": "565a6e5024fac4994df0de1163a1b72b",
		"BRANDED.GDV": "a9004e3528ad00db36f51dc9f1570660",
		"CAVERNS.RAW": "f8c281067b0cbb4377cd88ed1468765f",
		"CAVERNS2.RAW": "3305d61219ef7697d77e04fe3c250c72",
		"CAVERNS3.RAW": "8e3ea4e4789f1521cfb7db1dd2266158",
		"CHPTR1.GDV": "5bbd26033aa120146c54aa230ef184f3",
		"CHPTR10.GDV": "eb8682f9d31edba230b43862631d70f0",
		"CHPTR11.GDV": "77f54829b6af25c0f406198c6e5b5bb0",
		"CHPTR12.GDV": "461cf6a2216c5e72ad1bee41c7f765dc",
		"CHPTR13.GDV": "e55ab4624affce74bb92813d2477baab",
		"CHPTR14.GDV": "00373844c986fb19a65f339f17d4e9e2",
		"CHPTR15.GDV": "e346d9ddf51e9056cd1a500a638d2b41",
		"CHPTR16.GDV": "9f7e6ce5217d20ebbc02083a7894326f",
		"CHPTR17.GDV": "123543dde492cd4810596077f205af9b",
		"CHPTR18.GDV": "4cd7a1be16e0d7d4b5c0a8c6321ea104",
		"CHPTR19.GDV": "7ac24b1c05a79b0151020611db861d25",
		"CHPTR2.GDV": "d9c47caf5793a12a50297428b0d8113e",
		"CHPTR20.GDV": "062d986ba38ddf87b43b96d2f7cac34e",
		"CHPTR3.GDV": "1c04218c7d201e802aad48250eeeb577",
		"CHPTR4.GDV": "9d54acea782db4e73d3c2f38c247f126",
		"CHPTR5.GDV": "6d2e31d310ca53ad7a90ad07822f74e9",
		"CHPTR6.GDV": "16bb818c678de040401f288360aff51c",
		"CHPTR7.GDV": "27047cc3e730638dfd68442b2b0ba9db",
		"CHPTR8.GDV": "e24563145d261e53e862d847bacbb002",
		"CHPTR9.GDV": "466e1cfb92b762d0b685e85b4dcf3bbb",
		"CHURCH1.RAW": "6b9c0a0bc540feebd2b66a30a30c0d3d",
		"CONFIG.INI": "c80c58cb5f91ad76e6731961f2908e9a",
		"DBASE100.DAT": "429f0c346109f45f6ba90de98c958a84",
		"DBASE200.DAT": "091d3e26b7b29c404d23634e2ed95d00",
		"DBASE300.DAT": "c0f2491959b932c3f9179ef2edb9133e",
		"DBASE400.DAT": "0003a092689f3af8dd41ed1d6ef2f7e7",
		"DBASE500.DAT": "7917046758d91a4e33ca4d0a3c2e77f2",
		"DEMO.DAS": "19b82c32cebb7004b457d8a30e2d2afa",
		"DEMO1.DAS": "17272baf6c5c8a25dbb56f390a46192d",
		"DEMO2.DAS": "1829a6e04d3eb32bd12a7804816efbdb",
		"DEMO3.DAS": "e083da270769a0269091a527b6931fc6",
		"DEMO4.DAS": "bf36e0e63d439a5c31815743b9391197",
		"DODGERA.GDV": "b03fc6c3c95cdbfe596ace49774dc402",
		"DOMINION.RAW": "44896acbc3f57ff4d546754bc60d1514",
		"DOPPLE.GDV": "96ece8e5a4c42f12add504e19eb7a541",
		"DOPPLE.RAW": "6ef4d4f64212f65904b1d065270f9393",
		"DOS4GW.EXE": "006211ca60fa3d4b38a9a8da2ade1056",
		"DRUM.BNK": "7ecc2b70d2eb0d93475aca120b21e46b",
		"ECREDITS.GDV": "6c9d862f2a40c0a0600262f62be08362",
		"EFFIGYA1.GDV": "ec255c8e1dbef7a663f249df7cbf27c4",
		"EFFIGYA2.GDV": "b148f6e8d64d06e32fc7a077d4909a67",
		"EFFIGYA3.GDV": "1ceeb1f625448d5be5bed50023bc1c8a",
		"EFFIGYB.GDV": "170c0c470dcc65362db73b90a0b2c97b",
		"EFFIGYC.GDV": "4a900281ee9c5177b4c73fbabe334ce8",
		"ELOHIM1.RAW": "608a9d479938d1bfa0807f94439b1954",
		"ENCHANT.GDV": "4315d8e24ff000f312ea7c0d942f7d03",
		"EPILOGUE.GDV": "b7ede44ddbfdb03926ebc32a6e5014c0",
		"FALSHIRE.GDV": "9c393b2c2091d703d44d879d70082d4d",
		"FILELIST.TXT": "0095e72e781b14c6a9b3e457fa550bfa",
		"FLOTRAP1.GDV": "8f1e9e4af323580588b5b7a97229304f",
		"FLOTRAP2.GDV": "961be84fec8b3d2668961e43dd846f92",
		"FLOTRAP3.GDV": "c18163758c050f204385a80c7bcb0c53",
		"FLOTRAP4.GDV": "979d2f4846f186d48bc524f78f0132e6",
		"FLOTRAP5.GDV": "1c5ea9ea5529ea2c4f0a3188821e2dab",
		"FLOTRAP6.GDV": "d242355e1e8cc586c9c125284992a6de",
		"FULLKEEP.GDV": "eecdde2d2df9949e2ee899d2040e1482",
		"FX22.SFX": "3999135d197890ce4aad9895112ebc91",
		"FXSCRIPT.SFX": "6391d983dc94a976f6d20bc5c421043b",
		"GARDENS.GDV": "10ef604385d34772dabf4f8a2af54ca1",
		"GATE01A.GDV": "c1141f7ff234879f7b35a254684b4395",
		"GATE01B.GDV": "2bcfe8b755b4170c8c300469e697c2aa",
		"GAUL01.GDV": "84954717d724f2b843dbfdcf5610096b",
		"GAUL02.GDV": "9070447d681570064cd710c42ce7ee0b",
		"GLASSWIN.GDV": "320ab773dd2a5feac79d8eed7c9e2622",
		"GNARL1.RAW": "a7a8e89d195151ee4acdf8a85e9fa4c7",
		"GNARLA.GDV": "e2d327d3e7f0ecae41d6f4be81cb473b",
		"GNARLB.GDV": "e26b42880ff12402ae74a5694d868d16",
		"GNARLC.GDV": "50c152a5807d6340429cb7d3db47c969",
		"GNARLD.GDV": "3873cc2420c9053aed461c7cf200c613",
		"GNARLE.GDV": "640a1c6bf71467db7362a11aa766c66a",
		"GNARLF.GDV": "d5a3a160bec6fc9f65d3b9257d00173d",
		"GNARLG.GDV": "3a131cbb42afccd7d873ca9a2e098972",
		"GNARLH.GDV": "369a77e28f70a95dc7f7f939963b34d5",
		"GNARLI.GDV": "315ccd97bc69364edd17c344b3776dd2",
		"GRAVE.RAW": "2a943d9ede5841111a79a621c471cbca",
		"GRAVIS.INI": "e6f30ac9fe0d9f8d0a0f9bdb68ba0134",
		"GREMLOGO.GDV": "15ccc918aaedb72e5a4cd7f1efdf8ca5",
		"HAWK01.GDV": "92d51b9d2b2213ebc79f75bbaa4a6df9",
		"HAWK02A.GDV": "414d18f433ff64b0f34aba02294c9b7b",
		"HAWK02B.GDV": "394fcb1361f20c0b62838093ae226fd6",
		"HAWK02C.GDV": "8e9221ba3a692e89a487c810c9ca647c",
		"HAWK02D.GDV": "84793b4d2426d5fe19fae2e4b7f8ecfc",
		"HAWK03A.GDV": "8afc078881d722ef26cdaa9be65d3cb6",
		"HAWK03B.GDV": "3925a83d71c5deac656905cc05fd4b7f",
		"HAWK03C.GDV": "ac28a85789b64903aa25f72b4ce1bf2e",
		"HAWK03D.GDV": "171a70d5feb7497d00333cb4fdae7463",
		"HAWK03E.GDV": "9a60273e4bb8642605ab0a2711ce374b",
		"HAWK03F.GDV": "e48a7b9c7bbf46c366f959f48af27989",
		"HAWK03G.GDV": "244a62dcbbc7c86ef5d66b0a49891163",
		"HAWK03H.GDV": "30fd416327de73629482c0f4d5ea8747",
		"HAWK03I.GDV": "685e750c8265832ca944240b4d5639d3",
		"HAWK03J.GDV": "e02755bcaf03c126c64f0c7e1ca7c322",
		"HAWK03K.GDV": "2696c14f41e009f01e605ad666719e20",
		"HAWK03L.GDV": "3421cc807c745986e52a5a4087fa1b1b",
		"HAWK03M.GDV": "11cd87e37264b16b8e53d9ea3ead700e",
		"HAWK03N.GDV": "611f0cbceb690e2a1b28ab884760cd7b",
		"HAWK03O.GDV": "53bc84d9760ab4b793a2ac2864eac292",
		"HAWK03P.GDV": "be70faaf3accf9166c5f0c946e502b92",
		"HAWK04.GDV": "029801d7e4051c44874434d8ae562994",
		"HMIDET.386": "59b3f813848cda336763648d628db917",
		"HMIDET.DRV": "f4e4ed8302f18aac0a8f8bb153a61566",
		"HMIDRV.386": "d730de4f9e6e4d8b95738f7cdde7c140",
		"HMIDRV.DRV": "cef61ca0b324c59d8e38c59ba1ddb191",
		"HMIMDRV.386": "ddfac2fec57dfd841ccfb64adee3b35e",
		"HMIMDRV.DRV": "59ad9c334c62a258b576728ef69d8af5",
		"ICONS.ALL": "91c4c33339cc0661be9568d79ead8911",
		"INSTALL.EXE": "19af77f62aa8f966206595aad3df479a",
		"INSTALL.INI": "94d5405c5749726eaeb23550be454d51",
		"INSTALL.PIF": "86f4bc6a60a42ab1624f072184f9a8fb",
		"INSTGRAV.INI": "8bf7b8cf7ba33e1353d2de82e142b41c",
		"INSTRA.GDV": "196b68856fc939b3de8b2f2c31fa988b",
		"INSTRB.GDV": "cd083420aa3ac09ee18b138bd529da02",
		"INSTRC.GDV": "a568ecf3f03955168b378ae188280e79",
		"INSTRD.GDV": "d686aae283f02eb315e72ac03987f3b7",
		"INSTRE.GDV": "bef071b5edb7756aab5c261fc1a7df68",
		"INSTRF.GDV": "13cac11831fd158b137d854072a710d9",
		"INSTRG.GDV": "277921312328ad73dda16e9c59c18fbf",
		"INSTRH.GDV": "6c20e5618b0592d0323f22ae3db052a8",
		"INSTRI.GDV": "ee75cb713340aa76b5c17a4954bf50bf",
		"INTRO.GDV": "73b6c7f07e24487a4b4f9f1ddee19fb1",
		"LOADPATS.EXE": "b24134b3fab09b2727b49a0fe1d64d8e",
		"LRINTH.RAW": "c360fa64b41a962ef74715a752e2a601",
		"LRINTH1.RAW": "c33ea660d0195daadaefb9c4ac6d4973",
		"MAS3.RAW": "9433fe3c8650f02c17f660f48ed96105",
		"MAS4.RAW": "495899fadc5e0aed8bf4629f1dd10742",
		"MAS6.RAW": "1e126f561296eeaa88df5490455558c1",
		"MAS7.RAW": "ea15ea4a67b9a7953b5066821953a002",
		"MAUSO1EA.RAW": "a0b76820c14600753823f6bd723c5ff4",
		"MAUSO1EB.RAW": "fa68e3e52147984fad267fad99febd0a",
		"MAZE.RAW": "756dc91868d45eb57c6c67345f414b1d",
		"MELODIC.BNK": "2adb649534bd5cd78900d416149c9642",
		"MOVIE.GDV": "",
		"MT32MAP.MTX": "23aa641a6b7a5855eecfb6fb48be7dc1",
		"OPTEMP1.RAW": "474639af854a2703e63cd812244b6164",
		"RANDALL.GDV": "65d00915033de4ad55bcc9877f59e9bb",
		"RAPHAEL.GDV": "a0772f01d761df598f27476b6f9b7d00",
		"RAPHAELH.GDV": "181732aefaa32ab13aac7e7e2e478c62",
		"RAPHAELI.GDV": "7669fbd573e446e1e1bc624ce826fefa",
		"RAPHAELJ.GDV": "fc64683128aba67e8028709c0f146671",
		"RAPHAELK.GDV": "9641f45d07fdca35eff8447f31423fde",
		"RAPHAELL.GDV": "0a303290ef32eb98f0f703a4f85c03d1",
		"RAPHAELM.GDV": "d197c8b162483f48668c856d2a94a937",
		"RAPHAELN.GDV": "59664870973d29bfb0c12846fa6ec02e",
		"RAPHAELO.GDV": "2b95106b988d0016c75e3042195b1763",
		"RAQUIA1.RAW": "8a990b37e82d828325cda02e1490b979",
		"RAQUIA2.RAW": "5ddb7d371939be302e0a34a868999093",
		"RAQUIA3.RAW": "818341506472bb50a6ae8d7f1e586092",
		"RAQUIA4.RAW": "84e8913763e0be3f5122b3708e006c95",
		"RAQUIA5.RAW": "35392803892e846d34e1bbdfabbb79ab",
		"RATPFA.GDV": "d3a6b98b7646b231cedc5ffed966709e",
		"RATPFB.GDV": "48c8ef243ad78fc4b45c3da2d3742888",
		"RAYSEL1A.GDV": "fbe85823cd25dd429011b9a4d33916fb",
		"RAYSEL1B.GDV": "b0c99fc59bd55055fd2f33c8c4f8318a",
		"RAYSEL1C.GDV": "963a3cd833b97b4122b87bbf0ca75370",
		"RAYSEL1D.GDV": "25d53ee5b0b519fc94ded2a7fa838467",
		"RAYSEL1E.GDV": "3b97286f7cb5ccf5464134a50d4346c0",
		"RAYSEL1F.GDV": "10f7ba9a2275333c51e39b66661b1efe",
		"RAYSEL1G.GDV": "4367dd99baaa0fd8f4afee5c7df1de71",
		"README.TXT": "d275980bcbf13c6fc6f0307cb4491f71",
		"REALMS.BAT": "c12372f29e2a137620bd0d7833904f80",
		"REBECCA.GDV": "cc9fedf63d206c00de2f6534b1bd7470",
		"REFLECTA.GDV": "1238a356592e9332c6ce32bf1e24a73e",
		"REFLECTB.GDV": "42d524b5457453761e14f1b8149213eb",
		"REFLECTC.GDV": "aa39b4ea36f55dbf6cf6a3fc85c26b42",
		"RITUAL.GDV": "2004a14cb66f0e2f463ba54f523f9b78",
		"ROTH.EXE": "d56e7641e8f5d4ec3144bb1c140a7677",
		"ROTH.INI": "fc87215fb74f086fba326f835ef4a6ea",
		"ROTH.RES": "53b8c3bed00c314ea2dc5c6b25b10277",
		"SALVAT.RAW": "53b13eeb5ab7fec93515dd054e62aa08",
		"SHRIVEA.GDV": "9d310c959e4b48ed6a027d7b34093349",
		"SHRIVEB.GDV": "1cd4df142b70e236917cdac555d8af5b",
		"SHRIVEC.GDV": "f76ed3cbfb76ae9518f5d3528e201135",
		"SHRIVED.GDV": "272e24953e839592b7163a4f2ed6a7cc",
		"SOULST2.RAW": "f0cad71c8afc062d53608ef11cb6d909",
		"SOULST3.RAW": "a8e7a235bc9abcbd77b48b53edda6d65",
		"STUDY1.RAW": "bfa4969d04e8d3b7733422e48416b0de",
		"STUDY2.RAW": "08ff34b8a5fd3960e87348249baee7c5",
		"STUDY3.RAW": "470b41d230e77e43af5f61b83586fe1a",
		"STUDY4.RAW": "5d35a447593aca6456bbf7661db072bf",
		"TALEWELA.GDV": "f9948e553aa2a4fb42a7e778dbd6e50b",
		"TALEWELB.GDV": "896b31fd014a3d5b405be430ebb79605",
		"TALEWELC.GDV": "00df287713b027840c40e28a5be50c50",
		"TEMPLE1.RAW": "b3ce9b844300a9770bb1b81f9fc28b9b",
		"TGATE1F.RAW": "156e4258fe3ee25689f74e215a8f1e13",
		"TGATE1G.RAW": "1027ec59614318cae2d0e8b51bcaf54b",
		"TGATE1H.RAW": "23e6938f1d51a510c60fd72f961cbac9",
		"TGATE1I.RAW": "7e590a67b84c2f0dee9dcd918f7a51e5",
		"TOWER1.RAW": "b3a9b47f44776e99c37c626cb974a1b2",
		"TUNNWARP.GDV": "28ac2c20475d62c3c317d60feb01b426",
		"VERSE.GDV": "28ac2c20475d62c3c317d60feb01b426",
		"VICAR.RAW": "6d0916d51019a8dda162240aa72d4abf",
		"VICAR1.RAW": "7fd18c3fe1800bc607fe5a1cf30bb024",
		"WALKWAY1.GDV": "9561088482d032cdf0acae565fc9c6c2",
		"WALKWAY2.GDV": "66b0da01c3008f3ba7ef61cb66434660",
		"WALKWAY3.GDV": "2bc5b4ef929abb5007799d4d1df09f7a",
		"WALKWY1A.GDV": "00bfbd5caf042f1b824257e4081f03d4",
		"WIN95RUN.PIF": "3313c20924fc9fa83319c34ea840f264",
	},
	Region.SPAIN: {
		"ABADON1A.GDV": "d3c996eee45ae10e1d53d47e3c4d07ce",
		"ABADON1B.GDV": "4fd48e9c49cf5daea448536fc90ca5a7",
		"ABADON1C.GDV": "f254a862bbf04c9041c95b3c9cf59c95",
		"ABADON1D.GDV": "5e386cbe85a12325487ee9d5d7b171fa",
		"ABADON1E.GDV": "e0a3ca6598d9867dd224020ccc164655",
		"ABADON1F.GDV": "10d2f2a6f4d3c9f96522b9567cd4af39",
		"ABADON1G.GDV": "aec8432275c3c4c265583ecd097562f1",
		"ABADON1H.GDV": "5df6ae9876686a7b6148fdc04ef521f5",
		"ABADON2A.GDV": "60935d28cd2777c8397fbe31181f9123",
		"ABADON2B.GDV": "945bee3a77df757d1454b8cadfc051af",
		"ABADON2C.GDV": "411064b065175dd0500415e5d48c6291",
		"ABADON2D.GDV": "01decb58ffbabbd6ba5eb98e6fd5ef14",
		"ABADON2E.GDV": "7affc2d0e2aa91284647f08515b6a887",
		"ABADON2F.GDV": "8b863b8b763634670cc656d26f8fc338",
		"ABADON2G.GDV": "b3b8820ab339ce667afa49c25558266b",
		"ABADON2H.GDV": "34dd2abb2edbdda611f6c3724417c9b7",
		"ABADON2I.GDV": "0582dbcad3a0b60eeb10e6b67d3b5936",
		"ABAGATE2.RAW": "36a05ba9452fd5a14249a06cf78a304a",
		"ADEMO.DAS": "724ce6eb75a1b9e1a3eecceace333de0",
		"AELF.RAW": "615223ddd754fc89641c2c50a6fa5f84",
		"ALEF01.GDV": "a64938174392cb174aa21ae953231412",
		"ALEF02.GDV": "ad140164487b0e363895e0126da20743",
		"ALEF03A.GDV": "6685326b1120b616e8cf0c84d4154d84",
		"ALEF03B.GDV": "3d032c64ff032c74ab433f51a8cdc625",
		"ALEF03C.GDV": "643d144c72336ce605f96ad2cc1b602b",
		"ALEF03D.GDV": "b14404e90f77c3e403cd5028622bfbe7",
		"ALEF04.GDV": "e084a802db02376328728d1305068eab",
		"ANUBIS.RAW": "f3d0726ca2f54c7592f0db18f44b8fdb",
		"AQUA1.RAW": "359b9a370554826a7482422487e5155c",
		"AQUA2.RAW": "813d6ac1dad118a676494b7a82a3853f",
		"BACKDROP.RAW": "2c9debcf210c1bfa089c5dbfae7d2f40",
		"BEAST.GDV": "9b88ece8e7dc94a3eaa501a7178151e5",
		"BELEXTRA.GDV": "29303dc47b91b660cba292e4b917dc4e",
		"BELIAL1A.GDV": "f4de42f257e184e493054364b0980623",
		"BELIAL1B.GDV": "f652698e53686c8334ddd9af579f727f",
		"BELIAL1C.GDV": "24a37579f01ea034086b0ebe3186bf48",
		"BELIAL1E.GDV": "9dc41a28191332cb7022bf173430ad98",
		"BELIAL1F.GDV": "602032a991ebe732266baa15fb151585",
		"BELSB1.GDV": "e4286d7f82948bc21346de59c61385cf",
		"BELSB3.GDV": "02f70c7506b66e1d8cb7d31d4fb2a0d7",
		"BELSB4.GDV": "6b3fd6635e7049198e5bdb6a600649e2",
		"BELSB5.GDV": "4a2cfe5ad2bd158440f272a234dd2b7d",
		"BELSB6.GDV": "e168b990d9fcf39c8d07b361d3d3fe30",
		"BENDGMA.GDV": "a3f224130d8c919f877ff60c6d0a270f",
		"BENDGMB.GDV": "5218fb23fe3cf6fad8aaf171da553e90",
		"BETRAYAL.GDV": "2547af99868f9d7d91c1cc668aa163d9",
		"BRANDED.GDV": "a9004e3528ad00db36f51dc9f1570660",
		"CAVERNS.RAW": "f8c281067b0cbb4377cd88ed1468765f",
		"CAVERNS2.RAW": "3305d61219ef7697d77e04fe3c250c72",
		"CAVERNS3.RAW": "8e3ea4e4789f1521cfb7db1dd2266158",
		"CHPTR1.GDV": "57c6ed1217bd99386c7a71432a6db65e",
		"CHPTR10.GDV": "91cc951eea421eac16d4e9e034688f73",
		"CHPTR11.GDV": "b2ae50a4eb5afd92f01e012e12570a3e",
		"CHPTR12.GDV": "a4a60a0ff79087e10439034d43af2686",
		"CHPTR13.GDV": "fdcd039adc60042268fc80f5e1a3760a",
		"CHPTR14.GDV": "2ea1d558e2248114ff3a817789a77fcf",
		"CHPTR15.GDV": "a863885c0b9c5d60234e0e3aceaef711",
		"CHPTR16.GDV": "2abd9f0863c77383e4b63415d1452a74",
		"CHPTR17.GDV": "bead0d4422a5fefd6764798b87a7333a",
		"CHPTR18.GDV": "d94628f2a585ff2e1b637e5b36836c4a",
		"CHPTR19.GDV": "9acd4037e56754bd83b50676ebb54521",
		"CHPTR2.GDV": "1989267e78f2ef3a9b66f48355cabb90",
		"CHPTR20.GDV": "a01c558bc00ca79214ff875f775b13d1",
		"CHPTR3.GDV": "0d38e8511717fd179d101834f7fbcb2d",
		"CHPTR4.GDV": "b081e7033268b44304f5df95e5702d22",
		"CHPTR5.GDV": "0947a0a094de78e1d964eba1837e3b2e",
		"CHPTR6.GDV": "724ee3a9dd013f559c5bcff1265c00dd",
		"CHPTR7.GDV": "8f5ecdbc740b8ffeb73741bcba731ab7",
		"CHPTR8.GDV": "a436d824e523ba04a44a0f995ebf6bef",
		"CHPTR9.GDV": "83791e9ed196f6c1e18103bc3b5d395b",
		"CHURCH1.RAW": "6b9c0a0bc540feebd2b66a30a30c0d3d",
		"CONFIG.INI": "a385481d80196736099b403fa7322707",
		"DBASE100.DAT": "fba6b9727a114c7f0e8c2b73ba08a14c",
		"DBASE200.DAT": "091d3e26b7b29c404d23634e2ed95d00",
		"DBASE300.DAT": "85be3a5ad6e41621df5cb6742c4035c0",
		"DBASE400.DAT": "c293f90b9f7843be3c76ec49ac200456",
		"DBASE500.DAT": "9c99f68362c749a124f8bbfb081db5b8",
		"DEMO.DAS": "19b82c32cebb7004b457d8a30e2d2afa",
		"DEMO1.DAS": "dba2eef447750c304a812af2e9d560be",
		"DEMO2.DAS": "1829a6e04d3eb32bd12a7804816efbdb",
		"DEMO3.DAS": "e083da270769a0269091a527b6931fc6",
		"DEMO4.DAS": "ef7b591e711c0fe7cca0dd4804427695",
		"DODGERA.GDV": "a8062e3a7da0b6bfd66fc3e9815ba461",
		"DOMINION.RAW": "44896acbc3f57ff4d546754bc60d1514",
		"DOPPLE.GDV": "09d65696eb51a531278a9ed25e695fb2",
		"DOPPLE.RAW": "6ef4d4f64212f65904b1d065270f9393",
		"DOS4GW.EXE": "006211ca60fa3d4b38a9a8da2ade1056",
		"DRUM.BNK": "7ecc2b70d2eb0d93475aca120b21e46b",
		"ECREDITS.GDV": "529cb02a6f8793541eea78a6083273e6",
		"EFFIGYA1.GDV": "03432f86d7bfed01231de03b13a70993",
		"EFFIGYA2.GDV": "cef37376a7f0aff15c7824fec46aedf6",
		"EFFIGYA3.GDV": "57c6d4245a038bd70ca06e61b3e72eae",
		"EFFIGYB.GDV": "d34c60f9e2de6edeb447fd8293bafea2",
		"EFFIGYC.GDV": "e8a870767f2398b6b34bbbe5025c32ca",
		"ELOHIM1.RAW": "608a9d479938d1bfa0807f94439b1954",
		"ENCHANT.GDV": "a30e9d7d5b3739da08fc6100a92dc351",
		"EPILOGUE.GDV": "dcbef38e6a90ced4ed00aca5111f5e2a",
		"FALSHIRE.GDV": "a44d63d41024d2b7adf063e1d8cd7091",
		"FILELIST.TXT": "2559efbf3d7b005b2512995ca871ba2d",
		"FLOTRAP1.GDV": "3be925d05eca81c7c5224180fd626f72",
		"FLOTRAP2.GDV": "c3eedfaa3d875141543bd75768f642ed",
		"FLOTRAP3.GDV": "1711b0fa99696841ce34259936012055",
		"FLOTRAP4.GDV": "313d48ec36dd13dd488acfe6b967d963",
		"FLOTRAP5.GDV": "b78c19cd5533d95ba65e8cc9524d2038",
		"FLOTRAP6.GDV": "a6cfd947a83f14a501ba03a5a26763a6",
		"FULLKEEP.GDV": "c59e39189dc386312d304e223fa60843",
		"FX22.SFX": "3999135d197890ce4aad9895112ebc91",
		"FXSCRIPT.SFX": "6391d983dc94a976f6d20bc5c421043b",
		"GARDENS.GDV": "eba6e46d7d84b26ef81e5a2a378060db",
		"GATE01A.GDV": "7994568417818b44f3da7b262115d020",
		"GATE01B.GDV": "47718b538176def480234483dd0c9d1d",
		"GAUL01.GDV": "20396150f4cf8b296a84f1d26c05ee68",
		"GAUL02.GDV": "161b2ac56e1d1868220d9286b8c2c5a9",
		"GLASSWIN.GDV": "2ed274b1b82b1ce374e83787be081cce",
		"GNARL1.RAW": "a7a8e89d195151ee4acdf8a85e9fa4c7",
		"GNARLA.GDV": "ada64cfa066fdb63c2ccb10ced420a82",
		"GNARLB.GDV": "d12d847d1eaa1db02469949036d8b21e",
		"GNARLC.GDV": "4e2cd1d4f0d55d0f914c8ae0f9b3ee4d",
		"GNARLD.GDV": "ce94193151ac1a4b00b0300bb185756a",
		"GNARLE.GDV": "635f5cabba263e31a572a8ea0c947025",
		"GNARLF.GDV": "958dfc333350acd9dab3aceda6ff89da",
		"GNARLG.GDV": "04693faff0dcc03ac1933fcc43a4a9bd",
		"GNARLH.GDV": "d9ea296a679a4df6223909f4cc7e5b1b",
		"GNARLI.GDV": "315ccd97bc69364edd17c344b3776dd2",
		"GRAVE.RAW": "2a943d9ede5841111a79a621c471cbca",
		"GRAVIS.INI": "e6f30ac9fe0d9f8d0a0f9bdb68ba0134",
		"GREMLOGO.GDV": "15ccc918aaedb72e5a4cd7f1efdf8ca5",
		"HAWK01.GDV": "e3abf87aab8f9bd155a2a445d067238a",
		"HAWK02A.GDV": "6cff35a43d627b7a72cf9ae8754ed505",
		"HAWK02B.GDV": "fe5d6736f4150490a3c6463e07db77cb",
		"HAWK02C.GDV": "cd07a62b0e1b7d0bf9748fb27b7e128b",
		"HAWK02D.GDV": "a9faee72bc8e2c997c3e9335f1381d44",
		"HAWK03A.GDV": "dae19800a601112f4933bd892ac117f8",
		"HAWK03B.GDV": "6dc4d29d2ee297615172ced2b5ec0c56",
		"HAWK03C.GDV": "184d36dd286ea516ae1a10f10aeabbdd",
		"HAWK03D.GDV": "23ae2cda0263a7a6543fe14a2264bd19",
		"HAWK03E.GDV": "73c271435c5a8235e4ba690876d787c4",
		"HAWK03F.GDV": "f951a8eaec8419a1217de37493ad51a4",
		"HAWK03G.GDV": "d9fa9afe2578a4356fe3c5565e4213f5",
		"HAWK03H.GDV": "c0d4f01b3adcaaa024ec9e0ecca6ca2a",
		"HAWK03I.GDV": "f7ff2a40f2402fa04768d4041bfb1c8e",
		"HAWK03J.GDV": "4da073dcae49ac570e9b1397b2fddfd5",
		"HAWK03K.GDV": "60bc96480feee3825ebb741bc4b32a51",
		"HAWK03L.GDV": "b4264ed35267a9bfb68739dcaf00bd01",
		"HAWK03M.GDV": "fe9d2cc41ad419a4d489f61ae5ce4abf",
		"HAWK03N.GDV": "d3ebd4b663d50d06b71ce4a8873add31",
		"HAWK03O.GDV": "5beb14812eba80c32243549db1352cf5",
		"HAWK03P.GDV": "f3ec92afaea5e2284254ec2b56c14d9f",
		"HAWK04.GDV": "c87a07cf0ef1c07b065d06db5eafb22a",
		"HMIDET.386": "59b3f813848cda336763648d628db917",
		"HMIDET.DRV": "f4e4ed8302f18aac0a8f8bb153a61566",
		"HMIDRV.386": "d730de4f9e6e4d8b95738f7cdde7c140",
		"HMIDRV.DRV": "cef61ca0b324c59d8e38c59ba1ddb191",
		"HMIMDRV.386": "ddfac2fec57dfd841ccfb64adee3b35e",
		"HMIMDRV.DRV": "59ad9c334c62a258b576728ef69d8af5",
		"ICONS.ALL": "91c4c33339cc0661be9568d79ead8911",
		"INSTALL.EXE": "19af77f62aa8f966206595aad3df479a",
		"INSTALL.INI": "549ef9f2df48b5add527f70d26bb802f",
		"INSTALL.PIF": "86f4bc6a60a42ab1624f072184f9a8fb",
		"INSTGRAV.INI": "8bf7b8cf7ba33e1353d2de82e142b41c",
		"INSTRA.GDV": "02cc6bbe85e81803a672212a07cce195",
		"INSTRB.GDV": "3dfb0122dd80236a7c39d54a119772f8",
		"INSTRC.GDV": "b1f40aacc8936b879b7e771c22c35a71",
		"INSTRD.GDV": "d1ef3dac9d54e461ad4e2b604b1a9f07",
		"INSTRE.GDV": "1eadba1946da4403c3475cf18bd9d0de",
		"INSTRF.GDV": "f15fd0b2656768d1b7660b785d001dda",
		"INSTRG.GDV": "5359e915bee4bf3e06fd9aca4cc49eca",
		"INSTRH.GDV": "a149bb218e40077d0e8303bfdc6f8bd8",
		"INSTRI.GDV": "391c26f1a918e8acac7e00c32062269f",
		"INTRO.GDV": "52dd4a28c0ea21b43477fc37e83075d9",
		"LOADPATS.EXE": "b24134b3fab09b2727b49a0fe1d64d8e",
		"LRINTH.RAW": "c360fa64b41a962ef74715a752e2a601",
		"LRINTH1.RAW": "c33ea660d0195daadaefb9c4ac6d4973",
		"MAS3.RAW": "9433fe3c8650f02c17f660f48ed96105",
		"MAS4.RAW": "495899fadc5e0aed8bf4629f1dd10742",
		"MAS6.RAW": "1e126f561296eeaa88df5490455558c1",
		"MAS7.RAW": "ea15ea4a67b9a7953b5066821953a002",
		"MAUSO1EA.RAW": "a0b76820c14600753823f6bd723c5ff4",
		"MAUSO1EB.RAW": "fa68e3e52147984fad267fad99febd0a",
		"MAZE.RAW": "756dc91868d45eb57c6c67345f414b1d",
		"MELODIC.BNK": "2adb649534bd5cd78900d416149c9642",
		"MOVIE.GDV": "",
		"MT32MAP.MTX": "23aa641a6b7a5855eecfb6fb48be7dc1",
		"OPTEMP1.RAW": "474639af854a2703e63cd812244b6164",
		"RANDALL.GDV": "94daf9dccc8e844167bf007fa7b1b4c7",
		"RAPHAEL.GDV": "a80f2c710dace72cf7d4753b19a7a18d",
		"RAPHAELH.GDV": "1185412171d47e00cca66de8af8eaa56",
		"RAPHAELI.GDV": "7c9f85d8271e6617677c016327b19f9c",
		"RAPHAELJ.GDV": "13c6dec7655b909e0e5beb59c0f7ea27",
		"RAPHAELK.GDV": "480921094f9399785f7e4bce1522b97c",
		"RAPHAELL.GDV": "38a64abd1cb8abbfff8406d05994e0ec",
		"RAPHAELM.GDV": "d5cbb42839b4976521204534f5142acf",
		"RAPHAELN.GDV": "28ce0ce6951225ac47ee3dcc1fd9ec95",
		"RAPHAELO.GDV": "2c7a4eb43ac20b57844631ca73bbbd9d",
		"RAQUIA1.RAW": "8a990b37e82d828325cda02e1490b979",
		"RAQUIA2.RAW": "5ddb7d371939be302e0a34a868999093",
		"RAQUIA3.RAW": "818341506472bb50a6ae8d7f1e586092",
		"RAQUIA4.RAW": "84e8913763e0be3f5122b3708e006c95",
		"RAQUIA5.RAW": "35392803892e846d34e1bbdfabbb79ab",
		"RATPFA.GDV": "34178e1593194a819db363711609278a",
		"RATPFB.GDV": "8ebcc4f4d695589bcbba8c94b39934bf",
		"RAYSEL1A.GDV": "60c1d5aa2743da87d9fade4695e67be8",
		"RAYSEL1B.GDV": "b560f06860c86194501cdb81799ceedc",
		"RAYSEL1C.GDV": "ddbc482e47a2525c0dda6f6c5f00df98",
		"RAYSEL1D.GDV": "be22ed6eb4d74dd79d54ade66ff2b82a",
		"RAYSEL1E.GDV": "056a294c207e6ac32728f2016c4ce619",
		"RAYSEL1F.GDV": "4ac222e2e93036938740ea65dd300498",
		"RAYSEL1G.GDV": "e7fe1f26ea1b94037c63325605ab6ef8",
		"README.TXT": "6da4a009344d45f142603ac49fd48f7a",
		"REALMS.BAT": "c12372f29e2a137620bd0d7833904f80",
		"REBECCA.GDV": "75fb3fbfee3057c7834c4611a94a897f",
		"REFLECTA.GDV": "0235162659b8df4ab3caa8df2717d874",
		"REFLECTB.GDV": "42d524b5457453761e14f1b8149213eb",
		"REFLECTC.GDV": "f516d19bcecded58757331d5bd3f077a",
		"RITUAL.GDV": "b30e45055ff6ca09daaf8db833e9e543",
		"ROTH.EXE": "d56e7641e8f5d4ec3144bb1c140a7677",
		"ROTH.INI": "fc87215fb74f086fba326f835ef4a6ea",
		"ROTH.RES": "3fbf68be2ba8f681ec31af5019baa6a8",
		"SALVAT.RAW": "53b13eeb5ab7fec93515dd054e62aa08",
		"SHRIVEA.GDV": "21748fe0bcbd188b1622171237001a94",
		"SHRIVEB.GDV": "866e69928bd197e1aa58125ec29845a5",
		"SHRIVEC.GDV": "0313a5b72e32076ebefc81ec602ac6c2",
		"SHRIVED.GDV": "a38a82889a9875266d564f9d659c5ddd",
		"SOULST2.RAW": "f0cad71c8afc062d53608ef11cb6d909",
		"SOULST3.RAW": "a8e7a235bc9abcbd77b48b53edda6d65",
		"STUDY1.RAW": "bfa4969d04e8d3b7733422e48416b0de",
		"STUDY2.RAW": "08ff34b8a5fd3960e87348249baee7c5",
		"STUDY3.RAW": "470b41d230e77e43af5f61b83586fe1a",
		"STUDY4.RAW": "5d35a447593aca6456bbf7661db072bf",
		"TALEWELA.GDV": "56494b95ed713f5807e1d56c71e7fdd9",
		"TALEWELB.GDV": "7aae9e1f25833a4d20c30ca87cd9b54d",
		"TALEWELC.GDV": "31817dc7b6b81c4745503a57b7465a81",
		"TEMPLE1.RAW": "b3ce9b844300a9770bb1b81f9fc28b9b",
		"TGATE1F.RAW": "156e4258fe3ee25689f74e215a8f1e13",
		"TGATE1G.RAW": "1027ec59614318cae2d0e8b51bcaf54b",
		"TGATE1H.RAW": "23e6938f1d51a510c60fd72f961cbac9",
		"TGATE1I.RAW": "7e590a67b84c2f0dee9dcd918f7a51e5",
		"TOWER1.RAW": "b3a9b47f44776e99c37c626cb974a1b2",
		"TUNNWARP.GDV": "28ac2c20475d62c3c317d60feb01b426",
		"VERSE.GDV": "28ac2c20475d62c3c317d60feb01b426",
		"VICAR.RAW": "6d0916d51019a8dda162240aa72d4abf",
		"VICAR1.RAW": "7fd18c3fe1800bc607fe5a1cf30bb024",
		"WALKWAY1.GDV": "f0ba543d739fd8595ef818b3e4c8df65",
		"WALKWAY2.GDV": "a0af150cddc3173f8eb636e5c9fa2c3e",
		"WALKWAY3.GDV": "2bc5b4ef929abb5007799d4d1df09f7a",
		"WALKWY1A.GDV": "00bfbd5caf042f1b824257e4081f03d4",
		"WIN95RUN.PIF": "3313c20924fc9fa83319c34ea840f264",
	},
}
#endregion

const REQUIRED_FILES: Array = [
	"roth_exe",
	"roth_res",
	"roth_ini",
]

#region Filepaths

var directory: String = ""
var roth_exe: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/ROTH.EXE")):
			return directory.path_join("ROTH/ROTH.EXE")
		elif FileAccess.file_exists(directory.path_join("DATA/ROTH.EXE")):
			return directory.path_join("DATA/ROTH.EXE")
		elif FileAccess.file_exists(directory.path_join("DATA/INSTALL/ROTH.EXE")):
			return directory.path_join("DATA/INSTALL/ROTH.EXE")
		else:
			return ""
var roth_exe_md5: String :
	get():
		return MD5[roth_res_region][roth_exe.get_file()]
var roth_exe_md5_calculated: String :
	get():
		if not roth_exe_md5_calculated.is_empty():
			return roth_exe_md5_calculated
		if FileAccess.file_exists(roth_exe):
			roth_exe_md5_calculated = FileAccess.get_md5(roth_exe)
		return roth_exe_md5_calculated
var roth_exe_version: String :
	get():
		if not roth_exe_version.is_empty():
			return roth_exe_version
		var file: FileAccess = FileAccess.open(roth_exe, FileAccess.READ)
		file.seek(0x6ca69)
		if file.get_buffer(5) == "3.925".to_ascii_buffer():
			roth_exe_version = "3.925"
		file.seek(0x6d069)
		if file.get_buffer(5) == "3.983".to_ascii_buffer():
			roth_exe_version = "3.983"
		file.close()
		return roth_exe_version
var roth_custom_exe: String :
	get():
		if DirAccess.dir_exists_absolute(directory.path_join("ROTH")):
			return directory.path_join("ROTH/ROTH_CUSTOM.EXE")
		return directory.path_join("ROTH_CUSTOM.EXE")
var roth_res: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/ROTH.RES")):
			return directory.path_join("ROTH/ROTH.RES")
		elif FileAccess.file_exists(directory.path_join("DATA/ROTH.RES")):
			return directory.path_join("DATA/ROTH.RES")
		elif FileAccess.file_exists(directory.path_join("DATA/INSTALL/ROTH.RES")):
			return directory.path_join("DATA/INSTALL/ROTH.RES")
		else:
			return ""
var roth_res_version: String :
	get():
		if not roth_res_version.is_empty():
			return roth_res_version
		var file := FileAccess.open(roth_res, FileAccess.READ)
		var version: String = file.get_line().split("=")[1].trim_prefix("\"").trim_suffix("\"")
		match version:
			"Roth Version F1.4":
				roth_res_region = Region.UK
				roth_res_version = "F1.4"
				language = "English (UK)"
			"Roth Version 1.12":
				roth_res_region = Region.US
				roth_res_version = "1.12"
				language = "English (US)"
			"Roth Version F1.8":
				roth_res_region = Region.GERMANY
				roth_res_version = "F1.8"
				language = "German"
			"Roth Version F1.14":
				roth_res_region = Region.FRANCE
				roth_res_version = "F1.14"
				language = "French"
			"Spanish Roth F1":
				roth_res_region = Region.SPAIN
				roth_res_version = "F1"
				language = "Spanish"
			"Roth Version 1.8":
				roth_res_region = Region.ITALY
				roth_res_version = "1.8"
				language = "Italian"
		return roth_res_version
var roth_res_region: Region = Region.UNKNOWN :
	get():
		if roth_res_region != Region.UNKNOWN:
			return roth_res_region
		var file := FileAccess.open(roth_res, FileAccess.READ)
		var version: String = file.get_line().split("=")[1].trim_prefix("\"").trim_suffix("\"")
		match version:
			"Roth Version F1.4":
				roth_res_region = Region.UK
				roth_res_version = "F1.4"
				language = "English (UK)"
			"Roth Version 1.12":
				roth_res_region = Region.US
				roth_res_version = "1.12"
				language = "English (US)"
			"Roth Version F1.8":
				roth_res_region = Region.GERMANY
				roth_res_version = "F1.8"
				language = "German"
			"Roth Version F1.14":
				roth_res_region = Region.FRANCE
				roth_res_version = "F1.14"
				language = "French"
			"Spanish Roth F1":
				roth_res_region = Region.SPAIN
				roth_res_version = "F1"
				language = "Spanish"
			"Roth Version 1.8":
				roth_res_region = Region.ITALY
				roth_res_version = "1.8"
				language = "Italian"
		return roth_res_region
var language: String :
	get():
		if not language.is_empty():
			return language
		var file := FileAccess.open(roth_res, FileAccess.READ)
		var version: String = file.get_line().split("=")[1].trim_prefix("\"").trim_suffix("\"")
		match version:
			"Roth Version F1.4":
				roth_res_region = Region.UK
				roth_res_version = "F1.4"
				language = "English (UK)"
			"Roth Version 1.12":
				roth_res_region = Region.US
				roth_res_version = "1.12"
				language = "English (US)"
			"Roth Version F1.8":
				roth_res_region = Region.GERMANY
				roth_res_version = "F1.8"
				language = "German"
			"Roth Version F1.14":
				roth_res_region = Region.FRANCE
				roth_res_version = "F1.14"
				language = "French"
			"Spanish Roth F1":
				roth_res_region = Region.SPAIN
				roth_res_version = "F1"
				language = "Spanish"
			"Roth Version 1.8":
				roth_res_region = Region.ITALY
				roth_res_version = "1.8"
				language = "Italian"
		return language
var store: String :
	get():
		if FileAccess.file_exists(directory.path_join("gog.ico")):
			return "GOG"
		elif directory.to_lower().contains("steam"):
			return "Steam"
		else:
			return "Unknown"
var roth_ini: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/ROTH.INI")):
			return directory.path_join("ROTH/ROTH.INI")
		elif FileAccess.file_exists(directory.path_join("DATA/ROTH.INI")):
			return directory.path_join("DATA/ROTH.INI")
		elif FileAccess.file_exists(directory.path_join("DATA/INSTALL/ROTH.INI")):
			return directory.path_join("DATA/INSTALL/ROTH.INI")
		else:
			return ""
var config_ini: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/CONFIG.INI")):
			return directory.path_join("ROTH/CONFIG.INI")
		elif FileAccess.file_exists(directory.path_join("DATA/CONFIG.INI")):
			return directory.path_join("DATA/CONFIG.INI")
		elif FileAccess.file_exists(directory.path_join("DATA/INSTALL/CONFIG.INI")):
			return directory.path_join("DATA/INSTALL/CONFIG.INI")
		else:
			return ""
var dbase100: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/DBASE100.DAT")):
			return directory.path_join("ROTH/DBASE100.DAT")
		elif FileAccess.file_exists(directory.path_join("DATA/DBASE100.DAT")):
			return directory.path_join("DATA/DBASE100.DAT")
		else:
			return ""
var dbase200: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/DBASE200.DAT")):
			return directory.path_join("ROTH/DBASE200.DAT")
		elif FileAccess.file_exists(directory.path_join("DATA/DBASE200.DAT")):
			return directory.path_join("DATA/DBASE200.DAT")
		else:
			return ""
var dbase300: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/DBASE300.DAT")):
			return directory.path_join("ROTH/DBASE300.DAT")
		elif FileAccess.file_exists(directory.path_join("DATA/DBASE300.DAT")):
			return directory.path_join("DATA/DBASE300.DAT")
		else:
			return ""
var dbase400: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/DBASE400.DAT")):
			return directory.path_join("ROTH/DBASE400.DAT")
		elif FileAccess.file_exists(directory.path_join("DATA/DBASE400.DAT")):
			return directory.path_join("DATA/DBASE400.DAT")
		else:
			return ""
var dbase500: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/DBASE500.DAT")):
			return directory.path_join("ROTH/DBASE500.DAT")
		elif FileAccess.file_exists(directory.path_join("DATA/DBASE500.DAT")):
			return directory.path_join("DATA/DBASE500.DAT")
		else:
			return ""
var dos4gw: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/DOS4GW.EXE")):
			return directory.path_join("ROTH/DOS4GW.EXE")
		elif FileAccess.file_exists(directory.path_join("DATA/DOS4GW.EXE")):
			return directory.path_join("DATA/DOS4GW.EXE")
		elif FileAccess.file_exists(directory.path_join("DATA/INSTALL/DOS4GW.EXE")):
			return directory.path_join("DATA/INSTALL/DOS4GW.EXE")
		else:
			return ""

var backdrop: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/DATA/BACKDROP.RAW")):
			return directory.path_join("ROTH/DATA/BACKDROP.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/DATA/BACKDROP.RAW")):
			return directory.path_join("DATA/DATA/BACKDROP.RAW")
		else:
			return ""
var fxscript: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/DATA/FXSCRIPT.SFX")):
			return directory.path_join("ROTH/DATA/FXSCRIPT.SFX")
		elif FileAccess.file_exists(directory.path_join("DATA/DATA/FXSCRIPT.SFX")):
			return directory.path_join("DATA/DATA/FXSCRIPT.SFX")
		else:
			return ""
var fx22: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/DATA/FX22.SFX")):
			return directory.path_join("ROTH/DATA/FX22.SFX")
		elif FileAccess.file_exists(directory.path_join("DATA/DATA/FX22.SFX")):
			return directory.path_join("DATA/DATA/FX22.SFX")
		else:
			return ""
var icons: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/DATA/ICONS.ALL")):
			return directory.path_join("ROTH/DATA/ICONS.ALL")
		elif FileAccess.file_exists(directory.path_join("DATA/DATA/ICONS.ALL")):
			return directory.path_join("DATA/DATA/ICONS.ALL")
		else:
			return ""

var hmidet_386: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/DIGI/HMIDET.386")):
			return directory.path_join("ROTH/DIGI/HMIDET.386")
		elif FileAccess.file_exists(directory.path_join("DATA/DIGI/HMIDET.386")):
			return directory.path_join("DATA/DIGI/HMIDET.386")
		else:
			return ""
var hmidrv_386: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/DIGI/HMIDRV.386")):
			return directory.path_join("ROTH/DIGI/HMIDRV.386")
		elif FileAccess.file_exists(directory.path_join("DATA/DIGI/HMIDRV.386")):
			return directory.path_join("DATA/DIGI/HMIDRV.386")
		else:
			return ""
var hmimdrv_386: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/MIDI/HMIMDRV.386")):
			return directory.path_join("ROTH/MIDI/HMIMDRV.386")
		elif FileAccess.file_exists(directory.path_join("DATA/MIDI/HMIMDRV.386")):
			return directory.path_join("DATA/MIDI/HMIMDRV.386")
		else:
			return ""
var drum: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/MIDI/DRUM.BNK")):
			return directory.path_join("ROTH/MIDI/DRUM.BNK")
		elif FileAccess.file_exists(directory.path_join("DATA/MIDI/DRUM.BNK")):
			return directory.path_join("DATA/MIDI/DRUM.BNK")
		else:
			return ""
var melodic: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/MIDI/MELODIC.BNK")):
			return directory.path_join("ROTH/MIDI/MELODIC.BNK")
		elif FileAccess.file_exists(directory.path_join("DATA/MIDI/MELODIC.BNK")):
			return directory.path_join("DATA/MIDI/MELODIC.BNK")
		else:
			return ""

var ademo: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/ADEMO.DAS")):
			return directory.path_join("ROTH/M/ADEMO.DAS")
		elif FileAccess.file_exists(directory.path_join("DATA/M/ADEMO.DAS")):
			return directory.path_join("DATA/M/ADEMO.DAS")
		else:
			return ""
var demo: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/DEMO.DAS")):
			return directory.path_join("ROTH/M/DEMO.DAS")
		elif FileAccess.file_exists(directory.path_join("DATA/M/DEMO.DAS")):
			return directory.path_join("DATA/M/DEMO.DAS")
		else:
			return ""
var demo1: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/DEMO1.DAS")):
			return directory.path_join("ROTH/M/DEMO1.DAS")
		elif FileAccess.file_exists(directory.path_join("DATA/M/DEMO1.DAS")):
			return directory.path_join("DATA/M/DEMO1.DAS")
		else:
			return ""
var demo2: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/DEMO2.DAS")):
			return directory.path_join("ROTH/M/DEMO2.DAS")
		elif FileAccess.file_exists(directory.path_join("DATA/M/DEMO2.DAS")):
			return directory.path_join("DATA/M/DEMO2.DAS")
		else:
			return ""
var demo3: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/DEMO3.DAS")):
			return directory.path_join("ROTH/M/DEMO3.DAS")
		elif FileAccess.file_exists(directory.path_join("DATA/M/DEMO3.DAS")):
			return directory.path_join("DATA/M/DEMO3.DAS")
		else:
			return ""
var demo4: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/DEMO4.DAS")):
			return directory.path_join("ROTH/M/DEMO4.DAS")
		elif FileAccess.file_exists(directory.path_join("DATA/M/DEMO4.DAS")):
			return directory.path_join("DATA/M/DEMO4.DAS")
		else:
			return ""

var abagate2: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/ABAGATE2.RAW")):
			return directory.path_join("ROTH/M/ABAGATE2.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/ABAGATE2.RAW")):
			return directory.path_join("DATA/M/ABAGATE2.RAW")
		else:
			return ""
var aelf: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/AELF.RAW")):
			return directory.path_join("ROTH/M/AELF.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/AELF.RAW")):
			return directory.path_join("DATA/M/AELF.RAW")
		else:
			return ""
var anubis: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/ANUBIS.RAW")):
			return directory.path_join("ROTH/M/ANUBIS.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/ANUBIS.RAW")):
			return directory.path_join("DATA/M/ANUBIS.RAW")
		else:
			return ""
var aqua1: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/AQUA1.RAW")):
			return directory.path_join("ROTH/M/AQUA1.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/AQUA1.RAW")):
			return directory.path_join("DATA/M/AQUA1.RAW")
		else:
			return ""
var aqua2: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/AQUA2.RAW")):
			return directory.path_join("ROTH/M/AQUA2.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/AQUA2.RAW")):
			return directory.path_join("DATA/M/AQUA2.RAW")
		else:
			return ""
var caverns: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/CAVERNS.RAW")):
			return directory.path_join("ROTH/M/CAVERNS.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/CAVERNS.RAW")):
			return directory.path_join("DATA/M/CAVERNS.RAW")
		else:
			return ""
var caverns2: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/CAVERNS2.RAW")):
			return directory.path_join("ROTH/M/CAVERNS2.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/CAVERNS2.RAW")):
			return directory.path_join("DATA/M/CAVERNS2.RAW")
		else:
			return ""
var caverns3: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/CAVERNS3.RAW")):
			return directory.path_join("ROTH/M/CAVERNS3.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/CAVERNS3.RAW")):
			return directory.path_join("DATA/M/CAVERNS3.RAW")
		else:
			return ""
var church1: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/CHURCH1.RAW")):
			return directory.path_join("ROTH/M/CHURCH1.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/CHURCH1.RAW")):
			return directory.path_join("DATA/M/CHURCH1.RAW")
		else:
			return ""
var dominion: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/DOMINION.RAW")):
			return directory.path_join("ROTH/M/DOMINION.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/DOMINION.RAW")):
			return directory.path_join("DATA/M/DOMINION.RAW")
		else:
			return ""
var dopple: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/DOPPLE.RAW")):
			return directory.path_join("ROTH/M/DOPPLE.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/DOPPLE.RAW")):
			return directory.path_join("DATA/M/DOPPLE.RAW")
		else:
			return ""
var elohim1: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/ELOHIM1.RAW")):
			return directory.path_join("ROTH/M/ELOHIM1.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/ELOHIM1.RAW")):
			return directory.path_join("DATA/M/ELOHIM1.RAW")
		else:
			return ""
var gnarl1: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/GNARL1.RAW")):
			return directory.path_join("ROTH/M/GNARL1.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/GNARL1.RAW")):
			return directory.path_join("DATA/M/GNARL1.RAW")
		else:
			return ""
var grave: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/GRAVE.RAW")):
			return directory.path_join("ROTH/M/GRAVE.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/GRAVE.RAW")):
			return directory.path_join("DATA/M/GRAVE.RAW")
		else:
			return ""
var lrinth: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/LRINTH.RAW")):
			return directory.path_join("ROTH/M/LRINTH.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/LRINTH.RAW")):
			return directory.path_join("DATA/M/LRINTH.RAW")
		else:
			return ""
var lrinth1: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/LRINTH1.RAW")):
			return directory.path_join("ROTH/M/LRINTH1.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/LRINTH1.RAW")):
			return directory.path_join("DATA/M/LRINTH1.RAW")
		else:
			return ""
var mas3: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/MAS3.RAW")):
			return directory.path_join("ROTH/M/MAS3.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/MAS3.RAW")):
			return directory.path_join("DATA/M/MAS3.RAW")
		else:
			return ""
var mas4: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/MAS4.RAW")):
			return directory.path_join("ROTH/M/MAS4.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/MAS4.RAW")):
			return directory.path_join("DATA/M/MAS4.RAW")
		else:
			return ""
var mas6: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/MAS6.RAW")):
			return directory.path_join("ROTH/M/MAS6.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/MAS6.RAW")):
			return directory.path_join("DATA/M/MAS6.RAW")
		else:
			return ""
var mas7: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/MAS7.RAW")):
			return directory.path_join("ROTH/M/MAS7.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/MAS7.RAW")):
			return directory.path_join("DATA/M/MAS7.RAW")
		else:
			return ""
var mauso1ea: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/MAUSO1EA.RAW")):
			return directory.path_join("ROTH/M/MAUSO1EA.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/MAUSO1EA.RAW")):
			return directory.path_join("DATA/M/MAUSO1EA.RAW")
		else:
			return ""
var mauso1eb: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/MAUSO1EB.RAW")):
			return directory.path_join("ROTH/M/MAUSO1EB.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/MAUSO1EB.RAW")):
			return directory.path_join("DATA/M/MAUSO1EB.RAW")
		else:
			return ""
var maze: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/MAZE.RAW")):
			return directory.path_join("ROTH/M/MAZE.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/MAZE.RAW")):
			return directory.path_join("DATA/M/MAZE.RAW")
		else:
			return ""
var optemp1: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/OPTEMP1.RAW")):
			return directory.path_join("ROTH/M/OPTEMP1.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/OPTEMP1.RAW")):
			return directory.path_join("DATA/M/OPTEMP1.RAW")
		else:
			return ""
var raquia1: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/RAQUIA1.RAW")):
			return directory.path_join("ROTH/M/RAQUIA1.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/RAQUIA1.RAW")):
			return directory.path_join("DATA/M/RAQUIA1.RAW")
		else:
			return ""
var raquia2: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/RAQUIA2.RAW")):
			return directory.path_join("ROTH/M/RAQUIA2.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/RAQUIA2.RAW")):
			return directory.path_join("DATA/M/RAQUIA2.RAW")
		else:
			return ""
var raquia3: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/RAQUIA3.RAW")):
			return directory.path_join("ROTH/M/RAQUIA3.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/RAQUIA3.RAW")):
			return directory.path_join("DATA/M/RAQUIA3.RAW")
		else:
			return ""
var raquia4: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/RAQUIA4.RAW")):
			return directory.path_join("ROTH/M/RAQUIA4.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/RAQUIA4.RAW")):
			return directory.path_join("DATA/M/RAQUIA4.RAW")
		else:
			return ""
var raquia5: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/RAQUIA5.RAW")):
			return directory.path_join("ROTH/M/RAQUIA5.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/RAQUIA5.RAW")):
			return directory.path_join("DATA/M/RAQUIA5.RAW")
		else:
			return ""
var salvat: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/SALVAT.RAW")):
			return directory.path_join("ROTH/M/SALVAT.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/SALVAT.RAW")):
			return directory.path_join("DATA/M/SALVAT.RAW")
		else:
			return ""
var soulst2: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/SOULST2.RAW")):
			return directory.path_join("ROTH/M/SOULST2.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/SOULST2.RAW")):
			return directory.path_join("DATA/M/SOULST2.RAW")
		else:
			return ""
var soulst3: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/SOULST3.RAW")):
			return directory.path_join("ROTH/M/SOULST3.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/SOULST3.RAW")):
			return directory.path_join("DATA/M/SOULST3.RAW")
		else:
			return ""
var study1: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/STUDY1.RAW")):
			return directory.path_join("ROTH/M/STUDY1.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/STUDY1.RAW")):
			return directory.path_join("DATA/M/STUDY1.RAW")
		else:
			return ""
var study2: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/STUDY2.RAW")):
			return directory.path_join("ROTH/M/STUDY2.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/STUDY2.RAW")):
			return directory.path_join("DATA/M/STUDY2.RAW")
		else:
			return ""
var study3: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/STUDY3.RAW")):
			return directory.path_join("ROTH/M/STUDY3.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/STUDY3.RAW")):
			return directory.path_join("DATA/M/STUDY3.RAW")
		else:
			return ""
var study4: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/STUDY4.RAW")):
			return directory.path_join("ROTH/M/STUDY4.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/STUDY4.RAW")):
			return directory.path_join("DATA/M/STUDY4.RAW")
		else:
			return ""
var temple1: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/TEMPLE1.RAW")):
			return directory.path_join("ROTH/M/TEMPLE1.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/TEMPLE1.RAW")):
			return directory.path_join("DATA/M/TEMPLE1.RAW")
		else:
			return ""
var tgate1f: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/TGATE1F.RAW")):
			return directory.path_join("ROTH/M/TGATE1F.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/TGATE1F.RAW")):
			return directory.path_join("DATA/M/TGATE1F.RAW")
		else:
			return ""
var tgate1g: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/TGATE1G.RAW")):
			return directory.path_join("ROTH/M/TGATE1G.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/TGATE1G.RAW")):
			return directory.path_join("DATA/M/TGATE1G.RAW")
		else:
			return ""
var tgate1h: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/TGATE1H.RAW")):
			return directory.path_join("ROTH/M/TGATE1H.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/TGATE1H.RAW")):
			return directory.path_join("DATA/M/TGATE1H.RAW")
		else:
			return ""
var tgate1i: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/TGATE1I.RAW")):
			return directory.path_join("ROTH/M/TGATE1I.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/TGATE1I.RAW")):
			return directory.path_join("DATA/M/TGATE1I.RAW")
		else:
			return ""
var tower1: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/TOWER1.RAW")):
			return directory.path_join("ROTH/M/TOWER1.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/TOWER1.RAW")):
			return directory.path_join("DATA/M/TOWER1.RAW")
		else:
			return ""
var vicar: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/VICAR.RAW")):
			return directory.path_join("ROTH/M/VICAR.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/VICAR.RAW")):
			return directory.path_join("DATA/M/VICAR.RAW")
		else:
			return ""
var vicar1: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/M/VICAR1.RAW")):
			return directory.path_join("ROTH/M/VICAR1.RAW")
		elif FileAccess.file_exists(directory.path_join("DATA/M/VICAR1.RAW")):
			return directory.path_join("DATA/M/VICAR1.RAW")
		else:
			return ""

var abadon1a_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/ABADON1A.GDV")):
			return directory.path_join("ROTH/GDV/ABADON1A.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/ABADON1A.GDV")):
			return directory.path_join("DATA/GDV/ABADON1A.GDV")
		else:
			return ""
var abadon1b_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/ABADON1B.GDV")):
			return directory.path_join("ROTH/GDV/ABADON1B.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/ABADON1B.GDV")):
			return directory.path_join("DATA/GDV/ABADON1B.GDV")
		else:
			return ""
var abadon1c_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/ABADON1C.GDV")):
			return directory.path_join("ROTH/GDV/ABADON1C.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/ABADON1C.GDV")):
			return directory.path_join("DATA/GDV/ABADON1C.GDV")
		else:
			return ""
var abadon1d_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/ABADON1D.GDV")):
			return directory.path_join("ROTH/GDV/ABADON1D.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/ABADON1D.GDV")):
			return directory.path_join("DATA/GDV/ABADON1D.GDV")
		else:
			return ""
var abadon1e_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/ABADON1E.GDV")):
			return directory.path_join("ROTH/GDV/ABADON1E.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/ABADON1E.GDV")):
			return directory.path_join("DATA/GDV/ABADON1E.GDV")
		else:
			return ""
var abadon1f_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/ABADON1F.GDV")):
			return directory.path_join("ROTH/GDV/ABADON1F.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/ABADON1F.GDV")):
			return directory.path_join("DATA/GDV/ABADON1F.GDV")
		else:
			return ""
var abadon1g_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/ABADON1G.GDV")):
			return directory.path_join("ROTH/GDV/ABADON1G.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/ABADON1G.GDV")):
			return directory.path_join("DATA/GDV/ABADON1G.GDV")
		else:
			return ""
var abadon1h_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/ABADON1H.GDV")):
			return directory.path_join("ROTH/GDV/ABADON1H.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/ABADON1H.GDV")):
			return directory.path_join("DATA/GDV/ABADON1H.GDV")
		else:
			return ""
var abadon2a_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/ABADON2A.GDV")):
			return directory.path_join("ROTH/GDV/ABADON2A.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/ABADON2A.GDV")):
			return directory.path_join("DATA/GDV/ABADON2A.GDV")
		else:
			return ""
var abadon2b_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/ABADON2B.GDV")):
			return directory.path_join("ROTH/GDV/ABADON2B.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/ABADON2B.GDV")):
			return directory.path_join("DATA/GDV/ABADON2B.GDV")
		else:
			return ""
var abadon2c_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/ABADON2C.GDV")):
			return directory.path_join("ROTH/GDV/ABADON2C.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/ABADON2C.GDV")):
			return directory.path_join("DATA/GDV/ABADON2C.GDV")
		else:
			return ""
var abadon2d_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/ABADON2D.GDV")):
			return directory.path_join("ROTH/GDV/ABADON2D.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/ABADON2D.GDV")):
			return directory.path_join("DATA/GDV/ABADON2D.GDV")
		else:
			return ""
var abadon2e_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/ABADON2E.GDV")):
			return directory.path_join("ROTH/GDV/ABADON2E.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/ABADON2E.GDV")):
			return directory.path_join("DATA/GDV/ABADON2E.GDV")
		else:
			return ""
var abadon2f_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/ABADON2F.GDV")):
			return directory.path_join("ROTH/GDV/ABADON2F.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/ABADON2F.GDV")):
			return directory.path_join("DATA/GDV/ABADON2F.GDV")
		else:
			return ""
var abadon2g_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/ABADON2G.GDV")):
			return directory.path_join("ROTH/GDV/ABADON2G.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/ABADON2G.GDV")):
			return directory.path_join("DATA/GDV/ABADON2G.GDV")
		else:
			return ""
var abadon2h_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/ABADON2H.GDV")):
			return directory.path_join("ROTH/GDV/ABADON2H.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/ABADON2H.GDV")):
			return directory.path_join("DATA/GDV/ABADON2H.GDV")
		else:
			return ""
var abadon2i_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/ABADON2I.GDV")):
			return directory.path_join("ROTH/GDV/ABADON2I.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/ABADON2I.GDV")):
			return directory.path_join("DATA/GDV/ABADON2I.GDV")
		else:
			return ""
var alef01_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/ALEF01.GDV")):
			return directory.path_join("ROTH/GDV/ALEF01.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/ALEF01.GDV")):
			return directory.path_join("DATA/GDV/ALEF01.GDV")
		else:
			return ""
var alef02_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/ALEF02.GDV")):
			return directory.path_join("ROTH/GDV/ALEF02.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/ALEF02.GDV")):
			return directory.path_join("DATA/GDV/ALEF02.GDV")
		else:
			return ""
var alef03a_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/ALEF03A.GDV")):
			return directory.path_join("ROTH/GDV/ALEF03A.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/ALEF03A.GDV")):
			return directory.path_join("DATA/GDV/ALEF03A.GDV")
		else:
			return ""
var alef03b_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/ALEF03B.GDV")):
			return directory.path_join("ROTH/GDV/ALEF03B.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/ALEF03B.GDV")):
			return directory.path_join("DATA/GDV/ALEF03B.GDV")
		else:
			return ""
var alef03c_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/ALEF03C.GDV")):
			return directory.path_join("ROTH/GDV/ALEF03C.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/ALEF03C.GDV")):
			return directory.path_join("DATA/GDV/ALEF03C.GDV")
		else:
			return ""
var alef03d_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/ALEF03D.GDV")):
			return directory.path_join("ROTH/GDV/ALEF03D.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/ALEF03D.GDV")):
			return directory.path_join("DATA/GDV/ALEF03D.GDV")
		else:
			return ""
var alef04_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/ALEF04.GDV")):
			return directory.path_join("ROTH/GDV/ALEF04.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/ALEF04.GDV")):
			return directory.path_join("DATA/GDV/ALEF04.GDV")
		else:
			return ""
var beast_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/BEAST.GDV")):
			return directory.path_join("ROTH/GDV/BEAST.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/BEAST.GDV")):
			return directory.path_join("DATA/GDV/BEAST.GDV")
		else:
			return ""
var belextra_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/BELEXTRA.GDV")):
			return directory.path_join("ROTH/GDV/BELEXTRA.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/BELEXTRA.GDV")):
			return directory.path_join("DATA/GDV/BELEXTRA.GDV")
		else:
			return ""
var belial1a_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/BELIAL1A.GDV")):
			return directory.path_join("ROTH/GDV/BELIAL1A.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/BELIAL1A.GDV")):
			return directory.path_join("DATA/GDV/BELIAL1A.GDV")
		else:
			return ""
var belial1b_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/BELIAL1B.GDV")):
			return directory.path_join("ROTH/GDV/BELIAL1B.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/BELIAL1B.GDV")):
			return directory.path_join("DATA/GDV/BELIAL1B.GDV")
		else:
			return ""
var belial1c_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/BELIAL1C.GDV")):
			return directory.path_join("ROTH/GDV/BELIAL1C.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/BELIAL1C.GDV")):
			return directory.path_join("DATA/GDV/BELIAL1C.GDV")
		else:
			return ""
var belial1e_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/BELIAL1E.GDV")):
			return directory.path_join("ROTH/GDV/BELIAL1E.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/BELIAL1E.GDV")):
			return directory.path_join("DATA/GDV/BELIAL1E.GDV")
		else:
			return ""
var belial1f_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/BELIAL1F.GDV")):
			return directory.path_join("ROTH/GDV/BELIAL1F.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/BELIAL1F.GDV")):
			return directory.path_join("DATA/GDV/BELIAL1F.GDV")
		else:
			return ""
var belsb1_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/BELSB1.GDV")):
			return directory.path_join("ROTH/GDV/BELSB1.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/BELSB1.GDV")):
			return directory.path_join("DATA/GDV/BELSB1.GDV")
		else:
			return ""
var belsb2_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/BELSB2.GDV")):
			return directory.path_join("ROTH/GDV/BELSB2.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/BELSB2.GDV")):
			return directory.path_join("DATA/GDV/BELSB2.GDV")
		else:
			return ""
var belsb3_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/BELSB3.GDV")):
			return directory.path_join("ROTH/GDV/BELSB3.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/BELSB3.GDV")):
			return directory.path_join("DATA/GDV/BELSB3.GDV")
		else:
			return ""
var belsb4_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/BELSB4.GDV")):
			return directory.path_join("ROTH/GDV/BELSB4.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/BELSB4.GDV")):
			return directory.path_join("DATA/GDV/BELSB4.GDV")
		else:
			return ""
var belsb5_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/BELSB5.GDV")):
			return directory.path_join("ROTH/GDV/BELSB5.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/BELSB5.GDV")):
			return directory.path_join("DATA/GDV/BELSB5.GDV")
		else:
			return ""
var belsb6_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/BELSB6.GDV")):
			return directory.path_join("ROTH/GDV/BELSB6.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/BELSB6.GDV")):
			return directory.path_join("DATA/GDV/BELSB6.GDV")
		else:
			return ""
var bendgma_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/BENDGMA.GDV")):
			return directory.path_join("ROTH/GDV/BENDGMA.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/BENDGMA.GDV")):
			return directory.path_join("DATA/GDV/BENDGMA.GDV")
		else:
			return ""
var bendgmb_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/BENDGMB.GDV")):
			return directory.path_join("ROTH/GDV/BENDGMB.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/BENDGMB.GDV")):
			return directory.path_join("DATA/GDV/BENDGMB.GDV")
		else:
			return ""
var betrayal_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/BETRAYAL.GDV")):
			return directory.path_join("ROTH/GDV/BETRAYAL.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/BETRAYAL.GDV")):
			return directory.path_join("DATA/GDV/BETRAYAL.GDV")
		else:
			return ""
var branded_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/BRANDED.GDV")):
			return directory.path_join("ROTH/GDV/BRANDED.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/BRANDED.GDV")):
			return directory.path_join("DATA/GDV/BRANDED.GDV")
		else:
			return ""
var chptr1_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/CHPTR1.GDV")):
			return directory.path_join("ROTH/GDV/CHPTR1.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/CHPTR1.GDV")):
			return directory.path_join("DATA/GDV/CHPTR1.GDV")
		else:
			return ""
var chptr10_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/CHPTR10.GDV")):
			return directory.path_join("ROTH/GDV/CHPTR10.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/CHPTR10.GDV")):
			return directory.path_join("DATA/GDV/CHPTR10.GDV")
		else:
			return ""
var chptr11_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/CHPTR11.GDV")):
			return directory.path_join("ROTH/GDV/CHPTR11.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/CHPTR11.GDV")):
			return directory.path_join("DATA/GDV/CHPTR11.GDV")
		else:
			return ""
var chptr12_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/CHPTR12.GDV")):
			return directory.path_join("ROTH/GDV/CHPTR12.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/CHPTR12.GDV")):
			return directory.path_join("DATA/GDV/CHPTR12.GDV")
		else:
			return ""
var chptr13_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/CHPTR13.GDV")):
			return directory.path_join("ROTH/GDV/CHPTR13.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/CHPTR13.GDV")):
			return directory.path_join("DATA/GDV/CHPTR13.GDV")
		else:
			return ""
var chptr14_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/CHPTR14.GDV")):
			return directory.path_join("ROTH/GDV/CHPTR14.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/CHPTR14.GDV")):
			return directory.path_join("DATA/GDV/CHPTR14.GDV")
		else:
			return ""
var chptr15_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/CHPTR15.GDV")):
			return directory.path_join("ROTH/GDV/CHPTR15.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/CHPTR15.GDV")):
			return directory.path_join("DATA/GDV/CHPTR15.GDV")
		else:
			return ""
var chptr16_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/CHPTR16.GDV")):
			return directory.path_join("ROTH/GDV/CHPTR16.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/CHPTR16.GDV")):
			return directory.path_join("DATA/GDV/CHPTR16.GDV")
		else:
			return ""
var chptr17_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/CHPTR17.GDV")):
			return directory.path_join("ROTH/GDV/CHPTR17.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/CHPTR17.GDV")):
			return directory.path_join("DATA/GDV/CHPTR17.GDV")
		else:
			return ""
var chptr18_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/CHPTR18.GDV")):
			return directory.path_join("ROTH/GDV/CHPTR18.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/CHPTR18.GDV")):
			return directory.path_join("DATA/GDV/CHPTR18.GDV")
		else:
			return ""
var chptr19_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/CHPTR19.GDV")):
			return directory.path_join("ROTH/GDV/CHPTR19.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/CHPTR19.GDV")):
			return directory.path_join("DATA/GDV/CHPTR19.GDV")
		else:
			return ""
var chptr2_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/CHPTR2.GDV")):
			return directory.path_join("ROTH/GDV/CHPTR2.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/CHPTR2.GDV")):
			return directory.path_join("DATA/GDV/CHPTR2.GDV")
		else:
			return ""
var chptr20_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/CHPTR20.GDV")):
			return directory.path_join("ROTH/GDV/CHPTR20.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/CHPTR20.GDV")):
			return directory.path_join("DATA/GDV/CHPTR20.GDV")
		else:
			return ""
var chptr3_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/CHPTR3.GDV")):
			return directory.path_join("ROTH/GDV/CHPTR3.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/CHPTR3.GDV")):
			return directory.path_join("DATA/GDV/CHPTR3.GDV")
		else:
			return ""
var chptr4_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/CHPTR4.GDV")):
			return directory.path_join("ROTH/GDV/CHPTR4.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/CHPTR4.GDV")):
			return directory.path_join("DATA/GDV/CHPTR4.GDV")
		else:
			return ""
var chptr5_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/CHPTR5.GDV")):
			return directory.path_join("ROTH/GDV/CHPTR5.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/CHPTR5.GDV")):
			return directory.path_join("DATA/GDV/CHPTR5.GDV")
		else:
			return ""
var chptr6_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/CHPTR6.GDV")):
			return directory.path_join("ROTH/GDV/CHPTR6.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/CHPTR6.GDV")):
			return directory.path_join("DATA/GDV/CHPTR6.GDV")
		else:
			return ""
var chptr7_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/CHPTR7.GDV")):
			return directory.path_join("ROTH/GDV/CHPTR7.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/CHPTR7.GDV")):
			return directory.path_join("DATA/GDV/CHPTR7.GDV")
		else:
			return ""
var chptr8_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/CHPTR8.GDV")):
			return directory.path_join("ROTH/GDV/CHPTR8.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/CHPTR8.GDV")):
			return directory.path_join("DATA/GDV/CHPTR8.GDV")
		else:
			return ""
var chptr9_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/CHPTR9.GDV")):
			return directory.path_join("ROTH/GDV/CHPTR9.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/CHPTR9.GDV")):
			return directory.path_join("DATA/GDV/CHPTR9.GDV")
		else:
			return ""
var dodgera_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/DODGERA.GDV")):
			return directory.path_join("ROTH/GDV/DODGERA.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/DODGERA.GDV")):
			return directory.path_join("DATA/GDV/DODGERA.GDV")
		else:
			return ""
var dopple_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/DOPPLE.GDV")):
			return directory.path_join("ROTH/GDV/DOPPLE.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/DOPPLE.GDV")):
			return directory.path_join("DATA/GDV/DOPPLE.GDV")
		else:
			return ""
var ecredits_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/ECREDITS.GDV")):
			return directory.path_join("ROTH/GDV/ECREDITS.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/ECREDITS.GDV")):
			return directory.path_join("DATA/GDV/ECREDITS.GDV")
		else:
			return ""
var effigya1_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/EFFIGYA1.GDV")):
			return directory.path_join("ROTH/GDV/EFFIGYA1.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/EFFIGYA1.GDV")):
			return directory.path_join("DATA/GDV/EFFIGYA1.GDV")
		else:
			return ""
var effigya2_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/EFFIGYA2.GDV")):
			return directory.path_join("ROTH/GDV/EFFIGYA2.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/EFFIGYA2.GDV")):
			return directory.path_join("DATA/GDV/EFFIGYA2.GDV")
		else:
			return ""
var effigya3_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/EFFIGYA3.GDV")):
			return directory.path_join("ROTH/GDV/EFFIGYA3.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/EFFIGYA3.GDV")):
			return directory.path_join("DATA/GDV/EFFIGYA3.GDV")
		else:
			return ""
var effigyb_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/EFFIGYB.GDV")):
			return directory.path_join("ROTH/GDV/EFFIGYB.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/EFFIGYB.GDV")):
			return directory.path_join("DATA/GDV/EFFIGYB.GDV")
		else:
			return ""
var effigyc_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/EFFIGYC.GDV")):
			return directory.path_join("ROTH/GDV/EFFIGYC.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/EFFIGYC.GDV")):
			return directory.path_join("DATA/GDV/EFFIGYC.GDV")
		else:
			return ""
var enchant_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/ENCHANT.GDV")):
			return directory.path_join("ROTH/GDV/ENCHANT.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/ENCHANT.GDV")):
			return directory.path_join("DATA/GDV/ENCHANT.GDV")
		else:
			return ""
var epilogue_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/EPILOGUE.GDV")):
			return directory.path_join("ROTH/GDV/EPILOGUE.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/EPILOGUE.GDV")):
			return directory.path_join("DATA/GDV/EPILOGUE.GDV")
		else:
			return ""
var epilogue_mp4: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/EPILOGUE.mp4")):
			return directory.path_join("ROTH/GDV/EPILOGUE.mp4")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/EPILOGUE.mp4")):
			return directory.path_join("DATA/GDV/EPILOGUE.mp4")
		else:
			return ""
var falshire_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/FALSHIRE.GDV")):
			return directory.path_join("ROTH/GDV/FALSHIRE.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/FALSHIRE.GDV")):
			return directory.path_join("DATA/GDV/FALSHIRE.GDV")
		else:
			return ""
var falshire_mp4: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/FALSHIRE.mp4")):
			return directory.path_join("ROTH/GDV/FALSHIRE.mp4")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/FALSHIRE.mp4")):
			return directory.path_join("DATA/GDV/FALSHIRE.mp4")
		else:
			return ""
var flotrap1_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/FLOTRAP1.GDV")):
			return directory.path_join("ROTH/GDV/FLOTRAP1.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/FLOTRAP1.GDV")):
			return directory.path_join("DATA/GDV/FLOTRAP1.GDV")
		else:
			return ""
var flotrap2_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/FLOTRAP2.GDV")):
			return directory.path_join("ROTH/GDV/FLOTRAP2.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/FLOTRAP2.GDV")):
			return directory.path_join("DATA/GDV/FLOTRAP2.GDV")
		else:
			return ""
var flotrap3_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/FLOTRAP3.GDV")):
			return directory.path_join("ROTH/GDV/FLOTRAP3.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/FLOTRAP3.GDV")):
			return directory.path_join("DATA/GDV/FLOTRAP3.GDV")
		else:
			return ""
var flotrap4_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/FLOTRAP4.GDV")):
			return directory.path_join("ROTH/GDV/FLOTRAP4.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/FLOTRAP4.GDV")):
			return directory.path_join("DATA/GDV/FLOTRAP4.GDV")
		else:
			return ""
var flotrap5_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/FLOTRAP5.GDV")):
			return directory.path_join("ROTH/GDV/FLOTRAP5.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/FLOTRAP5.GDV")):
			return directory.path_join("DATA/GDV/FLOTRAP5.GDV")
		else:
			return ""
var flotrap6_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/FLOTRAP6.GDV")):
			return directory.path_join("ROTH/GDV/FLOTRAP6.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/FLOTRAP6.GDV")):
			return directory.path_join("DATA/GDV/FLOTRAP6.GDV")
		else:
			return ""
var fullkeep_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/FULLKEEP.GDV")):
			return directory.path_join("ROTH/GDV/FULLKEEP.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/FULLKEEP.GDV")):
			return directory.path_join("DATA/GDV/FULLKEEP.GDV")
		else:
			return ""
var gardens_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/GARDENS.GDV")):
			return directory.path_join("ROTH/GDV/GARDENS.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/GARDENS.GDV")):
			return directory.path_join("DATA/GDV/GARDENS.GDV")
		else:
			return ""
var gate01a_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/GATE01A.GDV")):
			return directory.path_join("ROTH/GDV/GATE01A.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/GATE01A.GDV")):
			return directory.path_join("DATA/GDV/GATE01A.GDV")
		else:
			return ""
var gate01b_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/GATE01B.GDV")):
			return directory.path_join("ROTH/GDV/GATE01B.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/GATE01B.GDV")):
			return directory.path_join("DATA/GDV/GATE01B.GDV")
		else:
			return ""
var gaul01_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/GAUL01.GDV")):
			return directory.path_join("ROTH/GDV/GAUL01.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/GAUL01.GDV")):
			return directory.path_join("DATA/GDV/GAUL01.GDV")
		else:
			return ""
var gaul02_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/GAUL02.GDV")):
			return directory.path_join("ROTH/GDV/GAUL02.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/GAUL02.GDV")):
			return directory.path_join("DATA/GDV/GAUL02.GDV")
		else:
			return ""
var glasswin_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/GLASSWIN.GDV")):
			return directory.path_join("ROTH/GDV/GLASSWIN.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/GLASSWIN.GDV")):
			return directory.path_join("DATA/GDV/GLASSWIN.GDV")
		else:
			return ""
var gnarla_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/GNARLA.GDV")):
			return directory.path_join("ROTH/GDV/GNARLA.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/GNARLA.GDV")):
			return directory.path_join("DATA/GDV/GNARLA.GDV")
		else:
			return ""
var gnarlb_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/GNARLB.GDV")):
			return directory.path_join("ROTH/GDV/GNARLB.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/GNARLB.GDV")):
			return directory.path_join("DATA/GDV/GNARLB.GDV")
		else:
			return ""
var gnarlc_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/GNARLC.GDV")):
			return directory.path_join("ROTH/GDV/GNARLC.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/GNARLC.GDV")):
			return directory.path_join("DATA/GDV/GNARLC.GDV")
		else:
			return ""
var gnarld_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/GNARLD.GDV")):
			return directory.path_join("ROTH/GDV/GNARLD.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/GNARLD.GDV")):
			return directory.path_join("DATA/GDV/GNARLD.GDV")
		else:
			return ""
var gnarle_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/GNARLE.GDV")):
			return directory.path_join("ROTH/GDV/GNARLE.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/GNARLE.GDV")):
			return directory.path_join("DATA/GDV/GNARLE.GDV")
		else:
			return ""
var gnarlf_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/GNARLF.GDV")):
			return directory.path_join("ROTH/GDV/GNARLF.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/GNARLF.GDV")):
			return directory.path_join("DATA/GDV/GNARLF.GDV")
		else:
			return ""
var gnarlg_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/GNARLG.GDV")):
			return directory.path_join("ROTH/GDV/GNARLG.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/GNARLG.GDV")):
			return directory.path_join("DATA/GDV/GNARLG.GDV")
		else:
			return ""
var gnarlh_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/GNARLH.GDV")):
			return directory.path_join("ROTH/GDV/GNARLH.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/GNARLH.GDV")):
			return directory.path_join("DATA/GDV/GNARLH.GDV")
		else:
			return ""
var gnarli_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/GNARLI.GDV")):
			return directory.path_join("ROTH/GDV/GNARLI.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/GNARLI.GDV")):
			return directory.path_join("DATA/GDV/GNARLI.GDV")
		else:
			return ""
var gremlogo_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/GREMLOGO.GDV")):
			return directory.path_join("ROTH/GDV/GREMLOGO.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/GREMLOGO.GDV")):
			return directory.path_join("DATA/GDV/GREMLOGO.GDV")
		else:
			return ""
var hawk01_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/HAWK01.GDV")):
			return directory.path_join("ROTH/GDV/HAWK01.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/HAWK01.GDV")):
			return directory.path_join("DATA/GDV/HAWK01.GDV")
		else:
			return ""
var hawk02a_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/HAWK02A.GDV")):
			return directory.path_join("ROTH/GDV/HAWK02A.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/HAWK02A.GDV")):
			return directory.path_join("DATA/GDV/HAWK02A.GDV")
		else:
			return ""
var hawk02b_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/HAWK02B.GDV")):
			return directory.path_join("ROTH/GDV/HAWK02B.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/HAWK02B.GDV")):
			return directory.path_join("DATA/GDV/HAWK02B.GDV")
		else:
			return ""
var hawk02c_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/HAWK02C.GDV")):
			return directory.path_join("ROTH/GDV/HAWK02C.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/HAWK02C.GDV")):
			return directory.path_join("DATA/GDV/HAWK02C.GDV")
		else:
			return ""
var hawk02d_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/HAWK02D.GDV")):
			return directory.path_join("ROTH/GDV/HAWK02D.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/HAWK02D.GDV")):
			return directory.path_join("DATA/GDV/HAWK02D.GDV")
		else:
			return ""
var hawk03a_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/HAWK03A.GDV")):
			return directory.path_join("ROTH/GDV/HAWK03A.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/HAWK03A.GDV")):
			return directory.path_join("DATA/GDV/HAWK03A.GDV")
		else:
			return ""
var hawk03b_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/HAWK03B.GDV")):
			return directory.path_join("ROTH/GDV/HAWK03B.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/HAWK03B.GDV")):
			return directory.path_join("DATA/GDV/HAWK03B.GDV")
		else:
			return ""
var hawk03c_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/HAWK03C.GDV")):
			return directory.path_join("ROTH/GDV/HAWK03C.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/HAWK03C.GDV")):
			return directory.path_join("DATA/GDV/HAWK03C.GDV")
		else:
			return ""
var hawk03d_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/HAWK03D.GDV")):
			return directory.path_join("ROTH/GDV/HAWK03D.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/HAWK03D.GDV")):
			return directory.path_join("DATA/GDV/HAWK03D.GDV")
		else:
			return ""
var hawk03e_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/HAWK03E.GDV")):
			return directory.path_join("ROTH/GDV/HAWK03E.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/HAWK03E.GDV")):
			return directory.path_join("DATA/GDV/HAWK03E.GDV")
		else:
			return ""
var hawk03f_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/HAWK03F.GDV")):
			return directory.path_join("ROTH/GDV/HAWK03F.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/HAWK03F.GDV")):
			return directory.path_join("DATA/GDV/HAWK03F.GDV")
		else:
			return ""
var hawk03g_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/HAWK03G.GDV")):
			return directory.path_join("ROTH/GDV/HAWK03G.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/HAWK03G.GDV")):
			return directory.path_join("DATA/GDV/HAWK03G.GDV")
		else:
			return ""
var hawk03h_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/HAWK03H.GDV")):
			return directory.path_join("ROTH/GDV/HAWK03H.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/HAWK03H.GDV")):
			return directory.path_join("DATA/GDV/HAWK03H.GDV")
		else:
			return ""
var hawk03i_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/HAWK03I.GDV")):
			return directory.path_join("ROTH/GDV/HAWK03I.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/HAWK03I.GDV")):
			return directory.path_join("DATA/GDV/HAWK03I.GDV")
		else:
			return ""
var hawk03j_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/HAWK03J.GDV")):
			return directory.path_join("ROTH/GDV/HAWK03J.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/HAWK03J.GDV")):
			return directory.path_join("DATA/GDV/HAWK03J.GDV")
		else:
			return ""
var hawk03k_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/HAWK03K.GDV")):
			return directory.path_join("ROTH/GDV/HAWK03K.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/HAWK03K.GDV")):
			return directory.path_join("DATA/GDV/HAWK03K.GDV")
		else:
			return ""
var hawk03l_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/HAWK03L.GDV")):
			return directory.path_join("ROTH/GDV/HAWK03L.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/HAWK03L.GDV")):
			return directory.path_join("DATA/GDV/HAWK03L.GDV")
		else:
			return ""
var hawk03m_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/HAWK03M.GDV")):
			return directory.path_join("ROTH/GDV/HAWK03M.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/HAWK03M.GDV")):
			return directory.path_join("DATA/GDV/HAWK03M.GDV")
		else:
			return ""
var hawk03n_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/HAWK03N.GDV")):
			return directory.path_join("ROTH/GDV/HAWK03N.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/HAWK03N.GDV")):
			return directory.path_join("DATA/GDV/HAWK03N.GDV")
		else:
			return ""
var hawk03o_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/HAWK03O.GDV")):
			return directory.path_join("ROTH/GDV/HAWK03O.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/HAWK03O.GDV")):
			return directory.path_join("DATA/GDV/HAWK03O.GDV")
		else:
			return ""
var hawk03p_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/HAWK03P.GDV")):
			return directory.path_join("ROTH/GDV/HAWK03P.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/HAWK03P.GDV")):
			return directory.path_join("DATA/GDV/HAWK03P.GDV")
		else:
			return ""
var hawk04_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/HAWK04.GDV")):
			return directory.path_join("ROTH/GDV/HAWK04.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/HAWK04.GDV")):
			return directory.path_join("DATA/GDV/HAWK04.GDV")
		else:
			return ""
var instra_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/INSTRA.GDV")):
			return directory.path_join("ROTH/GDV/INSTRA.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/INSTRA.GDV")):
			return directory.path_join("DATA/GDV/INSTRA.GDV")
		else:
			return ""
var instrb_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/INSTRB.GDV")):
			return directory.path_join("ROTH/GDV/INSTRB.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/INSTRB.GDV")):
			return directory.path_join("DATA/GDV/INSTRB.GDV")
		else:
			return ""
var instrc_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/INSTRC.GDV")):
			return directory.path_join("ROTH/GDV/INSTRC.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/INSTRC.GDV")):
			return directory.path_join("DATA/GDV/INSTRC.GDV")
		else:
			return ""
var instrd_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/INSTRD.GDV")):
			return directory.path_join("ROTH/GDV/INSTRD.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/INSTRD.GDV")):
			return directory.path_join("DATA/GDV/INSTRD.GDV")
		else:
			return ""
var instre_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/INSTRE.GDV")):
			return directory.path_join("ROTH/GDV/INSTRE.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/INSTRE.GDV")):
			return directory.path_join("DATA/GDV/INSTRE.GDV")
		else:
			return ""
var instrf_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/INSTRF.GDV")):
			return directory.path_join("ROTH/GDV/INSTRF.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/INSTRF.GDV")):
			return directory.path_join("DATA/GDV/INSTRF.GDV")
		else:
			return ""
var instrg_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/INSTRG.GDV")):
			return directory.path_join("ROTH/GDV/INSTRG.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/INSTRG.GDV")):
			return directory.path_join("DATA/GDV/INSTRG.GDV")
		else:
			return ""
var instrh_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/INSTRH.GDV")):
			return directory.path_join("ROTH/GDV/INSTRH.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/INSTRH.GDV")):
			return directory.path_join("DATA/GDV/INSTRH.GDV")
		else:
			return ""
var instri_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/INSTRI.GDV")):
			return directory.path_join("ROTH/GDV/INSTRI.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/INSTRI.GDV")):
			return directory.path_join("DATA/GDV/INSTRI.GDV")
		else:
			return ""
var intro_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/INTRO.GDV")):
			return directory.path_join("ROTH/GDV/INTRO.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/INTRO.GDV")):
			return directory.path_join("DATA/GDV/INTRO.GDV")
		else:
			return ""
var movie_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/MOVIE.GDV")):
			return directory.path_join("ROTH/GDV/MOVIE.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/MOVIE.GDV")):
			return directory.path_join("DATA/GDV/MOVIE.GDV")
		else:
			return ""
var randall_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/RANDALL.GDV")):
			return directory.path_join("ROTH/GDV/RANDALL.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/RANDALL.GDV")):
			return directory.path_join("DATA/GDV/RANDALL.GDV")
		else:
			return ""
var raphael_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/RAPHAEL.GDV")):
			return directory.path_join("ROTH/GDV/RAPHAEL.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/RAPHAEL.GDV")):
			return directory.path_join("DATA/GDV/RAPHAEL.GDV")
		else:
			return ""
var raphaelh_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/RAPHAELH.GDV")):
			return directory.path_join("ROTH/GDV/RAPHAELH.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/RAPHAELH.GDV")):
			return directory.path_join("DATA/GDV/RAPHAELH.GDV")
		else:
			return ""
var raphaeli_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/RAPHAELI.GDV")):
			return directory.path_join("ROTH/GDV/RAPHAELI.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/RAPHAELI.GDV")):
			return directory.path_join("DATA/GDV/RAPHAELI.GDV")
		else:
			return ""
var raphaelj_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/RAPHAELJ.GDV")):
			return directory.path_join("ROTH/GDV/RAPHAELJ.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/RAPHAELJ.GDV")):
			return directory.path_join("DATA/GDV/RAPHAELJ.GDV")
		else:
			return ""
var raphaelk_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/RAPHAELK.GDV")):
			return directory.path_join("ROTH/GDV/RAPHAELK.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/RAPHAELK.GDV")):
			return directory.path_join("DATA/GDV/RAPHAELK.GDV")
		else:
			return ""
var raphaell_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/RAPHAELL.GDV")):
			return directory.path_join("ROTH/GDV/RAPHAELL.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/RAPHAELL.GDV")):
			return directory.path_join("DATA/GDV/RAPHAELL.GDV")
		else:
			return ""
var raphaelm_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/RAPHAELM.GDV")):
			return directory.path_join("ROTH/GDV/RAPHAELM.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/RAPHAELM.GDV")):
			return directory.path_join("DATA/GDV/RAPHAELM.GDV")
		else:
			return ""
var raphaeln_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/RAPHAELN.GDV")):
			return directory.path_join("ROTH/GDV/RAPHAELN.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/RAPHAELN.GDV")):
			return directory.path_join("DATA/GDV/RAPHAELN.GDV")
		else:
			return ""
var raphaelo_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/RAPHAELO.GDV")):
			return directory.path_join("ROTH/GDV/RAPHAELO.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/RAPHAELO.GDV")):
			return directory.path_join("DATA/GDV/RAPHAELO.GDV")
		else:
			return ""
var ratpfa_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/RATPFA.GDV")):
			return directory.path_join("ROTH/GDV/RATPFA.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/RATPFA.GDV")):
			return directory.path_join("DATA/GDV/RATPFA.GDV")
		else:
			return ""
var ratpfb_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/RATPFB.GDV")):
			return directory.path_join("ROTH/GDV/RATPFB.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/RATPFB.GDV")):
			return directory.path_join("DATA/GDV/RATPFB.GDV")
		else:
			return ""
var raysel1a_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/RAYSEL1A.GDV")):
			return directory.path_join("ROTH/GDV/RAYSEL1A.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/RAYSEL1A.GDV")):
			return directory.path_join("DATA/GDV/RAYSEL1A.GDV")
		else:
			return ""
var raysel1b_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/RAYSEL1B.GDV")):
			return directory.path_join("ROTH/GDV/RAYSEL1B.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/RAYSEL1B.GDV")):
			return directory.path_join("DATA/GDV/RAYSEL1B.GDV")
		else:
			return ""
var raysel1c_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/RAYSEL1C.GDV")):
			return directory.path_join("ROTH/GDV/RAYSEL1C.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/RAYSEL1C.GDV")):
			return directory.path_join("DATA/GDV/RAYSEL1C.GDV")
		else:
			return ""
var raysel1d_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/RAYSEL1D.GDV")):
			return directory.path_join("ROTH/GDV/RAYSEL1D.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/RAYSEL1D.GDV")):
			return directory.path_join("DATA/GDV/RAYSEL1D.GDV")
		else:
			return ""
var raysel1e_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/RAYSEL1E.GDV")):
			return directory.path_join("ROTH/GDV/RAYSEL1E.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/RAYSEL1E.GDV")):
			return directory.path_join("DATA/GDV/RAYSEL1E.GDV")
		else:
			return ""
var raysel1f_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/RAYSEL1F.GDV")):
			return directory.path_join("ROTH/GDV/RAYSEL1F.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/RAYSEL1F.GDV")):
			return directory.path_join("DATA/GDV/RAYSEL1F.GDV")
		else:
			return ""
var raysel1g_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/RAYSEL1G.GDV")):
			return directory.path_join("ROTH/GDV/RAYSEL1G.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/RAYSEL1G.GDV")):
			return directory.path_join("DATA/GDV/RAYSEL1G.GDV")
		else:
			return ""
var rebecca_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/REBECCA.GDV")):
			return directory.path_join("ROTH/GDV/REBECCA.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/REBECCA.GDV")):
			return directory.path_join("DATA/GDV/REBECCA.GDV")
		else:
			return ""
var reflecta_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/REFLECTA.GDV")):
			return directory.path_join("ROTH/GDV/REFLECTA.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/REFLECTA.GDV")):
			return directory.path_join("DATA/GDV/REFLECTA.GDV")
		else:
			return ""
var reflectb_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/REFLECTB.GDV")):
			return directory.path_join("ROTH/GDV/REFLECTB.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/REFLECTB.GDV")):
			return directory.path_join("DATA/GDV/REFLECTB.GDV")
		else:
			return ""
var reflectc_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/REFLECTC.GDV")):
			return directory.path_join("ROTH/GDV/REFLECTC.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/REFLECTC.GDV")):
			return directory.path_join("DATA/GDV/REFLECTC.GDV")
		else:
			return ""
var ritual_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/RITUAL.GDV")):
			return directory.path_join("ROTH/GDV/RITUAL.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/RITUAL.GDV")):
			return directory.path_join("DATA/GDV/RITUAL.GDV")
		else:
			return ""
var shrivea_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/SHRIVEA.GDV")):
			return directory.path_join("ROTH/GDV/SHRIVEA.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/SHRIVEA.GDV")):
			return directory.path_join("DATA/GDV/SHRIVEA.GDV")
		else:
			return ""
var shriveb_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/SHRIVEB.GDV")):
			return directory.path_join("ROTH/GDV/SHRIVEB.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/SHRIVEB.GDV")):
			return directory.path_join("DATA/GDV/SHRIVEB.GDV")
		else:
			return ""
var shrivec_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/SHRIVEC.GDV")):
			return directory.path_join("ROTH/GDV/SHRIVEC.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/SHRIVEC.GDV")):
			return directory.path_join("DATA/GDV/SHRIVEC.GDV")
		else:
			return ""
var shrived_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/SHRIVED.GDV")):
			return directory.path_join("ROTH/GDV/SHRIVED.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/SHRIVED.GDV")):
			return directory.path_join("DATA/GDV/SHRIVED.GDV")
		else:
			return ""
var talewela_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/TALEWELA.GDV")):
			return directory.path_join("ROTH/GDV/TALEWELA.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/TALEWELA.GDV")):
			return directory.path_join("DATA/GDV/TALEWELA.GDV")
		else:
			return ""
var talewelb_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/TALEWELB.GDV")):
			return directory.path_join("ROTH/GDV/TALEWELB.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/TALEWELB.GDV")):
			return directory.path_join("DATA/GDV/TALEWELB.GDV")
		else:
			return ""
var talewelc_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/TALEWELC.GDV")):
			return directory.path_join("ROTH/GDV/TALEWELC.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/TALEWELC.GDV")):
			return directory.path_join("DATA/GDV/TALEWELC.GDV")
		else:
			return ""
var tunnwarp_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/TUNNWARP.GDV")):
			return directory.path_join("ROTH/GDV/TUNNWARP.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/TUNNWARP.GDV")):
			return directory.path_join("DATA/GDV/TUNNWARP.GDV")
		else:
			return ""
var verse_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/VERSE.GDV")):
			return directory.path_join("ROTH/GDV/VERSE.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/VERSE.GDV")):
			return directory.path_join("DATA/GDV/VERSE.GDV")
		else:
			return ""
var walkway1_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/WALKWAY1.GDV")):
			return directory.path_join("ROTH/GDV/WALKWAY1.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/WALKWAY1.GDV")):
			return directory.path_join("DATA/GDV/WALKWAY1.GDV")
		else:
			return ""
var walkway2_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/WALKWAY2.GDV")):
			return directory.path_join("ROTH/GDV/WALKWAY2.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/WALKWAY2.GDV")):
			return directory.path_join("DATA/GDV/WALKWAY2.GDV")
		else:
			return ""
var walkway3_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/WALKWAY3.GDV")):
			return directory.path_join("ROTH/GDV/WALKWAY3.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/WALKWAY3.GDV")):
			return directory.path_join("DATA/GDV/WALKWAY3.GDV")
		else:
			return ""
var walkwy1a_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/WALKWY1A.GDV")):
			return directory.path_join("ROTH/GDV/WALKWY1A.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/WALKWY1A.GDV")):
			return directory.path_join("DATA/GDV/WALKWY1A.GDV")
var iplogo_gdv: String :
	get():
		if FileAccess.file_exists(directory.path_join("ROTH/GDV/IPLOGO.GDV")):
			return directory.path_join("ROTH/GDV/IPLOGO.GDV")
		elif FileAccess.file_exists(directory.path_join("DATA/GDV/IPLOGO.GDV")):
			return directory.path_join("DATA/GDV/IPLOGO.GDV")
		elif FileAccess.file_exists(directory.path_join("GDV/IPLOGO.GDV")):
			return directory.path_join("GDV/IPLOGO.GDV")
		else:
			return ""
var name: String :
	get():
		if is_valid():
			return language
		return "Invalid Installation"
var id: String :
	get():
		return directory.md5_text()
var save_directory: String :
	get():
		if DirAccess.dir_exists_absolute(directory.path_join("ROTH/SAVEGAME")):
			return directory.path_join("ROTH/SAVEGAME")
		elif DirAccess.dir_exists_absolute(directory.path_join("SAVEGAME")):
			return directory.path_join("SAVEGAME")
		else:
			return ""
var gdv_directory: String :
	get():
		if DirAccess.dir_exists_absolute(directory.path_join("ROTH/GDV")):
			return directory.path_join("ROTH/GDV")
		elif DirAccess.dir_exists_absolute(directory.path_join("DATA/GDV")):
			return directory.path_join("DATA/GDV")
		elif DirAccess.dir_exists_absolute(directory.path_join("GDV")):
			return directory.path_join("GDV")
		else:
			return ""
var dosbox_exe: String :
	get():
		if FileAccess.file_exists(directory.path_join("DOSBOX/DOSBox.exe")):
			return directory.path_join("DOSBOX/DOSBox.exe")
		elif FileAccess.file_exists(directory.path_join("DOSBox.exe")):
			return directory.path_join("DOSBox.exe")
		else:
			return ""
var dosbox_config: String :
	get():
		if FileAccess.file_exists(directory.path_join("dosboxROTH.conf")):
			return directory.path_join("dosboxROTH.conf")
		else:
			return ""

#endregion

#region Write Files
static func write_dosbox_mapper_file(dosbox_mapper_filepath: String, dosbox_keymap: Dictionary) -> void:
	if not DirAccess.dir_exists_absolute(dosbox_mapper_filepath.get_base_dir()):
		DirAccess.make_dir_recursive_absolute(dosbox_mapper_filepath.get_base_dir())
	
	var dosbox_mapper_file := FileAccess.open(dosbox_mapper_filepath, FileAccess.WRITE)
	dosbox_mapper_file.store_string("hand_shutdown\n")
	dosbox_mapper_file.store_string("hand_capmouse %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("capture_mouse", [])))
	dosbox_mapper_file.store_string("hand_fullscr %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("toggle_fullscreen", [])))
	dosbox_mapper_file.store_string("hand_mapper\n")
	dosbox_mapper_file.store_string("hand_speedlock %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("fast_forward", [])))
	dosbox_mapper_file.store_string("hand_recwave\n")
	dosbox_mapper_file.store_string("hand_caprawmidi\n")
	dosbox_mapper_file.store_string("hand_scrshot\n")
	dosbox_mapper_file.store_string("hand_video\n")
	dosbox_mapper_file.store_string("hand_decfskip\n")
	dosbox_mapper_file.store_string("hand_incfskip\n")
	dosbox_mapper_file.store_string("hand_cycledown %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("cycles_down", [])))
	dosbox_mapper_file.store_string("hand_cycleup %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("cycles_up", [])))
	dosbox_mapper_file.store_string("hand_debugger %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("debugger", [])))
	dosbox_mapper_file.store_string("hand_caprawopl\n")
	dosbox_mapper_file.store_string("hand_swapimg\n")
	dosbox_mapper_file.store_string("key_esc %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("escape", [])))
	dosbox_mapper_file.store_string("key_f1 %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("hide_weapon", [])))
	dosbox_mapper_file.store_string("key_f2 %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("toggle_subtitles", [])))
	dosbox_mapper_file.store_string("key_f3 %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("toggle_adventure_mode", [])))
	dosbox_mapper_file.store_string("key_f4\n")
	dosbox_mapper_file.store_string("key_f5 %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("toggle_mouse_buttons", [])))
	dosbox_mapper_file.store_string("key_f6\n")
	dosbox_mapper_file.store_string("key_f7\n")
	dosbox_mapper_file.store_string("key_f8 %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("display_version", [])))
	dosbox_mapper_file.store_string("key_f9 %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("quicksave", [])))
	dosbox_mapper_file.store_string("key_f10 %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("quickload", [])))
	dosbox_mapper_file.store_string("key_f11\n")
	dosbox_mapper_file.store_string("key_f12\n")
	dosbox_mapper_file.store_string("key_grave\n")
	dosbox_mapper_file.store_string("key_1 %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("unarmed", [])))
	dosbox_mapper_file.store_string("key_2 %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("quickslot_1", [])))
	dosbox_mapper_file.store_string("key_3 %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("quickslot_2", [])))
	dosbox_mapper_file.store_string("key_4 %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("quickslot_3", [])))
	dosbox_mapper_file.store_string("key_5 %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("quickslot_4", [])))
	dosbox_mapper_file.store_string("key_6 %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("quickslot_5", [])))
	dosbox_mapper_file.store_string("key_7\n")
	dosbox_mapper_file.store_string("key_8\n")
	dosbox_mapper_file.store_string("key_9\n")
	dosbox_mapper_file.store_string("key_0\n")
	dosbox_mapper_file.store_string("key_minus %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("decrease_viewport", [])))
	dosbox_mapper_file.store_string("key_equals %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("increase_viewport", [])))
	dosbox_mapper_file.store_string("key_bspace\n")
	dosbox_mapper_file.store_string("key_tab\n")
	dosbox_mapper_file.store_string("key_q\n")
	dosbox_mapper_file.store_string("key_w %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("map_warp", [])))
	dosbox_mapper_file.store_string("key_e\n")
	dosbox_mapper_file.store_string("key_r\n")
	dosbox_mapper_file.store_string("key_t %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("redraw_screen", [])))
	dosbox_mapper_file.store_string("key_y\n")
	dosbox_mapper_file.store_string("key_u\n")
	dosbox_mapper_file.store_string("key_i %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("inventory", [])))
	dosbox_mapper_file.store_string("key_o\n")
	dosbox_mapper_file.store_string("key_p\n")
	dosbox_mapper_file.store_string("key_lbracket\n")
	dosbox_mapper_file.store_string("key_rbracket\n")
	dosbox_mapper_file.store_string("key_enter %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("enter", [])))
	dosbox_mapper_file.store_string("key_capslock %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("toggle_run", [])))
	dosbox_mapper_file.store_string("key_a %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("jump", [])))
	dosbox_mapper_file.store_string("key_s\n")
	dosbox_mapper_file.store_string("key_d %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("main_menu", [])))
	dosbox_mapper_file.store_string("key_f\n")
	dosbox_mapper_file.store_string("key_g\n")
	dosbox_mapper_file.store_string("key_h\n")
	dosbox_mapper_file.store_string("key_j\n")
	dosbox_mapper_file.store_string("key_k\n")
	dosbox_mapper_file.store_string("key_l %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("map_overlay", [])))
	dosbox_mapper_file.store_string("key_semicolon\n")
	dosbox_mapper_file.store_string("key_quote\n")
	dosbox_mapper_file.store_string("key_backslash\n")
	dosbox_mapper_file.store_string("key_lshift %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("hold_run", [])))
	dosbox_mapper_file.store_string("key_lessthan\n")
	dosbox_mapper_file.store_string("key_z %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("crouch", [])))
	dosbox_mapper_file.store_string("key_x\n")
	dosbox_mapper_file.store_string("key_c %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("increase_gamma", [])))
	dosbox_mapper_file.store_string("key_v %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("decrease_gamma", [])))
	dosbox_mapper_file.store_string("key_b\n")
	dosbox_mapper_file.store_string("key_n\n")
	dosbox_mapper_file.store_string("key_m %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("toggle_textures", [])))
	dosbox_mapper_file.store_string("key_comma %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("strafe_left", [])))
	dosbox_mapper_file.store_string("key_period %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("strafe_right", [])))
	dosbox_mapper_file.store_string("key_slash\n")
	dosbox_mapper_file.store_string("key_rshift\n")
	dosbox_mapper_file.store_string("key_lctrl %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("attack", [])))
	dosbox_mapper_file.store_string("key_lalt\n")
	dosbox_mapper_file.store_string("key_space %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("reload_after_death", [])))
	dosbox_mapper_file.store_string("key_ralt\n")
	dosbox_mapper_file.store_string("key_rctrl\n")
	dosbox_mapper_file.store_string("key_printscreen\n")
	dosbox_mapper_file.store_string("key_scrolllock\n")
	dosbox_mapper_file.store_string("key_pause\n")
	dosbox_mapper_file.store_string("key_insert\n")
	dosbox_mapper_file.store_string("key_home %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("tilt_up", [])))
	dosbox_mapper_file.store_string("key_pageup %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("look_up", [])))
	dosbox_mapper_file.store_string("key_delete\n")
	dosbox_mapper_file.store_string("key_end %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("tilt_down", [])))
	dosbox_mapper_file.store_string("key_pagedown %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("look_down", [])))
	dosbox_mapper_file.store_string("key_up %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("move_forward", [])))
	dosbox_mapper_file.store_string("key_left %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("turn_left", [])))
	dosbox_mapper_file.store_string("key_down %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("move_backward", [])))
	dosbox_mapper_file.store_string("key_right %s\n" % SDL.get_sdl_keycode_string(dosbox_keymap.get("turn_right", [])))
	dosbox_mapper_file.store_string("key_numlock\n")
	dosbox_mapper_file.store_string("key_kp_divide\n")
	dosbox_mapper_file.store_string("key_kp_multiply\n")
	dosbox_mapper_file.store_string("key_kp_minus\n")
	dosbox_mapper_file.store_string("key_kp_7\n")
	dosbox_mapper_file.store_string("key_kp_8\n")
	dosbox_mapper_file.store_string("key_kp_9\n")
	dosbox_mapper_file.store_string("key_kp_plus\n")
	dosbox_mapper_file.store_string("key_kp_4\n")
	dosbox_mapper_file.store_string("key_kp_5\n")
	dosbox_mapper_file.store_string("key_kp_6\n")
	dosbox_mapper_file.store_string("key_kp_1\n")
	dosbox_mapper_file.store_string("key_kp_2\n")
	dosbox_mapper_file.store_string("key_kp_3\n")
	dosbox_mapper_file.store_string("key_kp_enter\n")
	dosbox_mapper_file.store_string("key_kp_0\n")
	dosbox_mapper_file.store_string("key_kp_period\n")
	dosbox_mapper_file.store_string("jbutton_0_0\n")
	dosbox_mapper_file.store_string("jbutton_0_1\n")
	dosbox_mapper_file.store_string("jaxis_0_1-\n")
	dosbox_mapper_file.store_string("jaxis_0_1+\n")
	dosbox_mapper_file.store_string("jaxis_0_0-\n")
	dosbox_mapper_file.store_string("jaxis_0_0+\n")
	dosbox_mapper_file.store_string("jbutton_0_2\n")
	dosbox_mapper_file.store_string("jbutton_0_3\n")
	dosbox_mapper_file.store_string("jbutton_1_0\n")
	dosbox_mapper_file.store_string("jbutton_1_1\n")
	dosbox_mapper_file.store_string("jaxis_0_2-\n")
	dosbox_mapper_file.store_string("jaxis_0_2+\n")
	dosbox_mapper_file.store_string("jaxis_0_3-\n")
	dosbox_mapper_file.store_string("jaxis_0_3+\n")
	dosbox_mapper_file.store_string("jaxis_1_0-\n")
	dosbox_mapper_file.store_string("jaxis_1_0+\n")
	dosbox_mapper_file.store_string("jaxis_1_1-\n")
	dosbox_mapper_file.store_string("jaxis_1_1+\n")
	dosbox_mapper_file.store_string("jbutton_0_4\n")
	dosbox_mapper_file.store_string("jbutton_0_5\n")
	dosbox_mapper_file.store_string("jhat_0_0_0\n")
	dosbox_mapper_file.store_string("jhat_0_0_3\n")
	dosbox_mapper_file.store_string("jhat_0_0_2\n")
	dosbox_mapper_file.store_string("jhat_0_0_1\n")
	dosbox_mapper_file.store_string("mod_1\n")
	dosbox_mapper_file.store_string("mod_2\n")
	dosbox_mapper_file.store_string("mod_3\n")
	dosbox_mapper_file.close()


static func write_dosbox_config_file(dosbox_config_filepath: String, dosbox_settings: Dictionary) -> void:
	if not DirAccess.dir_exists_absolute(dosbox_config_filepath.get_base_dir()):
		DirAccess.make_dir_recursive_absolute(dosbox_config_filepath.get_base_dir())
	
	var dosbox_settings_file := FileAccess.open(dosbox_config_filepath, FileAccess.WRITE)
	dosbox_settings_file.store_string("[sdl]\n")
	dosbox_settings_file.store_string("fullscreen=%s\n" % dosbox_settings.get("fullscreen", false))
	dosbox_settings_file.store_string("fulldouble=%s\n" % dosbox_settings.get("fulldouble", true))
	dosbox_settings_file.store_string("fullresolution=%s\n" % dosbox_settings.get("fullresolution", "desktop"))
	dosbox_settings_file.store_string("windowresolution=%s\n" % dosbox_settings.get("windowresolution", "1280x960"))
	dosbox_settings_file.store_string("output=%s\n" % dosbox_settings.get("output", "opengl"))
	dosbox_settings_file.store_string("autolock=%s\n" % dosbox_settings.get("autolock", true))
	dosbox_settings_file.store_string("sensitivity=100\n")
	dosbox_settings_file.store_string("waitonerror=true\n")
	dosbox_settings_file.store_string("priority=higher,normal\n")
	dosbox_settings_file.store_string("mapperfile=%s\n" % dosbox_config_filepath.get_base_dir().path_join("mapper.txt"))
	dosbox_settings_file.store_string("usescancodes=true\n")
	dosbox_settings_file.store_string("[dosbox]\n")
	dosbox_settings_file.store_string("language=\n")
	dosbox_settings_file.store_string("machine=svga_s3\n")
	dosbox_settings_file.store_string("captures=capture\n")
	dosbox_settings_file.store_string("memsize=16\n")
	dosbox_settings_file.store_string("[render]\n")
	dosbox_settings_file.store_string("frameskip=0\n")
	dosbox_settings_file.store_string("aspect=false\n")
	dosbox_settings_file.store_string("scaler=%s\n" % dosbox_settings.get("scaler", "normal2x"))
	dosbox_settings_file.store_string("[cpu]\n")
	dosbox_settings_file.store_string("core=auto\n")
	dosbox_settings_file.store_string("cputype=auto\n")
	dosbox_settings_file.store_string("cycles=%s\n" % dosbox_settings.get("cycles", "auto"))
	dosbox_settings_file.store_string("cycleup=10000\n")
	dosbox_settings_file.store_string("cycledown=10000\n")
	dosbox_settings_file.store_string("[mixer]\n")
	dosbox_settings_file.store_string("nosound=false\n")
	dosbox_settings_file.store_string("rate=22050\n")
	dosbox_settings_file.store_string("blocksize=2048\n")
	dosbox_settings_file.store_string("prebuffer=80\n")
	dosbox_settings_file.store_string("[midi]\n")
	dosbox_settings_file.store_string("mpu401=intelligent\n")
	dosbox_settings_file.store_string("device=default\n")
	dosbox_settings_file.store_string("config=\n")
	dosbox_settings_file.store_string("[sblaster]\n")
	dosbox_settings_file.store_string("sbtype=sb16\n")
	dosbox_settings_file.store_string("sbbase=220\n")
	dosbox_settings_file.store_string("irq=7\n")
	dosbox_settings_file.store_string("dma=1\n")
	dosbox_settings_file.store_string("hdma=5\n")
	dosbox_settings_file.store_string("mixer=true\n")
	dosbox_settings_file.store_string("oplmode=auto\n")
	dosbox_settings_file.store_string("oplrate=22050\n")
	dosbox_settings_file.store_string("[gus]\n")
	dosbox_settings_file.store_string("gus=false\n")
	dosbox_settings_file.store_string("[speaker]\n")
	dosbox_settings_file.store_string("pcspeaker=false\n")
	dosbox_settings_file.store_string("pcrate=22050\n")
	dosbox_settings_file.store_string("tandy=auto\n")
	dosbox_settings_file.store_string("tandyrate=22050\n")
	dosbox_settings_file.store_string("disney=true\n")
	dosbox_settings_file.store_string("[joystick]\n")
	dosbox_settings_file.store_string("joysticktype=none\n")
	dosbox_settings_file.store_string("timed=true\n")
	dosbox_settings_file.store_string("autofire=false\n")
	dosbox_settings_file.store_string("swap34=false\n")
	dosbox_settings_file.store_string("buttonwrap=true\n")
	dosbox_settings_file.store_string("[serial]\n")
	dosbox_settings_file.store_string("serial1=dummy\n")
	dosbox_settings_file.store_string("serial2=dummy\n")
	dosbox_settings_file.store_string("serial3=disabled\n")
	dosbox_settings_file.store_string("serial4=disabled\n")
	dosbox_settings_file.store_string("[dos]\n")
	dosbox_settings_file.store_string("xms=true\n")
	dosbox_settings_file.store_string("ems=true\n")
	dosbox_settings_file.store_string("umb=true\n")
	dosbox_settings_file.store_string("keyboardlayout=none\n")
	dosbox_settings_file.close()


static func write_config_ini(config_ini_filepath: String, original_game: bool = false) -> void:
	if not DirAccess.dir_exists_absolute(config_ini_filepath.get_base_dir()):
		DirAccess.make_dir_recursive_absolute(config_ini_filepath.get_base_dir())
	var config_ini_file := FileAccess.open(config_ini_filepath, FileAccess.WRITE)
	if original_game:
		config_ini_file.store_string("SourcePath=C:\\DATA\n")
		config_ini_file.store_string("DestinationPath=C:\\ROTH\n")
	config_ini_file.store_string("SoundCard=0xe018\n")
	config_ini_file.store_string("SoundPort=0x220\n")
	config_ini_file.store_string("SoundIRQ=7\n")
	config_ini_file.store_string("SoundDMA=5\n")
	config_ini_file.store_string("MusicCard=0xa009\n")
	config_ini_file.store_string("MusicPort=0x388\n")
	config_ini_file.close()


static func write_roth_ini(roth_ini_filepath: String) -> void:
	if not DirAccess.dir_exists_absolute(roth_ini_filepath.get_base_dir()):
		DirAccess.make_dir_recursive_absolute(roth_ini_filepath.get_base_dir())
	var roth_ini_file := FileAccess.open(roth_ini_filepath, FileAccess.WRITE)
	roth_ini_file.store_string("SpeechSub=ON\n")
	roth_ini_file.store_string("SpeechAud=ON\n")
	roth_ini_file.store_string("MovieSub=ON\n")
	roth_ini_file.store_string("MovieAud=ON\n")
	roth_ini_file.store_string("VideoMode=8\n")
	roth_ini_file.store_string("ViewSize=0\n")
	roth_ini_file.store_string("SoundFXVol=0x100\n")
	roth_ini_file.store_string("SpeechVol=0xd0\n")
	roth_ini_file.store_string("MovieVol=0x100\n")
	roth_ini_file.store_string("MusicVol=0x100\n")
	roth_ini_file.store_string("MouseSpeed=0x40\n")
	roth_ini_file.close()
#endregion

func _init(p_directory: String) -> void:
	directory = p_directory


func _to_string() -> String:
	return JSON.stringify({
		directory = directory,
		roth_exe = roth_exe,
		roth_exe_version = roth_exe_version,
		roth_res = roth_res,
		roth_res_region = roth_res_region,
		roth_ini = roth_ini,
		config_ini = config_ini,
		dbase100 = dbase100,
		dbase200 = dbase200,
		dbase300 = dbase300,
		dbase400 = dbase400,
		dbase500 = dbase500,
		dos4gw = dos4gw,
		backdrop = backdrop,
		fxscript = fxscript,
		icons = icons,
		hmidet_386 = hmidet_386,
		hmidrv_386 = hmidrv_386,
		hmimdrv_386 = hmimdrv_386,
		drum = drum,
		melodic = melodic,
		ademo = ademo,
		demo = demo,
		demo1 = demo1,
		demo2 = demo2,
		demo3 = demo3,
		demo4 = demo4,
		abagate2 = abagate2,
		aelf = aelf,
		anubis = anubis,
		aqua1 = aqua1,
		aqua2 = aqua2,
		caverns = caverns,
		caverns2 = caverns2,
		caverns3 = caverns3,
		church1 = church1,
		dominion = dominion,
		dopple = dopple,
		elohim1 = elohim1,
		gnarl1 = gnarl1,
		grave = grave,
		lrinth = lrinth,
		lrinth1 = lrinth1,
		mas3 = mas3,
		mas4 = mas4,
		mas6 = mas6,
		mas7 = mas7,
		mauso1ea = mauso1ea,
		mauso1eb = mauso1eb,
		maze = maze,
		optemp1 = optemp1,
		raquia1 = raquia1,
		raquia2 = raquia2,
		raquia3 = raquia3,
		raquia4 = raquia4,
		raquia5 = raquia5,
		salvat = salvat,
		soulst2 = soulst2,
		soulst3 = soulst3,
		study1 = study1,
		study2 = study2,
		study3 = study3,
		study4 = study4,
		temple1 = temple1,
		tgate1f = tgate1f,
		tgate1g = tgate1g,
		tgate1h = tgate1h,
		tgate1i = tgate1i,
		vicar = vicar,
		vicar1 = vicar1,
		abadon1a_gdv = abadon1a_gdv,
		abadon1b_gdv = abadon1b_gdv,
		abadon1c_gdv = abadon1c_gdv,
		abadon1d_gdv = abadon1d_gdv,
		abadon1e_gdv = abadon1e_gdv,
		abadon1f_gdv = abadon1f_gdv,
		abadon1g_gdv = abadon1g_gdv,
		abadon1h_gdv = abadon1h_gdv,
		abadon2a_gdv = abadon2a_gdv,
		abadon2b_gdv = abadon2b_gdv,
		abadon2c_gdv = abadon2c_gdv,
		abadon2d_gdv = abadon2d_gdv,
		abadon2e_gdv = abadon2e_gdv,
		abadon2f_gdv = abadon2f_gdv,
		abadon2g_gdv = abadon2g_gdv,
		abadon2h_gdv = abadon2h_gdv,
		abadon2i_gdv = abadon2i_gdv,
		alef01_gdv = alef01_gdv,
		alef02_gdv = alef02_gdv,
		alef03a_gdv = alef03a_gdv,
		alef03b_gdv = alef03b_gdv,
		alef03c_gdv = alef03c_gdv,
		alef03d_gdv = alef03d_gdv,
		alef04_gdv = alef04_gdv,
		beast_gdv = beast_gdv,
		belextra_gdv = belextra_gdv,
		belial1a_gdv = belial1a_gdv,
		belial1b_gdv = belial1b_gdv,
		belial1c_gdv = belial1c_gdv,
		belial1e_gdv = belial1e_gdv,
		belial1f_gdv = belial1f_gdv,
		belsb1_gdv = belsb1_gdv,
		belsb3_gdv = belsb3_gdv,
		belsb4_gdv = belsb4_gdv,
		belsb5_gdv = belsb5_gdv,
		belsb6_gdv = belsb6_gdv,
		bendgma_gdv = bendgma_gdv,
		bendgmb_gdv = bendgmb_gdv,
		betrayal_gdv = betrayal_gdv,
		branded_gdv = branded_gdv,
		chptr1_gdv = chptr1_gdv,
		chptr10_gdv = chptr10_gdv,
		chptr11_gdv = chptr11_gdv,
		chptr12_gdv = chptr12_gdv,
		chptr13_gdv = chptr13_gdv,
		chptr14_gdv = chptr14_gdv,
		chptr15_gdv = chptr15_gdv,
		chptr16_gdv = chptr16_gdv,
		chptr17_gdv = chptr17_gdv,
		chptr18_gdv = chptr18_gdv,
		chptr19_gdv = chptr19_gdv,
		chptr2_gdv = chptr2_gdv,
		chptr20_gdv = chptr20_gdv,
		chptr3_gdv = chptr3_gdv,
		chptr4_gdv = chptr4_gdv,
		chptr5_gdv = chptr5_gdv,
		chptr6_gdv = chptr6_gdv,
		chptr7_gdv = chptr7_gdv,
		chptr8_gdv = chptr8_gdv,
		chptr9_gdv = chptr9_gdv,
		dodgera_gdv = dodgera_gdv,
		dopple_gdv = dopple_gdv,
		ecredits_gdv = ecredits_gdv,
		effigya1_gdv = effigya1_gdv,
		effigya2_gdv = effigya2_gdv,
		effigya3_gdv = effigya3_gdv,
		effigyb_gdv = effigyb_gdv,
		effigyc_gdv = effigyc_gdv,
		enchant_gdv = enchant_gdv,
		epilogue_gdv = epilogue_gdv,
		epilogue_mp4 = epilogue_mp4,
		falshire_gdv = falshire_gdv,
		falshire_mp4 = falshire_mp4,
		flotrap1_gdv = flotrap1_gdv,
		flotrap2_gdv = flotrap2_gdv,
		flotrap3_gdv = flotrap3_gdv,
		flotrap4_gdv = flotrap4_gdv,
		flotrap5_gdv = flotrap5_gdv,
		flotrap6_gdv = flotrap6_gdv,
		fullkeep_gdv = fullkeep_gdv,
		gardens_gdv = gardens_gdv,
		gate01a_gdv = gate01a_gdv,
		gate01b_gdv = gate01b_gdv,
		gaul01_gdv = gaul01_gdv,
		gaul02_gdv = gaul02_gdv,
		glasswin_gdv = glasswin_gdv,
		gnarla_gdv = gnarla_gdv,
		gnarlb_gdv = gnarlb_gdv,
		gnarlc_gdv = gnarlc_gdv,
		gnarld_gdv = gnarld_gdv,
		gnarle_gdv = gnarle_gdv,
		gnarlf_gdv = gnarlf_gdv,
		gnarlg_gdv = gnarlg_gdv,
		gnarlh_gdv = gnarlh_gdv,
		gnarli_gdv = gnarli_gdv,
		gremlogo_gdv = gremlogo_gdv,
		hawk01_gdv = hawk01_gdv,
		hawk02a_gdv = hawk02a_gdv,
		hawk02b_gdv = hawk02b_gdv,
		hawk02c_gdv = hawk02c_gdv,
		hawk02d_gdv = hawk02d_gdv,
		hawk03a_gdv = hawk03a_gdv,
		hawk03b_gdv = hawk03b_gdv,
		hawk03c_gdv = hawk03c_gdv,
		hawk03d_gdv = hawk03d_gdv,
		hawk03e_gdv = hawk03e_gdv,
		hawk03f_gdv = hawk03f_gdv,
		hawk03g_gdv = hawk03g_gdv,
		hawk03h_gdv = hawk03h_gdv,
		hawk03i_gdv = hawk03i_gdv,
		hawk03j_gdv = hawk03j_gdv,
		hawk03k_gdv = hawk03k_gdv,
		hawk03l_gdv = hawk03l_gdv,
		hawk03m_gdv = hawk03m_gdv,
		hawk03n_gdv = hawk03n_gdv,
		hawk03o_gdv = hawk03o_gdv,
		hawk03p_gdv = hawk03p_gdv,
		hawk04_gdv = hawk04_gdv,
		instra_gdv = instra_gdv,
		instrb_gdv = instrb_gdv,
		instrc_gdv = instrc_gdv,
		instrd_gdv = instrd_gdv,
		instre_gdv = instre_gdv,
		instrf_gdv = instrf_gdv,
		instrg_gdv = instrg_gdv,
		instrh_gdv = instrh_gdv,
		instri_gdv = instri_gdv,
		intro_gdv = intro_gdv,
		movie_gdv = movie_gdv,
		randall_gdv = randall_gdv,
		raphael_gdv = raphael_gdv,
		raphaelh_gdv = raphaelh_gdv,
		raphaeli_gdv = raphaeli_gdv,
		raphaelj_gdv = raphaelj_gdv,
		raphaelk_gdv = raphaelk_gdv,
		raphaell_gdv = raphaell_gdv,
		raphaelm_gdv = raphaelm_gdv,
		raphaeln_gdv = raphaeln_gdv,
		raphaelo_gdv = raphaelo_gdv,
		ratpfa_gdv = ratpfa_gdv,
		ratpfb_gdv = ratpfb_gdv,
		raysel1a_gdv = raysel1a_gdv,
		raysel1b_gdv = raysel1b_gdv,
		raysel1c_gdv = raysel1c_gdv,
		raysel1d_gdv = raysel1d_gdv,
		raysel1e_gdv = raysel1e_gdv,
		raysel1f_gdv = raysel1f_gdv,
		raysel1g_gdv = raysel1g_gdv,
		rebecca_gdv = rebecca_gdv,
		reflecta_gdv = reflecta_gdv,
		reflectb_gdv = reflectb_gdv,
		reflectc_gdv = reflectc_gdv,
		ritual_gdv = ritual_gdv,
		shrivea_gdv = shrivea_gdv,
		shriveb_gdv = shriveb_gdv,
		shrivec_gdv = shrivec_gdv,
		shrived_gdv = shrived_gdv,
		talewela_gdv = talewela_gdv,
		talewelb_gdv = talewelb_gdv,
		talewelc_gdv = talewelc_gdv,
		tunnwarp_gdv = tunnwarp_gdv,
		verse_gdv = verse_gdv,
		walkway1_gdv = walkway1_gdv,
		walkway2_gdv = walkway2_gdv,
		walkway3_gdv = walkway3_gdv,
		walkwy1a_gdv = walkwy1a_gdv,
	}, "\t", false)


func is_valid() -> bool:
	for file: String in REQUIRED_FILES:
		if get(file).is_empty():
			return false
	return true


func verify_installation_integrity() -> void:
	pass
